...
void R3BFi11::Initialize()
{
    ...
    LOG(info) << "R3BFi11: initialisation";
    LOG(debug) << "R3BFi11: Vol. (McId) " << gMC->VolId("FI11Log");
}

void R3BFi11::SetSpecialPhysicsCuts()
{
    LOG(info) << "-I- R3BFi11: Adding customized Physics cut ... ";
...
void R3BFi11::Print(Option_t* option) const
{
    ...
    LOG(info) << "R3BFi11: " << nHits << " points registered in this event";
}
// ----------------------------------------------------------------------------
...
void R3BFi11::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset)
{
    ...
    LOG(info) << "R3BFi11: " << nEntries << " entries to add";
    ...
    LOG(info) << "R3BFi11: " << cl2->GetEntriesFast() << " merged entries";
}
...
{
    // printf("FI11 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> %s\n", name.c_str());
    if (TString(name).Contains("FI11LogActive"))
    {
        return kTRUE;
    }
    return kFALSE;
