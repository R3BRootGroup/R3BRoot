...
R3BFi23a::R3BFi23a(const TString& geoFile, const TGeoTranslation& trans, const TGeoRotation& rot)
    : R3BFi23a(geoFile, { trans, rot })
{
}

R3BFi23a::R3BFi23a(const TString& geoFile, const TGeoCombiTrans& combi)
    : R3BDetector("R3BFI23a", kFI23a, geoFile, combi)
    , fFi23aCollection(new TClonesArray("R3BFibPoint"))
    , fPosIndex(0)
    , kGeoSaved(kFALSE)
    , flGeoPar(new TList())
{
...
R3BFi23a::~R3BFi23a()
{
    ...
    if (fFi23aCollection)
    {
        fFi23aCollection->Delete();
        delete fFi23aCollection;
    }
}

void R3BFi23a::Initialize()
{
...
Bool_t R3BFi23a::ProcessHits(FairVolume* vol)
{
    ...
    // Set additional parameters at exit of active volume. Create R3BFi23aPoint.
...
void R3BFi23a::BeginEvent() {}

// -----   Public method EndOfEvent   -----------------------------------------
void R3BFi23a::EndOfEvent()
{
    if (fVerboseLevel)
        Print();
    fFi23aCollection->Clear();
...
void R3BFi23a::Register() { FairRootManager::Instance()->Register("Fi23aPoint", GetName(), fFi23aCollection, kTRUE); }
// ----------------------------------------------------------------------------

// -----   Public method GetCollection   --------------------------------------
TClonesArray* R3BFi23a::GetCollection(Int_t iColl) const
{
    if (iColl == 0)
        return fFi23aCollection;
    else
        return NULL;
}
// ----------------------------------------------------------------------------
...
R3BFibPoint* R3BFi23a::AddHit(Int_t trackID,
                              Int_t detID,
                              Int_t plane,
                              TVector3 posIn,
                              TVector3 posOut,
                              ...
                              Double_t eLoss)
{
    TClonesArray& clref = *fFi23aCollection;
...
Bool_t R3BFi23a::CheckIfSensitive(std::string name)
{
    if (TString(name).Contains("FI23ALogActive"))
    {
        return kTRUE;
    }
    return kFALSE;
