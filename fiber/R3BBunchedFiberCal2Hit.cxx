...
InitStatus R3BBunchedFiberCal2Hit::Init()
{
    ...
    */
    ...
    fh_ToT_MA_Fib = new TH2F(chistName.Data(), chistTitle.Data(), 2100, 0., 2100., 1000, 0., 400.);
    ...
    fh_Fib_ToF = new TH2F(chistName.Data(), chistTitle.Data(), 1024, 0, 1024, 10000, -1024., 1024.);
...
void R3BBunchedFiberCal2Hit::Exec(Option_t* option)
{
    ...
    for (auto it_mapmt = mapmt_array.begin(); mapmt_array.end() != it_mapmt; ++it_mapmt)
    {
        ...
        for (auto it_mapmt_tot = mapmt.tot_list.begin(); mapmt.tot_list.end() != it_mapmt_tot; ++it_mapmt_tot)
        {
            ...
            //                    mapmt_tot.tot<<endl; cout<<"S channels: "<<spmt_tot.lead->GetChannel()<<" ToT:
            //                    "<<spmt_tot.tot<<" mult: "<<s_mult<<endl;
            ...
            */
            ...
            if (tofdin)
            {
                ...
                Int_t nHitsTofd = detTofd->GetEntriesFast();
    ...
    if (fIsCalibrator)
    {
        ...
        if (tofdin)
        {
            for (UInt_t i = 1; i <= max; i++)
            {
                ...
                cout << "MA fiber: " << i << " tsync: " << proj->GetBinCenter(proj->GetMaximumBin()) << endl;
            }
        }
        // time offset
        /* MH
