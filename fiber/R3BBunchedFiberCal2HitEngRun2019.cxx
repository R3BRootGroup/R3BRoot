...
R3BBunchedFiberCal2HitEngRun2019::ToT::ToT(R3BBunchedFiberCalData const* a_lead,
...
R3BBunchedFiberCal2HitEngRun2019::R3BBunchedFiberCal2HitEngRun2019(const char* a_name,
                                               Int_t a_verbose,
...
R3BBunchedFiberCal2HitEngRun2019::~R3BBunchedFiberCal2HitEngRun2019()
{
    delete fHitItems;
    delete fCalPar;
}
...
*/
 ...
 // ToT MAPMT max:
   ...
   // tsapmt:
    ...
    // ToT vs ToT SPMT:
    chistName = fName + "_ToT1_ToT1ac";
    chistTitle = fName + " raw ToT1 vs ToT1ac of single PMTs";
    fh_ToT1_ToT2 = new TH2F(chistName.Data(), chistTitle.Data(), 500, 0., 100., 500, 0., 100.);
    fh_ToT1_ToT2->GetXaxis()->SetTitle("SPMT 1");
    fh_ToT1_ToT2->GetYaxis()->SetTitle("SPMT 1 ac");
...
InitStatus R3BBunchedFiberCal2HitEngRun2019::ReInit() { return kSUCCESS; }

void R3BBunchedFiberCal2HitEngRun2019::SetParContainers()
{
...
void R3BBunchedFiberCal2HitEngRun2019::Exec(Option_t* option)
{ 
...
//   cout<<"in exec "<<fName<<endl;
 ...
 //   cout<<"mapmt trig read "<<fName<<endl;
    ...
    for (size_t j = 0; j < spmt_trig_num; ++j)
    {
      ...
      if (idx >= spmt_trig_table.size()) spmt_trig_table.resize(idx + 1);
      spmt_trig_table.at(idx) = cal;
    }
      
 //   cout<<"sapmt trig read "<<fName<<endl;  
 ...
 //   cout<<"channel side read "<<fName<<endl;
   ...
   //     if(!cur_cal->IsMAPMT()) cout<<"Cal2Hit *** "<<fName<<", Nev="<<fnEvents<<", iHit="<<j<<", iCha="<<cur_cal->GetChannel() <<", isLead="<<cur_cal->IsLeading()<<", time="<<cur_cal->GetTime_ns()<<endl;
        ...
        if (cur_cal->IsTrailing())
        {
            ...
            Double_t c_period = 0 == side_i
              ? 4096. * (1000. / fClockFreq)     // CTDC
              : 2048. * (1000. / 200.);          // Tamex
            ...
            if (cur_cal->IsMAPMT() && fMAPMTTriggerMap)
            {
                ...
                if (cur_cal_trig_i < mapmt_trig_table.size() &&
                    lead_trig_i < mapmt_trig_table.size())
                {
            ...
            else if (cur_cal->IsSPMT() && fSPMTTriggerMap)
            {
                ...
                if (cur_cal_trig_i < spmt_trig_table.size() &&
                    lead_trig_i < spmt_trig_table.size())
                {
            ...
            auto cur_cal_ns = fmod(cur_cal->GetTime_ns() - cur_cal_trig_ns + c_period + c_period / 2, c_period) - c_period / 2;
          ...
          //  if(tot_ns < 0) cout<<fName<<"; "<<side_i<<"; "<<tot_ns<<", "<<cur_cal_ns <<", "<<lead_ns<<", "<<n_lead<<", "<<n_trail<<endl;

            
            if (side_i == 0) summmpt += 1;
            if (side_i == 1 && ch_i == 0) summsm1 += 1;
            if (side_i == 1 && ch_i == 1) summsm2 += 1;
            if (side_i == 1 && ch_i == 2) summsm3 += 1;
            if (side_i == 1 && ch_i == 3) summsm4 += 1;
            ...
            if (tot_ns < 1000. && tot_ns > 0.)
            {
                ...
                // t1<t2<t3....
               if(side_i == 0)  
                channel.tot_list.push_front(ToT(lead, cur_cal, lead_raw, trail_raw, lead_ns, cur_cal_ns, tot_ns, lead_trig_ns, trail_trig_ns));
                else
                channel.tot_list.push_back(ToT(lead, cur_cal, lead_raw, trail_raw, lead_ns, cur_cal_ns, tot_ns, lead_trig_ns, trail_trig_ns));
	...
	{
		if(!(mapmt_array.begin()==mapmt_array.end()))isumNMA += 1;
		auto const& mapmt = *it_mapmt;		
		Int_t isumNhitMA = 0; 
		//cout<<"list size "<<mapmt.tot_list.size()<<endl;
		for (auto it_mapmt_tot = mapmt.tot_list.begin(); mapmt.tot_list.end() != it_mapmt_tot; ++it_mapmt_tot) // over ihit(fiber)
		{
			...
			if(mapmt_tot.tot_ns < 2.) fh_lowMtot->Fill(isumNhitMA, mch + 1);
 		 ...
 		 // if(isumNhitMA > 1) continue;    // only first hit in each event
	         ...
	         for (auto it_spmt = spmt_array.begin(); spmt_array.end() != it_spmt; ++it_spmt)  // over No of SAPMT
             {
				...
				if(!(spmt.tot_list.end() == spmt.tot_list.begin())) isumNSA += 1;
				//if(isumNSA > 1) continue;
				
				for (auto it_spmt_tot = spmt.tot_list.begin(); spmt.tot_list.end() != it_spmt_tot; ++it_spmt_tot)  // over ihit in each SAPMT
				{ 
					auto const& spmt_tot = *it_spmt_tot;
					
					isumNhitSA += 1;
			
					if(spmt_tot.lead->GetChannel() == 1 ) s1 = spmt_tot.tot_ns;
					if(spmt_tot.lead->GetChannel() == 2 ) s2 = spmt_tot.tot_ns;
					if(spmt_tot.lead->GetChannel() == 3 ) s3 = spmt_tot.tot_ns;
					if(spmt_tot.lead->GetChannel() == 4 ) s4 = spmt_tot.tot_ns;
      ...
      //fh_ToT_SA_Fib_raw->Fill(spmt_tot.lead->GetChannel(), spmt_tot.tot_ns);  
        ...
        /*    while (dt_trig > 256. / 2. * 1000. / fClockFreq)
           ...
           // Double_t eloss = tot_mapmt;
            Double_t t = (t_spmt+t_mapmt)/2.; //tof;
           //  Double_t t = t_spmt;
            // Double_t t = t_mapmt;

            
			if((tof > -30. && tof < 30.) && (dtime > -20. && dtime < 20.)){	
				if(single == 1 && tot_spmt > 0.  ) s1_ac = tot_spmt;
				if(single == 2 && tot_spmt > 0.  ) s2_ac = tot_spmt;
				if(single == 3 && tot_spmt > 0.  ) s3_ac = tot_spmt;
				if(single == 4 && tot_spmt > 0.  ) s3_ac = tot_spmt;
		    }
         ...
         //   if((tof > -30. && tof < 30.) && (dtime > -20. && dtime < 20.) && eloss > 0. && tot_spmt < 50.){
                ...
                if(tot_mapmt > 0. && isumNhitSA == 1 && isumNSA == 1) summmpt_ac++;
                
                if( isumNhitMA == 1 && isumNMA == 1){
					if(single == 1 && tot_spmt > 0.) summsm1_ac++;
           ...
           //  }   
				...
				if (!fIsCalibrator)
					new ((*fHitItems)[fNofHitItems++])
						R3BBunchedFiberHitData(0, x, y, eloss, t, fiber_id, t_mapmt,
...
void R3BBunchedFiberCal2HitEngRun2019::FinishEvent()
{
    fHitItems->Clear();
    fNofHitItems = 0;
    Double_t t1,t2,t3,t4;
   if(cal_num > 0 && (summsm1_ac) > 0){ 
			Int_t norm = summmpt_ac;
			if(norm < 1) norm = 1;
			fh_hit1_ac->Fill(summmpt_ac,summsm1_ac);
...
void R3BBunchedFiberCal2HitEngRun2019::FinishTask()
{
    ...
    if (fIsCalibrator)
    {
        ...
        for (UInt_t i = 1; i <= max; i++)
        {
            ...
            if(i < 257) cout<<fName<<" Fiber: "<< i<<", offset: "<< 0.5 * proj1->GetBinCenter(proj1->GetMaximumBin())<<endl;
            ...
            // gain MA     
                ...
                for (UInt_t j = proj->GetNbinsX() - 2; j > 2; j--)
                {
                    ...
                    if (proj->GetBinContent(j) > proj->GetMaximum() / 100.)
                    {
                      ...
                      if(i < 257) cout<<fName<<" Fiber: "<< i<<" gain MA: "<<proj->GetBinCenter(j)<<endl;
                        break;
                    }
                }
             // gain SA
                ...
                for (UInt_t j = proj3->GetNbinsX() - 2; j > 2; j--)
                {
                    ...
                    if (proj3->GetBinContent(j) > proj3->GetMaximum() * 20. / 100.)
                    {
                        ...
                        par3->SetGainS(proj->GetBinCenter(j));
                         ...
                         if(i < 257) cout<<fName<<" Fiber: "<< i<<" gain SA: "<<proj3->GetBinCenter(j)<<endl;
                        break;
                    }
                }
...
void R3BBunchedFiberCal2HitEngRun2019::MAPMTTriggerMapSet(unsigned const* a_map, size_t a_map_bytes)
{
...
void R3BBunchedFiberCal2HitEngRun2019::SPMTTriggerMapSet(unsigned const* a_map, size_t a_map_bytes)
{
    size_t const c_length = a_map_bytes / sizeof *a_map;
...
ClassImp(R3BBunchedFiberCal2HitEngRun2019)
