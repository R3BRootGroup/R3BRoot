...
void R3BBunchedFiberCal2HitEngRun2019::Exec(Option_t* option)
{
    ...
    for (size_t j = 0; j < cal_num; ++j)
    {
        ...
        //     if(!cur_cal->IsMAPMT()) cout<<"Cal2Hit *** "<<fName<<", Nev="<<fnEvents<<", iHit="<<j<<",
        ...
        if (cur_cal->IsTrailing())
        {
            ...
            //  if(tot_ns < 0) cout<<fName<<"; "<<side_i<<"; "<<tot_ns<<", "<<cur_cal_ns <<", "<<lead_ns<<",
            //  "<<n_lead<<", "<<n_trail<<endl;
...
void R3BBunchedFiberCal2HitEngRun2019::FinishTask()
{
    ...
    if (fIsCalibrator)
    {
        ...
        for (UInt_t i = 1; i <= max; i++)
        {
            ...
            if (i < 257)
                cout << fName << " Fiber: " << i << ", offset: " << 0.5 * proj1->GetBinCenter(proj1->GetMaximumBin())
                     << endl;
