...
//   ----- General Macro to create the geometry of Fiber-30 detector
//
...
void create_fi30_geo(std::string geoTag = "v2022.1")
{
    ...
    TString geoFileName = geoPath + "/geometry/fi30_";
    ...
    auto* pMatrix3 = new TGeoRotation();
    auto* pMatrix2 = new TGeoCombiTrans("", dx, dy, dz, pMatrix3);
    auto* pMatrix21 = new TGeoCombiTrans("", dx, dy, dz + fiber_thickness, pMatrix3);
    auto* pMatrix22 = new TGeoCombiTrans("", dx, dy, dz - fiber_thickness, pMatrix3);

    // World definition
    auto* pWorld = gGeoManager->GetTopVolume();
    pWorld->SetVisLeaves(kTRUE);

    auto* airbox = gGeoManager->MakeBox(
        "Fiber30World", pMed2, detector_width / 2. + 1, detector_height / 2., fiber_thickness * 2.);
    airbox->SetLineColor(2);
    airbox->SetVisLeaves(kTRUE);

    auto fFiber = new TGeoTube("FiberTube", 0, fiber_thickness / 2, detector_height / 2.);
    auto fFiberLog = new TGeoVolume("Fi30", fFiber, pMed35);
    ...
    auto* pMatrixTube = new TGeoRotation();
