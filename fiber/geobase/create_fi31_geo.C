...
//   ----- General Macro to create the geometry of Fiber-31 detector
//
...
void create_fi31_geo(std::string geoTag = "v2022.1")
{
    ...
    TString geoFileName = geoPath + "/geometry/fi31_";
    ...
    TGeoVolume* airbox = gGeoManager->MakeBox(
        "Fiber31World", pMed2, detector_width / 2. + 1, detector_height / 2., fiber_thickness * 2.);
    airbox->SetLineColor(2);
    airbox->SetVisLeaves(kTRUE);

    auto fFiber = new TGeoTube("FiberTube", 0, fiber_thickness / 2, detector_height / 2.);
    auto fFiberLog = new TGeoVolume("Fi31", fFiber, pMed35);
