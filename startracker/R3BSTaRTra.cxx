...
R3BSTaRTra::R3BSTaRTra(const TString& geoFile, const TGeoTranslation& trans, const TGeoRotation& rot)
    : R3BSTaRTra(geoFile, { trans, rot })
{
}

R3BSTaRTra::R3BSTaRTra(const TString& geoFile, const TGeoCombiTrans& combi)
    : R3BDetector("R3BSTaRTra", kSTaRTrack, geoFile, combi)
                                    ...
                                    Double_t eLoss)
{
    ...
    if (fVerboseLevel > 1)
        LOG(INFO) << "R3BSTaRTra: Adding Point at (" << posIn.X() << ", " << posIn.Y() << ", " << posIn.Z()
