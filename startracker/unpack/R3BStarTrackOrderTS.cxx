...
InitStatus R3BStarTrackOrderTS::Init()
{
	...
	//	fh_tdc[i]=new TH1F(Form("h_tdc_%d",i), Form("TDC distribution channel %d", i), 128, 0., 4096.);

	//	run->AddObject(fh_tdc[i]);  // Added for web-base event display
	//}
...
//	for(Int_t i=0; i<7791; i++)
	...
	//				if(ts_sort_flag==1) cout << "ts_sort_flag=" <<  ts_sort_flag << endl;
		...
		if(no_sync_flag){  // let's fill the vectors ts_temp, index_block, index_hit_temp for word of type 3
	      ...
	      //cout << "n_firstType3= " << n_firstType3 << endl;	  
	  ...
	  for(long long j=0; j<n_loop; j++){	    //transport sorted data to output_tree
	    ...
	    fill_flag=true;
	      ...
	      if(fill_flag && (my_tm_stp/1e9 >1000))
	      {
		  ...
		  //cout << " filling my_tm_stp_ext=" << my_tm_stp_ext << endl; // 
	           //cout << "n_loop=" << n_loop << endl;	  
	           //cout << "n_first=" << n_first << endl;	  
		  s_entry.tm_stp_ext = my_tm_stp_ext/1e9; // 
		  //s_entry.nhit        = j+n_first; // This is reset at each new block
		  s_entry.nhit        = fTotalHits; 
		  //s_entry.nhit        = TotalEvent+TotalHit+j; // j+n_first;
		  //s_entry.nhit        = TotalEvent;
...
void R3BStarTrackOrderTS::FinishTask()
{
	...
	//for(Int_t i =0; i <16; i++)
	//{
	//LOG(INFO) << "Number of entries in channel" << i << ": " << fh_tdc[i]->GetEntries() << FairLogger::endl;
