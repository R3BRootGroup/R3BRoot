...
# Last modification: 13/07/10
# 
# Program to create a TGeo file with the full CALIFA 7.05 (also called Version A)
# Uses the information extracted from the STEP file for the CALIFA 7.05 alveoli
...
@crystalLength=(18.0,18.0,17.0,17.0,16.0,16.0,15.0,15.0,14.0,14.0,13.0,13.0);
###################################################################################
# Input files
$INPUTFILE1 = 'CLF705_AlveolarPositionsDirectionsOrderedCorrected.dat';
$INPUTFILE2 = 'CLF705_Dimensions_FiberBoxBarrel.dat';
$INPUTFILE3 = 'CLF705_Vertex_Crystal_all.dat';
$OUTPUTFILE = 'CLF705_Geometry.geo';
...
print OUTPUTFILE " //Generated ",$date , " from macro createGeometryCLF705.pl by Hector Alvarez Pol (hector.alvarez\@usc.es) \n";
print OUTPUTFILE "\n";
...
for ($h = 0; $h<=$#Alv_dx1; $h++) {
	for($m = 0; $m<4; $m++) {		
		...
		#############################################################################################################
		if($zy[$h*4+$m]<0.) {
			print OUTPUTFILE "\t phiEuler = 180.*TMath::ATan2(",$zx[$h*4+$m],",",-$zy[$h*4+$m],")/TMath::Pi(); \n";
			print OUTPUTFILE "\t thetaEuler = 180.*TMath::ATan2(",$zx[$h*4+$m],"/TMath::Sin(TMath::Pi()*phiEuler/180.),",$zz[$h*4+$m],")/TMath::Pi(); \n";
			print OUTPUTFILE "\t psiEuler = 180.*TMath::ASin(",$xz[$h*4+$m],"/TMath::Sin(TMath::Pi()*thetaEuler/180.))/TMath::Pi(); \n\n";
		}	
		else {
			print OUTPUTFILE "\t phiEuler = 180.*TMath::ATan2(",-$zx[$h*4+$m],",",$zy[$h*4+$m],")/TMath::Pi(); \n";
			print OUTPUTFILE "\t thetaEuler = 180.*TMath::ATan2(",$zx[$h*4+$m],"/TMath::Sin(TMath::Pi()*phiEuler/180.),",$zz[$h*4+$m],")/TMath::Pi(); \n";
		...
		print OUTPUTFILE "\t TGeoRotation *rotAlv_",$h+1,"_",$m," = new TGeoRotation(\"rotAlv",$h+1,"_",$m,"\",phiEuler,thetaEuler,psiEuler); \n";
		
		for($rot=0; $rot<10; $rot++) {		
			if($rot==0) { 			
				print OUTPUTFILE "\t TGeoCombiTrans* transAlv_",$h+1,"_",$rot*4+$m," = new TGeoCombiTrans(",$x[$h*4+$m],",",$y[$h*4+$m],",",$z[$h*4+$m],",rotAlv_",$h+1,"_",$m,"); \n";
				# the shift in the alveoli center to one of the faces should be included: transDef makes the required transformation!
				print OUTPUTFILE "\t *transAlv_",$h+1,"_",$rot*4+$m," =  (*transAlv_",$h+1,"_",$rot*4+$m,") * (*transDef); \n";
			}
			else {
				# the shift in the alveoli center plus the rotation 
				print OUTPUTFILE "\t rotAngle = TMath::Pi()*36.*",$rot,"/180.;  \n"; 
				print OUTPUTFILE "\t rotAlv_",$h+1,"_",$m,"->RotateZ(-36.);     \n";
				print OUTPUTFILE "\t TGeoCombiTrans* transAlv_",$h+1,"_",$rot*4+$m," = new TGeoCombiTrans(",$x[$h*4+$m],"*cos(rotAngle)+",$y[$h*4+$m],"*sin(rotAngle),",-$x[$h*4+$m],"*sin(rotAngle)+",$y[$h*4+$m],"*cos(rotAngle),",$z[$h*4+$m],",rotAlv_",$h+1,"_",$m,"); \n";
				print OUTPUTFILE "\t *transAlv_",$h+1,"_",$rot*4+$m," =  (*transAlv_",$h+1,"_",$rot*4+$m,") * (*transDef); \n";
			}
			print OUTPUTFILE "\t pWorld->AddNode(Alveolus_",$h+1,",",$rot*4+$m,",transAlv_",$h+1,"_",$rot*4+$m,"); \n\n";
		}
	}
}		
close(INPUTFILE);
