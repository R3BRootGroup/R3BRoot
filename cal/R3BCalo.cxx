...
//                                        23/08/11 by E Fiori (silent output)
...
R3BCalo::R3BCalo() : R3BDetector("R3BCalo", kTRUE, kCALIFA) {
  ...
  //  fVerboseLevel = 1;
  fVerboseLevel = 0; //changed to non verbose EF 23/08/11
  fNonUniformity = 0.;
  fGeometryVersion = 1;
}
// -------------------------------------------------------------------------
...
R3BCalo::R3BCalo(const char* name, Bool_t active) 
  ...
  flGeoPar->SetName( GetName());
  //fVerboseLevel = 1;
  fVerboseLevel = 0;  //non verbose mode EF 23/08/11
  fNonUniformity = 0.;
  fGeometryVersion = 1;
}
// -------------------------------------------------------------------------
...
//TGeoNode *bla = gGeoManager->FindNextBoundary(2);
	...
	*/  // non verbose EF 23/08/11 
	...
	Double_t polarAngle=0.;			// EF  caloHits reconstructed polar angle
	Double_t azimuthalAngle=0.;		// EF  caloHits reconstructed azimuthal angle
	Double_t rhoAngle=0.;		// EF  caloHits reconstructed crystal distance from center 

	GetAngles(crystalId,&polarAngle,&azimuthalAngle, &rhoAngle);     // EF
	  
	if(nCrystalHits==0) AddCrystalHit(crystalType , crystalCopy , crystalId, NUSmearing(fELoss), fTime, polarAngle, azimuthalAngle, rhoAngle);
	else {
	  ...
	  if(!existHit) AddCrystalHit(crystalType , crystalCopy , crystalId, NUSmearing(fELoss), fTime, polarAngle, azimuthalAngle, rhoAngle);	
	}
...
R3BCaloCrystalHit* R3BCalo::AddCrystalHit(Int_t type, Int_t copy, Int_t ident,
	...
	return new(clref[size]) R3BCaloCrystalHit(type, copy, ident, energy, time, theta, phi, rho);
}
...
void R3BCalo::GetAngles(Int_t iD, Double_t* polar, Double_t* azimuthal, Double_t* rho)
{
