...
module calib
 ...
 double precision                :: chi_treshold= 2.d1					 ! chi2-cut off to be counted as good event
...
subroutine multi_track_from_cpp(array_size,n_points,det_coordinates,double_track, local_target_position, &
 ...
 integer n_det_track_old, n_det_track_old1, n_det_track_old2
 integer n_det_track, n_det_track1, n_det_track2
 integer n_points, array_size
 logical*1 det_coordinates, double_track,  left, right, new_combination
 logical all_done, all_done1, all_done2, skip_combination, skip
 logical change_status1(nbr_detectors),change_status2(nbr_detectors)
...
subroutine invert_matrix(n,a_in,a_inv_out,ok)
 implicit none
 integer n,k
 double precision a_in(n,n),a_inv_out(n,n),e_vector(n),x_vector(n), a_lgs(n,n), a_gauss(n,n)
 logical ok
!
...
subroutine vary_parameter(var,io_unit)
 ...
 character (LEN=length_detector_name) :: detector_name_local,dummy_ch
...
subroutine initialize_y_corrections
 use vars
 implicit none
 integer io_unit, d, var, var2
 logical debug_track_backup,ok
 double precision dy_theta, dx_theta, theta_max, dy_th, dx_th, nbr_runs
 double precision dy_interactions(nbr_detectors), y_interactions_0(nbr_detectors), incr
 double precision vector(nbr_detectors,nbr_functions_y_correction), x_interactions(nbr_detectors), y_interactions(nbr_detectors)
...
subroutine get_detector_position_spectra_parameter
 ...
 integer i, j, k, channel, minimum, maximum, max_dim
!
 do i=1,nbr_detectors
...
!
 ...
 if (get_new_derivatives) then
   ...
   if (.not. sweep_calibration) write(output_unit,*) 'NEW derivatives will be stored in: ',trim(derivative_file)
 else
   if (.not. sweep_calibration) write(output_unit,*) 'Derivatives definition: ',trim(derivative_file)
 end if 
!
 do i=1,3
...
double precision function basic_functions_y_correction(n,x,y)
 ...
 double precision basic_functions_y_correction_deg5, basic_functions_y_correction_deg3
!
...
subroutine initialize_trigger_logic
 use vars
 implicit none
 integer d, i, det_pos_in_string,pos_in_string,pattern_id, d1, d2
...
subroutine make_event_sweep_detektor_position_calibration
 ...
 implicit none
 integer i,j,d,d1
 logical break_out, all_detectors_done, detector_done
 logical dimension_free(nbr_detectors,nbr_dimensions_optimize),detector_free(nbr_detectors)
 double precision qs_0, qs_old, norm(2), qs_vary
 double precision d_qs(3),dx_local(3)
 double precision d_det_pos(4), detector_position_in_ref(nbr_detectors,4)
 double precision d_det_pos_lab(3,4), d_det_pos_det(3)
...
!
         ...
         break_out = ((d_hits_m(1)*d_hits_p(1) < 0.d0).or.(d_hits_m(1)== 0.d0 .and. d_hits_p(1) == 0.d0))
         break_out = ((d_hits_m(1)*d_hits_p(2) < 0.d0).or.(d_hits_m(2)== 0.d0 .and. d_hits_p(2) == 0.d0)) .and. break_out
         break_out = ((d_hits_m(1)*d_hits_p(3) < 0.d0).or.(d_hits_m(3)== 0.d0 .and. d_hits_p(3) == 0.d0)) .and. break_out
         if (break_out) then
           detector_done = .true.
         else
           detector_done = .false.
           d_hits        = (d_hits_m + d_hits_p) / 2.d0
           norm(1)       = dsqrt(dot_product(d_hits,d_hits))
           dx_local      = 0.d0
           do i=1,3
             dx_local  = dx_local + d_hits(i)/norm(1)*d_det_pos_lab(i,1:3)/2.
           end do  
           d_det_pos(1:3) = dx_local     
           detector_position_in_lab(d,:) = detector_position_in_lab(d,:) + d_det_pos(:)
           do i=1,3
             do d1 = d+1,nbr_detectors
               if (locked_pair(d,d1,i)) then
                 detector_position_in_lab(d1,1:3) = detector_position_in_lab(d1,1:3) + d_hits(i)/norm(1)*d_det_pos_lab(i,1:3)/2.
               end if  
             end do
           end do
           call calculate_quadratic_deviations(qs_0,sum_0)
           ...
           all_detectors_done = detector_done .and. all_detectors_done
         end if
       end if      
     end do
 ...
 do d=1, nbr_detectors
  ...
  do run=1, max_dim_pos_spectra/2
   if (10*sum_hits_histo(d,run)> maxval(sum_hits_histo(d,:)) ) then
     ...
     print*,b_field_sweep(run), sum_hits_histo(d,run),ave(1), delta(1), sigma(1)
     print*,b_field_sweep(run), sum_hits_histo(d,run+nbr_runs),ave(2), delta(2), sigma(2)
   end if 
  end do  
  print*
 end do
...
subroutine calculate_quadratic_deviations(qs_event_by_event,sum_hits)
 ...
 integer run,hit, event, d, i,new_hit,det_pattern_id, pattern_id, d1, d2
 ...
 double precision chi_parameter_out(6) 
 integer counter_local(2,nbr_runs),sum_hits
...
!             n_det_track = n_det_track + 1
             ...
             if (det_step_function(d,1) ) then
               if (dabs(dpos)> sigma_track(d,1)) then
                 chi2_single(1) = 1.d35
                 exit
               end if
             else
         ...
         new_combination = (n_det_track > n_det_track_old)
         new_combination = new_combination .or. ((n_det_track == n_det_track_old) .and. chi2_single(1) < chi_parameter_out(1))
!         new_combination = (chi2_single(1) < chi_parameter_out(1))
         if (new_combination) then
           chi_parameter_out(1:2)   = chi2_single   
           n_det_track_old          = n_det_track
     ...
     if (chi_parameter_out(1) < 1.d35) then                            ! some valid combination found
       counter_local(1,run)   = counter_local(1,run) + 1                ! some good combination found  
  !     call get_single_track_parameter_var5                             ! this is just a test. it is wrong, since we have the wrong matrices
  !     call get_incoming_momentum(x_track1(4:6))
!       if (chi_parameter_out(1) < 3.d1) then
...
!print*,'qs_event_by_event ', qs_event_by_event
 if (max_nbr_good_events == max_nbr_events_run) max_nbr_good_events = minval(counter_local(1,:))
end 
...
subroutine make_sweep_incoming_beam_calibration
 use vars
 implicit none
 integer unit_read,i,j,eastat, d
 integer, parameter :: max_nbr_points = 1000
 double precision derivative_pos(3)
 double precision local_current, x_pos
 double precision detector_ref_track_interactions_det_frame(nbr_detectors,3)
 ...
 double precision qs, qs_new, qs_old, qs_0, d_qs(2), norm, vec3(3)
!
...
subroutine make_sweep_b_field_position_calibration
 ...
 double precision x_track(max_nbr_points), c_track(max_nbr_points)
 integer          d_track(max_nbr_points), nbr_points
 double precision qs, qs_new, qs_old, qs_0, d_qs(3), dx_local(3), dx_b_0(3), norm
!
...
subroutine make_sweep_detector_position_calibration
 ...
 double precision local_track_lab_frame(nbr_detectors,3), local_current, x_pos
 ...
 double precision saxaz(nbr_detectors), theta_x, theta_y, new_positions(nbr_detectors,2)
 integer,parameter ::  n_local=2
 double precision m_local(n_local,n_local),v1_local(n_local),v2_local(n_local)
 double precision mi_local(n_local,n_local), test(n_local,n_local), quadratic_sum(2)
!
...
subroutine write_new_geometry_file
 ...
 use calib
 implicit none
 integer unit_write,eastat, d,j
!
 unit_write = 77
 open (unit=unit_write, file=geometry_outpout_file)
