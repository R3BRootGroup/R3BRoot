...
#include "R3BFi23aDigitizerCal.h"
...
R3BFi23aDigitizerCal::R3BFi23aDigitizerCal()
    : FairTask("R3B Fi23a Digitization scheme ", 1)
    , fFi23aPoints(NULL)
{
}

R3BFi23aDigitizerCal::R3BFi23aDigitizerCal(const char* name, Int_t iVerbose)
...
InitStatus R3BFi23aDigitizerCal::Init()
{
    ...
    fFi23aPoints = (TClonesArray*)ioman->GetObject("Fi23aPoint");

    fMCTrack = (TClonesArray*)ioman->GetObject("MCTrack");

    // Register output array Fib13Digi
    fFi23aCals = new TClonesArray("R3BBunchedFiberCalData", 1000);
    ioman->Register("Fi23aCal", "Digital response in Fi23a", fFi23aCals, kTRUE);

    fFi23aTriggerCals = new TClonesArray("R3BBunchedFiberCalData", 1000);
    ioman->Register("Fi23aTriggerCal", "Digital response in Fi23a", fFi23aTriggerCals, kTRUE);
...
void R3BFi23aDigitizerCal::Exec(Option_t* opt)
{
    // cout << "New event!" << endl;
        ...
        for (Int_t i = 0; i < NumOfFibers; ++i)
        {
            for (Double_t& energyl : energy[i])
            {
                if (energyl > 0.000001)
                {
                     ...
                     * How to get a fiber ID for a fiber detector Fib1a-Fib13 defined as:
                     *  SubNum = 2
                     *  MAPMT = 256
                     *  SPMT = 2
    ...
    if (fFi23aPoints)
    {
        Digitize(fFi23aPoints, fFi23aCals, fFi23aTriggerCals, fiber_nbr);
    }
}
// -------------------------------------------------------------------------

void R3BFi23aDigitizerCal::Reset()
{
    if (fFi23aCals)
        fFi23aCals->Clear();
    if (fFi23aTriggerCals)
        fFi23aTriggerCals->Clear();
}

void R3BFi23aDigitizerCal::Finish() {}

ClassImp(R3BFi23aDigitizerCal)
