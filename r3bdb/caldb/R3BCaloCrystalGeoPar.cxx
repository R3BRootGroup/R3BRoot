...
ClassImp(R3BCaloCrystalGeoPar);

#include "FairDbReader.tpl"
template class  FairDbReader<R3BCaloCrystalGeoPar>;

#include "FairDbWriter.tpl"
template class  FairDbWriter<R3BCaloCrystalGeoPar>;



R3BCaloCrystalGeoPar::R3BCaloCrystalGeoPar(const char* name, const char* title, const char* context, Bool_t own)
...
R3BCaloCrystalGeoPar::~R3BCaloCrystalGeoPar()
{
...
void R3BCaloCrystalGeoPar::putParams(FairParamList* list)
{
  std::cout<<"-I- R3BCaloCrystalGeoPar::putParams() called"<<std::endl;
  if(!list) { return; }
...
Bool_t R3BCaloCrystalGeoPar::getParams(FairParamList* list)
{
...
void R3BCaloCrystalGeoPar::clear()
{
...
string R3BCaloCrystalGeoPar::GetTableDefinition(const char* Name)
{

  string sql("create table ");
  if ( Name ) { sql += Name; }
  else { sql += "R3BCALOCRYSTALGEOPAR"; }
...
void R3BCaloCrystalGeoPar::Fill(FairDbResultPool& res_in,
...
void R3BCaloCrystalGeoPar::Store(FairDbOutTableBuffer& res_out,
...
void R3BCaloCrystalGeoPar::fill(UInt_t rid)
{
  ...
  for (int i = 0; i < numRows; ++i) {
    R3BCaloCrystalGeoPar* cgd = (R3BCaloCrystalGeoPar*) fParam_Reader->GetRow(i);
    if (!cgd) { continue; }
...
void R3BCaloCrystalGeoPar::store(UInt_t rid)
{
  ...
  if ( ! stmtDbn.get() ) {
    cout << "-E-  R3BCaloCrystalGeoPar::Store()  Cannot create statement for Database_id: " << GetDbEntry()
  ...
  if (! fMultConn->GetConnection(GetDbEntry())->TableExists("R3BCALOCRYSTALGEOPAR") ) {
    sql_cmds.push_back(FairDb::GetValDefinition("R3BCALOCRYSTALGEOPAR").Data());
    sql_cmds.push_back(R3BCaloCrystalGeoPar::GetTableDefinition());
  }
  ...
  while( itr != itrEnd ) {
    ...
    if ( stmtDbn->PrintExceptions() ) {
      fail = true;
      cout << "-E- R3BCaloCrystalGeoPar::Store() ******* Error Executing SQL commands ***********  " << endl;
    }
  ...
  *fParam_Writer<< (*this);
  ...
  if ( !fParam_Writer->Close() ) {
    fail = true;
    cout << "-E- R3BCaloCrystalGeoPar::Store() ******** Cannot do IO on class: " << GetName() <<  endl;
  }

}

void R3BCaloCrystalGeoPar::Print()
{
  std::cout<<"   R3BCaloCrystalGeoPar  TRAP Parameters: "<<std::endl;
...
Bool_t R3BCaloCrystalGeoPar::SetShape(TGeoArb8*  trap ){
...
Bool_t R3BCaloCrystalGeoPar::Compare(const R3BCaloCrystalGeoPar& that ) const {
  //  
  ...
  for(Int_t i=0; i<NVERTICES;i++){
    ...
    if ( !a || !b ){
	  cout << "-E- R3BCaloCrystalGeoPar::Compare() no vertices ! " << endl;
	  test_d = kFALSE;
      break;
    }      
...
void R3BCaloCrystalGeoPar::InitArray(){
  // Remap TObjArray
  if (!fVertices){ 
	fVertices = new TObjArray(NVERTICES);
