  ...
  for (Int_t i = 0; i < 2; i++)
    {
      ...
      printf ("-I- R3BAladinFieldMap ---> %d: cos %7.4f sin %7.4f\n",
  ...
  for (unsigned int i = 0; i < measured_I.GetSize(); i++)
    {
      ...
      cout << "-I-R3BAladinFieldMap opening Field Map file : " << fMapFileName <<endl;

      fin = fopen(fMapFileName,"r");

      if (!fin)
	  cout << "-E- Failure opening field map : " << fMapFileName << endl;;
      ...
      for (int line = 0; !feof(fin); line++)
	{
	  ...
	  int n = fscanf(fin," %d %d %d %d %d %f %f %f %f\n",&I,&rl,
			 &ixyz[2],&ixyz[1],&ixyz[0],
			 &bxyz[0],&bxyz[1],&bxyz[2],
			 &bdummy);

	  if (n != 9)
	      cout << "-E- Failure parsing field from map:" << filename <<  " @ line: % "
		   << line << endl;

	  if (I != measured_I[i])
	      cout << "-E- Wrong current " << I << " when parsing field from map "<< filename
		   << " @ line: " << line << endl;

	  if (rl != 0 && rl != 1)
	      cout << "-E- Wrong box " << rl << " when parsing field from map "
		   << filename << " @line: " << line << endl;
	  ...
	  for (int j = 0; j < 3; j++)
	    {
	      ...
	      if (ixyz[j] < 0 ||
		  ixyz[j] >= field->f[rl][0]._np[j])
		  printf("-E- Wrong coordinate(%d) (%d -> %d) >= %d when parsing field from map %s, line? %d.",
	...
	for (int rl = 0; rl < 2; rl++)
      ...
      printf ("-I- R3BAladinFieldMap:  Reading  field map: %s\n",filename);
    }
	  ...
	  for (i[2] = 0; i[2] < f[0]._np[2]-1; i[2]++)
	      ...
	      for (int c = 0; c < 3; c++)
		{
		  ...
		  for (ai[a] = 0; ai[a] < 2; ai[a]++)
			...
			ai[c] = 0;
			double v1 = 
			  f[c].get_data_pt(i[0]+ai[0],i[1]+ai[1],i[2]+ai[2]);
			ai[c] = 1;
			double v2 = 
			  f[c].get_data_pt(i[0]+ai[0],i[1]+ai[1],i[2]+ai[2]);
		  ...
		  double dFa_da_var = 
		    (sum_dFa_da_sqr - sum_dFa_da*sum_dFa_da / 4)/3;;
		...
		printf ("%5.2f ",divF / sqrt(divF_var));      
	    }
	  printf ("\n");
	}
    }
...
void R3BAladinFieldMap::InitField()
{
  ...
  if (iter2 == gMapIFieldOrig.begin())
    {
	cout << "-E- R3BAladinFieldMap: Cannot interpolate ALADiN field for current " << current
	     << " A, too low." << endl;
  ...
  if (!fCurField)
    cout << "-E  R3BAladinFieldMap: ALADiN field interpolation, memory allocation failure." << endl;
  ...
  for (int rl = 0; rl < 2; rl++)
    for (int j = 0; j < 3; j++)
      {
	fCurField->f[rl][j].interpolate(iter1->second->f[rl][j],w1,
					  iter2->second->f[rl][j],w2);
      }
