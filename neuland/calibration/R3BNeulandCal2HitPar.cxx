...
#define PLANE_IS_HORZ(pl) ((pl & 1) == 1 ? 0 : 1)
...
bool n_calib_mean::analyse_history(ident_no_set& bad_fit_idents)
{
    ...
    {
        for (UInt_t k = 0; k < n; k++)
            if (od[k] < min_accept || od[k] > max_accept)
            {
...
InitStatus R3BNeulandCal2HitPar::Init()
{
    ...
    _collect_diff.resize(fPlanes, std::vector<n_calib_diff>(fPaddles));
    _collect_mean_within.resize(fPlanes, std::vector<n_calib_mean>(fPaddles - 1));
    _collect_mean_cross.resize(fPlanes - 1,
                               std::vector<std::vector<n_calib_mean>>(fPaddles, std::vector<n_calib_mean>(fPaddles)));
    bars.resize(fPlanes, std::vector<bar*>(0));
...
void R3BNeulandCal2HitPar::Exec(Option_t* option)
{
    ...
    for (;;)
    {
        ...
        LOG(DEBUG1) << "x hits: " << x_hit_fPlanes << "(" << (Int_t)x_plot->GetN() << ") "
                    << "y hits: " << y_hit_fPlanes << "(" << (Int_t)y_plot->GetN() << ")";
        LOG(DEBUG1) << "x = " << x0 << dxdz << " * z";
        LOG(DEBUG1) << "y = " << y0 << dydz << " * z";
                ...
                for (UInt_t pdl = 0; pdl < fPaddles - 1; pdl++)
                {
                    ...
                    else
                    {
                        ...
                        if (fabs(dydz) > 0.5)
                        {
                            ...
                            if (dydz * dxdz > 0)
                                dmm._mean_corr = (Float_t)(horz_flight_path * (PADDLE_SPACING / C_LIGHT));
                            else
                                dmm._mean_corr = (Float_t)(horz_flight_path * (-PADDLE_SPACING / C_LIGHT));
                        }
                    }
                ...
                for (UInt_t pdl1 = 0; pdl1 < fPaddles; pdl1++)
                {
                    ...
                    for (UInt_t pdl2 = 0; pdl2 < fPaddles; pdl2++)
                    {
                        ...
                        if (fabs(dydz) > 0.5)
                        {
                            ...
                            if (dydz < 0)
                            { // downwards when increasing z, so plane 2 is later
                ...
                for (Int_t pdl = 0; pdl < fPaddles; pdl++)
                {
                    ...
                    if (hit == 2)
                    {
                        ...
                        if (dist > 4 && dist < 10)
                        {
                            ...
                            if (PLANE_IS_HORZ(pl))
                                _ecalgraphs[pl][pdl]->SetPoint(_ecalgraphs[pl][pdl]->GetN(),
                                                               (p[0] + 0.5 * (p[1] - p[0])).X(),
                                                               log(b->fQdc[0] / b->fQdc[1]));
                            else
                                _ecalgraphs[pl][pdl]->SetPoint(_ecalgraphs[pl][pdl]->GetN(),
                                                               (p[0] + 0.5 * (p[1] - p[0])).Y(),
                                                               log(b->fQdc[0] / b->fQdc[1]));
    ...
    */
    ...
    Double_t tsync[fPlanes][fPaddles][2];
    {
        ...
        */
        ...
        for (Int_t pl = 0; pl < fPlanes; pl++)
            for (Int_t pdl = 0; pdl < fPaddles - 1; pdl++)
                if (TMath::Finite(mean_diff_within_t[pl][pdl]._e2_inv) && mean_diff_within_t[pl][pdl]._e2_inv > 0)
                {
                    Double_t weight = sqrt(mean_diff_within_t[pl][pdl]._e2_inv);

                    syncer_t.add_sync_pair(pl * fPaddles + pdl,
                                           -weight,
                                           pl * fPaddles + pdl + 1,
                                           weight,
                ...
                for (Int_t pdl2 = 0; pdl2 < fPaddles; pdl2++)
                        ...
                        mean_diff_cross_t[pl][pdl1][pdl2]._e2_inv > 0)
                    {
                        ...
                        */
                        ...
                        syncer_t.add_sync_pair((pl)*fPaddles + pdl1,
                                               -weight,
                                               (pl + 1) * fPaddles + pdl2,
                                               weight,
    ...
    for (Int_t pl = 0; pl < fPlanes; pl++)
        for (Int_t pdl = 0; pdl < fPaddles; pdl++)
        {
            ...
            ecalerr[pl][pdl][0] = MINIMUM_IONIZING * pow(k0k1 * k0dk1, -1.5) *
                                  sqrt((k0dk1 * k0k1err) * (k0dk1 * k0k1err) + (k0k1 * k0dk1err) * (k0k1 * k0dk1err));
            ecalerr[pl][pdl][1] = MINIMUM_IONIZING * pow(k0k1 / k0dk1, -1.5) *
                                  sqrt((k0k1err / k0dk1) * (k0k1err / k0dk1) +
                                       ((k0k1 * k0dk1err / (k0dk1 * k0dk1)) * (k0k1 * k0dk1err / (k0dk1 * k0dk1))));
        }
    ...
    for (Int_t pl = 0; pl < fPlanes; pl++)
        for (Int_t pdl = 0; pdl < fPaddles; pdl++)
            for (Int_t pm = 0; pm < 2; pm++)
            {
                ...
                syncmodpar->SetTimeOffsetError(
                    sqrt(tdiff[pl][pdl][1] * tdiff[pl][pdl][1] + tsync[pl][pdl][1] * tsync[pl][pdl][1]));
                syncmodpar->SetEffectiveSpeed(0.5 / invveff[pl][pdl][0]);
                syncmodpar->SetEffectiveSpeedError(
                    fabs(0.5 * invveff[pl][pdl][1] / (invveff[pl][pdl][0] * invveff[pl][pdl][0])));
                syncmodpar->SetEnergieGain(ecal[pl][pdl][pm]);
                syncmodpar->SetEnergieGainError(ecalerr[pl][pdl][pm]);

                LOG(INFO) << "[" << pl + 1 << "][" << pdl + 1 << "][" << pm + 1 << "]: "
                          << "t = " << syncmodpar->GetTimeOffset() << "±" << tdiff[pl][pdl][1] << "±"
                          << tsync[pl][pdl][1] << "keV/QDC = " << syncmodpar->GetEnergieGain() * 1000 << "±"
                ...
                if (!TMath::IsNaN(syncmodpar->GetTimeOffset()) && !TMath::IsNaN(syncmodpar->GetEnergieGain()))
                {
                    ...
                    {
                        sprintf(msg,
                                "[%2d][%2d][%1d]: Gain-Fit-Error of %3.1f %s",
                                pl + 1,
                                pdl + 1,
                                pm + 1,
                                dev * 100,
        ...
        for (Int_t pdl = 0; pdl < fPaddles; pdl++)
            if (calib[fPlanes][fPaddles] && !susp_b[fPlanes][fPaddles])
                for (Int_t pm = 0; pm < 2; pm++)
                {
                    ...
                    {
                        sprintf(msg,
                                "[%2d][%2d][%1d]: Deviation of %3.1f%s from average gain!",
                                pl + 1,
                                pdl + 1,
                                pm + 1,
                                dev * 100,
