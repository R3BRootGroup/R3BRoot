...
TVector3 R3BNeulandHitFiller::getDirection() { return TVector3(0, -1, 0.7 + r->Rndm() * 0.8); }
...
void R3BNeulandHitFiller::Exec(Option_t* option)
{
    ...
    { // Top
        ...
        if (_x_ < 12.5)
            ...
            direction.RotateY(+0.8 * r->Rndm() * TMath::Pi() - 0.4 * TMath::Pi() +
                              ((Int_t)_z_ < Planes / 2 ? 0 : TMath::Pi()));
    ...
    while (kTRUE)
    {
        ...
        Double_t t = time + tSync[plane][barID] + tOffset[plane][barID] / 2 + (0.5 * scale * direction).Mag() / cLight +
                     (50 - pos) * 5 / (vEff[plane][barID]);
        Int_t qdc = QDCsplitting * scale * direction.Mag() * minIonizing * eGain[plane][barID][0] *
                    TMath::Exp(-lAtt * (50 - pos));
        new ((*data)[nData++]) R3BNeulandCalData(currentID + 1, 1, t, qdc);

        t = time + tSync[plane][barID] - tOffset[plane][barID] / 2 + (0.5 * scale * direction).Mag() / cLight +
            pos * 5 / (vEff[plane][barID]);
