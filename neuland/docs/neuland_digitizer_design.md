# The Design principle of NeuLAND Digitizing module {#neuland_digi_design}
...
The Digitizing task `R3BNeulandDigitizer` handles the Input and Output of data. The task takes the `R3BNeulandPoints` from the Simulation, runs them through a `R3B::Digitizing::Engine` and fills `R3BNeulandHits`. It also requires the configuration storage `NeulandGeoPar` to convert position coordinates.
...
A `R3B::Digitizing::Engine` object handles the actual data processing, which requires another two objects as the input parameters for its instantiation: `UseChannel` and `UsePaddle`. As suggested by their names, these two objects behave like factories, which are used by `R3B::Digitizing::Engine` to generate `Channel` and `Paddle` objects.

### Interface design

To minimize the coupling between different algorithm components, users should use the base class of `R3B::Digitizing::Engine` (`R3B::Digitizing::EngineInterface`) containing all necessary essential interfaces. The following diagram depicts the correct usage of this class:
...
class MyDigitizer
{
    public:
        MyDigitizer() = default;
        SetDigiEngine(EngineInterface* engine){
            engine_ = engine;
        }
    private:
...
As a class template, `R3B::Digitizing::Engine` has the following structure:
...
```cpp
using Digitizing::Engine;
auto myEngine = std::make_unique<Digitizing::Engine<MyPaddle, MyChannel>>(UsePaddle<MyPaddle>{}, UseChannel<MyChannel>{});
```
...
There is a third optional parameter of the constructor of the class template `R3B::Digitizing::Engine` called `InitFunc`. It takes a `std::function` or a lambda function as an input, which will be invoked inside the member function `R3B::Digitizing::Engine::Init()`. The existence of such a parameter introduces another dependency injection of any kind of initialization function needed for the paddle or channel classes, such as the parameter initialization before the event loop. One example could be:
...
The `UsePaddle` and `UseChannel` class template are two input parameter types of the class template `R3B::Digitizing::Engine`. They specify the paddle and channel class (containing all implementations of the real physical paddle and channel) and the input parameters of the corresponding class constructors.
...
{
    public:
        MyChannel(R3B::Side side, int channelPar1, double channelPar2)
...
Remember that only additional input parameters should be given in the curly brackets. The parameters, such as `R3B::Side` or `paddleId`, already required in the base class `Digitizing::Channel` and `Digitizing::Paddle` should not be given as input parameters to `UsePaddle` and `UseChannel`.

### Relations inside the framework

![digiDesign](DigiDesignPattern.svg)
...
| --------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PaddleSignal`  | Output of paddle class <br> passed to the output of R3B::Digitizing::Engine<br> (reconstructed hit level data) | `double energy` (in _`MeV`_) <br> `double time` (in _`ns`_) <br> `double position` (in _`cm`_) <br> `const Channel::Signal& leftChannel` <br> `const Channel::Signal& rightChannel` |
| `ChannelSignal` | Output of channel class <br> passed to the paddle class as an input                                            | `double qdc`(in _`MeV`_) <br>`double tdc` (in _`ns`_)<br>`double qdcUnSat`(unsaturated QDC value) <br> `R3B::Side side` (left or right)                                             |
| `PaddleHit`     | Input of the paddle class from <br> the input of R3B::Digitizing::Engine                                       | `double time`(in _`ns`_) <br> `double LightDep` (in _`MeV`_) <br> `double DistToPaddleCenter` (in _`cm`_)                                                                           |
...
It's called every time an input hit is fed into the `R3B::Digitizing::Engine`, without any threshold. The input parameter is the return value of the paddle member function `ComputeTime`. In almost all cases, the derived channel class should have a private member variable to store useful input hits for later data processing (such as applying a threshold or pileup effects).
