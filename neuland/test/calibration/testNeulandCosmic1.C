...
void testNeulandCosmic1()
{
    ...
    if (par->GetNumModulePar() < cosmics->Planes * 50 * 2 * 0.9)
    { // more than 90% have to be calibrated
    ...
    else
    {
        ...
        for (Int_t i = 0; i < par->GetNumModulePar(); i++)
        {
            ...
            Int_t plane = (mpar->GetModuleId() - 1) / 50, pdl = (mpar->GetModuleId() - 1) % 50,
                  side = mpar->GetSide() - 1;
        ...
        for (Int_t pl = 0; pl < cosmics->Planes; pl++)
            for (Int_t pdl = 0; pdl < 50 && !failed; pdl++)
            {
                ...
                if (fabs(t_Off[pl][pdl][0] - t_Off[pl][pdl][1]) > 0.001 ||
                    fabs(t_Sync[pl][pdl][0] - t_Sync[pl][pdl][1]) > 0.001 ||
                    fabs(v_Eff[pl][pdl][0] - v_Eff[pl][pdl][1]) > 0.001)
                {
        ...
        if (!failed)
        {
            for (Int_t pl = 0; pl < cosmics->Planes; pl++)
            {
                ...
                {
                    Double_t eGain1 =
                        (cosmics->QDCsplitting * cosmics->eGain[pl][pdl][0] * TMath::Exp(-cosmics->lAtt * 25));
                    Double_t eGain2 =
                        (cosmics->QDCsplitting * cosmics->eGain[pl][pdl][1] * TMath::Exp(-cosmics->lAtt * 25));
            ...
            if (h_veff->GetMean() - h_veff->GetStdDev() < 1 - v_tol ||
                h_veff->GetMean() + h_veff->GetStdDev() > 1 + v_tol)
            {
                cout << "vEff failed!" << endl;
                failed = kTRUE;
            }

            if (h_egain->GetMean() - h_egain->GetStdDev() < 1 - e_tol ||
                h_egain->GetMean() + h_egain->GetStdDev() > 1 + e_tol)
            {
