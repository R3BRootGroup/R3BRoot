...
// cout<<"Rolu Mapped2Cal nHits: "<<nHits<<endl;
 ...
 // cout<<"TEST 1: "<<iDet<<", "<<iCha<<"; "<<iType<<endl;
		...
		{
			LOG(INFO) << "R3BRoluMapped2Cal::Exec : Detector number out of range: " << 
				iDet ;
			continue;
		}
		...
		if (!par)
		{
			LOG(INFO) << "R3BRoluMapped2Cal::Exec : Tcal par not found, Detector: " << 
		...
		if (times_raw_ns < 0. || times_raw_ns > fClockFreq || IS_NAN(times_raw_ns) )
		{
			...
			LOG(INFO) << 
				"R3BRoluMapped2Cal::Exec : Bad time in ns: det= " << iDet << 
				", ch= " << iCha << 
				", type= "<< iType <<
 ...
 // cout<<"M2Cal: "<<iCha<<", "<<iType<<"; "<<hit->GetTimeFine()<<", "<<hit->GetTimeCoarse()<<", "<<times_ns<<endl;
		...
		for (iCal=0;iCal<fNofCalItems;iCal++)
		{
			...
			Rolu_COINC_WINDOW_NS = Rolu_COINC_WINDOW_T_NS;   
                        ...
                        if(Tdev < Rolu_COINC_WINDOW_NS && aCalItem->GetTAMEXLNcha() > 0 ) Rolu_COINC = true;
                        if(IS_NAN(Tdev) && aCalItem->GetTAMEXLNcha() == 0) Rolu_COINC = true; // First Tamex leading time
                       }
		       if(iType == 1) {
			Rolu_COINC_WINDOW_NS = Rolu_COINC_WINDOW_T_NS;   
                        Tdev = fabs(aCalItem->GetMeanTimeTAMEXT()-times_ns);               
                        if(Tdev < Rolu_COINC_WINDOW_NS && aCalItem->GetTAMEXTNcha() > 0 ) Rolu_COINC = true;
                        if(IS_NAN(Tdev) && aCalItem->GetTAMEXTNcha() == 0) Rolu_COINC = true; // First Tamex trailing time
                       }            
			...
			if(Rolu_COINC){
				...
				switch (iCha)
				{
					...
					case 1 : {  // R  
							 if (iType == 0 && !IS_NAN(aCalItem->GetTimeL_ns(0)))  goto skip_event_pileup;
							 if (iType == 1 && !IS_NAN(aCalItem->GetTimeT_ns(0)))  goto skip_event_pileup;
						 } break;
						 
					case 3 : {  // L  
							 if (iType == 0 && !IS_NAN(aCalItem->GetTimeL_ns(2)))  goto skip_event_pileup;
							 if (iType == 1 && !IS_NAN(aCalItem->GetTimeT_ns(2)))  goto skip_event_pileup;
						 } break;
						         				   
					case 2 : { // O   
							 if (iType == 0 && !IS_NAN(aCalItem->GetTimeL_ns(1)))  goto skip_event_pileup;
							 if (iType == 1 && !IS_NAN(aCalItem->GetTimeT_ns(1)))  goto skip_event_pileup;
						 } break;
						 
					case 4 : {  // U  
							 if (iType == 0 && !IS_NAN(aCalItem->GetTimeL_ns(3)))  goto skip_event_pileup;
							 if (iType == 1 && !IS_NAN(aCalItem->GetTimeT_ns(3)))  goto skip_event_pileup;
						 } break;
	...
	//		cout<<" TEST3: "<<iCha<<", "<<iType<<", "<<calItem->fTimeT_ns[iCha-1]<<endl;	
		...
		continue;
skip_event_pileup:
				LOG(WARNING) << "R3BRoluMapped2Cal::Exec : " << fNEvent
					<< " iCha: " << iCha 
					<< " iType: " << iType
					<< " iCal: " << iCal
