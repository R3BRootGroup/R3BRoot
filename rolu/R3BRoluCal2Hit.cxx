...
{	
	...
	if(fhQ_O) delete(fhQ_O) ;
	if(fhQ_U) delete(fhQ_U) ;
...
InitStatus R3BRoluCal2Hit::ReInit()
{
    return kSUCCESS;
}
 ...
 */ 
	...
	for(Int_t ipm = 0; ipm < 4; ipm++){ 
	   if(!(IS_NAN(calItem->GetTimeL_ns(ipm)))){   // TAMEX leading
	      time_L[iPart][ipm]=calItem->GetTimeL_ns(ipm); 
	   }	
	   if(!(IS_NAN(calItem->GetTimeT_ns(ipm)))){   // TAMEX trailing
	...
	fhQ_U->Fill(tot[iPart][3]);
        ...
        new ((*fHitItems)[fNofHitItems]) R3BRoluHitData(nDet, tot[iPart][0] ,tot[iPart][1] ,tot[iPart][2] ,tot[iPart][3]);
        fNofHitItems += 1;
...
void R3BRoluCal2Hit::FinishTask()
{
	...
	if(fhQ_O) fhQ_O->Write() ;
	if(fhQ_U) fhQ_U->Write();		
}
