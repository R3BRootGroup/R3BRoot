...
R3BLosCal2Hit::~R3BLosCal2Hit()
{
    ...
    if (fhTres_MT_corr)
        delete (fhTres_MT_corr);
    if (fhTres_M_evCh)
        delete (fhTres_M_evCh);
    if (fhTres_T_evCh)
        delete (fhTres_T_evCh);
    if (fhTres_T_evCh_corr)
        delete (fhTres_T_evCh_corr);
    if (fhTres_M_evCh_corr)
        delete (fhTres_M_evCh_corr);
    if (fhTres_M_oddCh)
        delete (fhTres_M_oddCh);
    if (fhTres_T_oddCh)
        delete (fhTres_T_oddCh);
    if (fhTres_T_oddCh_corr)
        delete (fhTres_T_oddCh_corr);
    if (fhTres_M_oddCh_corr)
        delete (fhTres_M_oddCh_corr);
    if (fhQ_L)
        delete (fhQ_L);
    if (fhQ_B)
        delete (fhQ_B);
    ...
    if (fhQ_oddCh)
        delete (fhQ_oddCh);
    if (fhQ_evCh_corr)
        delete (fhQ_evCh_corr);
    if (fhQ_oddCh_corr)
        delete (fhQ_oddCh_corr);
    if (fhQtest)
        delete (fhQtest);
    if (fhQ_vs_X)
        delete (fhQ_vs_X);
    ...
    if (fhXY_ToT)
        delete (fhXY_ToT);
    if (fhXY_ToT_corr)
        delete (fhXY_ToT_corr);
    if (fhXY1_ToT_corr)
        delete (fhXY1_ToT_corr);
    if (fhXY2_ToT_corr)
        delete (fhXY2_ToT_corr);
    if (fhXY1_corr)
        delete (fhXY1_corr);
    if (fhXY2_corr)
        delete (fhXY2_corr);
    ...
    if (infile.is_open())
    {
        for (Int_t ivec = 0; ivec < 16; ivec++)
        {
            ...
            cout << setprecision(10) << ivec << ", " << walk_par[ivec][0] << ", " << walk_par[ivec][1] << ", "
                 << walk_par[ivec][2] << ", " << walk_par[ivec][3] << ", " << walk_par[ivec][4] << ", "
                 << walk_par[ivec][5] << ", " << walk_par[ivec][6] << ", " << walk_par[ivec][7] << ", "
                 << walk_par[ivec][8] << ", " << walk_par[ivec][9] << ", " << walk_par[ivec][10] << endl;
        }
    }
    else
        cerr << "Unable to open file \"" << fwalk_param_file << "\" with walk parameters!" << endl;
    ...
    if (infile1.is_open())
    {
        for (Int_t ivec = 0; ivec < 8; ivec++)
        {
            infile1 >> tot_par[ivec][0] >> tot_par[ivec][1] >> tot_par[ivec][2] >> tot_par[ivec][3] >>
                tot_par[ivec][4] >> tot_par[ivec][5] >> tot_par[ivec][6] >> tot_par[ivec][7];
        }
    }
    else
        cerr << "Unable to open file \"" << ftot_param_file << "\" with tot parameters!" << endl;
...
void R3BLosCal2Hit::Exec(Option_t* option)
{
    ...
    Double_t y1real = 0.0 / 0.0, x1real = 0.0 / 0.0, y2real = 0.0 / 0.0, x2real = 0.0 / 0.0, xreal = 0.0 / 0.0,
             yreal = 0.0 / 0.0;
    ...
    if (NULL == fhXYmean)
    {
        ...
        fhXYmean = new TH2F(strName, "", fhXbin, fhXmin, fhXmax, fhXbin, fhXmin, fhXmax);
    }
    ...
    if (NULL == fhXY_ToT)
    {
        ...
        fhXY_ToT = new TH2F(strName, "", 2000, fhXmin, fhXmax, 2000, fhXmin, fhXmax);
    }
    ...
    if (NULL == fhXY_ToT_corr)
    {
        ...
        fhXY_ToT_corr = new TH2F(strName, "", fhXbin, fhXmin, fhXmax, fhXbin, fhXmin, fhXmax);
    }
    ...
    if (NULL == fhXYproj)
    {
        ...
        fhXYproj = new TH2F(strName, "", 5000, fhXmin, fhXmax, 5000, fhXmin, fhXmax);
    }
    ...
    for (Int_t ihit = 0; ihit < nHits; ihit++)
    {
        ...
        /*
         cout<<"VFTX 1-4 "<<ihit<<", "<<calItem->fTimeV_r_ns<<", "<<calItem->fTimeV_t_ns<<", "<<calItem->fTimeV_l_ns<<",
         "<<calItem->fTimeV_b_ns<<endl; cout<<"TAMEX L 1-4 "<<ihit<<", "<<calItem->fTimeL_r_ns<<",
         "<<calItem->fTimeL_t_ns<<", "<<calItem->fTimeL_l_ns<<", "<<calItem->fTimeL_b_ns<<endl; cout<<"TAMEX T 1-4
         "<<ihit<<", "<<calItem->fTimeT_r_ns<<", "<<calItem->fTimeT_t_ns<<", "<<calItem->fTimeT_l_ns<<",
         "<<calItem->fTimeT_b_ns<<endl; cout<<"VFTX 5-8 "<<ihit<<", "<<calItem->fTimeV_rt_ns<<",
         "<<calItem->fTimeV_lt_ns<<", "<<calItem->fTimeV_lb_ns<<", "<<calItem->fTimeV_rb_ns<<endl; cout<<"TAMEX L 5-8
         "<<ihit<<", "<<calItem->fTimeL_rt_ns<<", "<<calItem->fTimeL_lt_ns<<", "<<calItem->fTimeL_lb_ns<<",
         "<<calItem->fTimeL_rb_ns<<endl; cout<<"TAMEX T 5-8 "<<ihit<<", "<<calItem->fTimeT_rt_ns<<",
         "<<calItem->fTimeT_lt_ns<<", "<<calItem->fTimeT_lb_ns<<", "<<calItem->fTimeT_rb_ns<<endl;
        */
        if (ihit == 0)
        {
            // Channels 1-4 VFTX:
            if (!(IS_NAN(calItem->fTimeV_r_ns)))
                time_r_V = calItem->fTimeV_r_ns;
            if (!(IS_NAN(calItem->fTimeV_t_ns)))
                time_t_V = calItem->fTimeV_t_ns;
            if (!(IS_NAN(calItem->fTimeV_l_ns)))
                time_l_V = calItem->fTimeV_l_ns;
            if (!(IS_NAN(calItem->fTimeV_b_ns)))
                time_b_V = calItem->fTimeV_b_ns;
            // Channels 5-8 VFTX:
            if (!(IS_NAN(calItem->fTimeV_rt_ns)))
                time_rt_V = calItem->fTimeV_rt_ns;
            if (!(IS_NAN(calItem->fTimeV_lt_ns)))
                time_lt_V = calItem->fTimeV_lt_ns;
            if (!(IS_NAN(calItem->fTimeV_lb_ns)))
                time_lb_V = calItem->fTimeV_lb_ns;
            if (!(IS_NAN(calItem->fTimeV_rb_ns)))
                time_rb_V = calItem->fTimeV_rb_ns;
            // Pixel signal (15th channel):
            if (!(IS_NAN(calItem->fTimeV_pix_ns)))
                time_pix_V = calItem->fTimeV_pix_ns;
            // Reference signal (16th channel):
            if (!(IS_NAN(calItem->fTimeV_ref_ns)))
                time_ref_V = calItem->fTimeV_ref_ns;

            //  cout<<"VFTX in "<<calItem->fTimeV_t_ns<<", "<<calItem->fTimeV_lt_ns<<", "<<calItem->fTimeV_l_ns<<",
            //  "<<calItem->fTimeV_lb_ns<<", "<<calItem->fTimeV_b_ns<<", "<<calItem->fTimeV_rb_ns<<",
            //  "<<calItem->fTimeV_r_ns<<", "<<calItem->fTimeV_rt_ns<<endl;
        }
        else if (ihit == 1)
        {
            // Channels 1-4 TAMEX:
            if (!(IS_NAN(calItem->fTimeL_r_ns)))
                time_r_L = calItem->fTimeL_r_ns;
            if (!(IS_NAN(calItem->fTimeT_r_ns)))
                time_r_T = calItem->fTimeT_r_ns;
            if (!(IS_NAN(calItem->fTimeL_t_ns)))
                time_t_L = calItem->fTimeL_t_ns;
            if (!(IS_NAN(calItem->fTimeT_t_ns)))
                time_t_T = calItem->fTimeT_t_ns;
            if (!(IS_NAN(calItem->fTimeL_l_ns)))
                time_l_L = calItem->fTimeL_l_ns;
            if (!(IS_NAN(calItem->fTimeT_l_ns)))
                time_l_T = calItem->fTimeT_l_ns;
            if (!(IS_NAN(calItem->fTimeL_b_ns)))
                time_b_L = calItem->fTimeL_b_ns;
            if (!(IS_NAN(calItem->fTimeT_b_ns)))
                time_b_T = calItem->fTimeT_b_ns;
            // Channels 5-8 TAMEX:
            if (!(IS_NAN(calItem->fTimeL_rt_ns)))
                time_rt_L = calItem->fTimeL_rt_ns;
            if (!(IS_NAN(calItem->fTimeT_rt_ns)))
                time_rt_T = calItem->fTimeT_rt_ns;
            if (!(IS_NAN(calItem->fTimeL_lt_ns)))
                time_lt_L = calItem->fTimeL_lt_ns;
            if (!(IS_NAN(calItem->fTimeT_lt_ns)))
                time_lt_T = calItem->fTimeT_lt_ns;
            if (!(IS_NAN(calItem->fTimeL_lb_ns)))
                time_lb_L = calItem->fTimeL_lb_ns;
            if (!(IS_NAN(calItem->fTimeT_lb_ns)))
                time_lb_T = calItem->fTimeT_lb_ns;
            if (!(IS_NAN(calItem->fTimeL_rb_ns)))
                time_rb_L = calItem->fTimeL_rb_ns;
            if (!(IS_NAN(calItem->fTimeT_rb_ns)))
                time_rb_T = calItem->fTimeT_rb_ns;
            // Pixel signal (15th channel):
            if (!(IS_NAN(calItem->fTimeL_pix_ns)))
                time_pix_L = calItem->fTimeL_pix_ns;
            if (!(IS_NAN(calItem->fTimeT_pix_ns)))
                time_pix_T = calItem->fTimeT_pix_ns;
            // Reference signal (16th channel):
            if (!(IS_NAN(calItem->fTimeL_ref_ns)))
                time_ref_L = calItem->fTimeL_ref_ns;
            if (!(IS_NAN(calItem->fTimeT_ref_ns)))
                time_ref_T = calItem->fTimeT_ref_ns;

            // cout<<"TAMEX L in "<<calItem->fTimeL_t_ns<<", "<<calItem->fTimeL_lt_ns<<", "<<calItem->fTimeL_l_ns<<",
            // "<<calItem->fTimeL_lb_ns<<", "<<calItem->fTimeL_b_ns<<", "<<calItem->fTimeL_rb_ns<<",
            // "<<calItem->fTimeL_r_ns<<", "<<calItem->fTimeL_rt_ns<<endl;

            // cout<<"TAMEX T in "<<calItem->fTimeT_t_ns<<", "<<calItem->fTimeT_lt_ns<<", "<<calItem->fTimeT_l_ns<<",
            // "<<calItem->fTimeT_lb_ns<<", "<<calItem->fTimeT_b_ns<<", "<<calItem->fTimeT_rb_ns<<",
            // "<<calItem->fTimeT_r_ns<<", "<<calItem->fTimeT_rt_ns<<endl;
        }
    ...
    if (time_r_V > 0. && !(IS_NAN(time_r_V)) && time_t_V > 0. && !(IS_NAN(time_t_V)) && time_l_V > 0. &&
        !(IS_NAN(time_l_V)) && time_b_V > 0. && !(IS_NAN(time_b_V)) && IS_NAN(time_rt_V) && IS_NAN(time_rb_V) &&
        IS_NAN(time_lt_V) && IS_NAN(time_lb_V))
    {
        iLOSTypeMCFD = 1; // 4PM LOS
    }
    if (time_r_V > 0. && !(IS_NAN(time_r_V)) && time_t_V > 0. && !(IS_NAN(time_t_V)) && time_l_V > 0. &&
        !(IS_NAN(time_l_V)) && time_b_V > 0. && !(IS_NAN(time_b_V)) && !(IS_NAN(time_rt_V)) && !(IS_NAN(time_rb_V)) &&
        !(IS_NAN(time_lt_V)) && !(IS_NAN(time_lb_V)) && time_rt_V > 0. && time_lt_V > 0. && time_lb_V > 0. &&
        time_rb_V > 0.)
    {
        iLOSTypeMCFD = 2; // 8PM LOS
    }
    if (time_r_L > 0. && !(IS_NAN(time_r_L)) && time_t_L > 0. && !(IS_NAN(time_t_L)) && time_l_L > 0. &&
        !(IS_NAN(time_l_L)) && time_b_L > 0. && !(IS_NAN(time_b_L)) && time_r_T > 0. && !(IS_NAN(time_r_T)) &&
        time_t_T > 0. && !(IS_NAN(time_t_T)) && time_l_T > 0. && !(IS_NAN(time_l_T)) && time_b_T > 0. &&
        !(IS_NAN(time_b_T)) && IS_NAN(time_rt_L) && IS_NAN(time_rb_L) && IS_NAN(time_lt_L) && IS_NAN(time_lb_L))
    {
    ...
    if (time_r_L > 0. && !(IS_NAN(time_r_L)) && time_t_L > 0. && !(IS_NAN(time_t_L)) && time_l_L > 0. &&
        !(IS_NAN(time_l_L)) && time_b_L > 0. && !(IS_NAN(time_b_L)) && time_r_T > 0. && !(IS_NAN(time_r_T)) &&
        time_t_T > 0. && !(IS_NAN(time_t_T)) && time_l_T > 0. && !(IS_NAN(time_l_T)) && time_b_T > 0. &&
        !(IS_NAN(time_b_T)) && !(IS_NAN(time_rt_L)) && !(IS_NAN(time_rb_L)) && !(IS_NAN(time_lt_L)) &&
        !(IS_NAN(time_lb_L)) && time_rt_L > 0. && time_lt_L > 0. && time_lb_L > 0. && time_rb_L > 0. &&
    ...
    if (iLOSTypeTAMEX == 1 && iLOSTypeMCFD == 1)
        iLOSType = 1;
    if (iLOSTypeTAMEX == 2 && iLOSTypeMCFD == 2)
        iLOSType = 2;

    //    cout<<"ILOSType: "<< iLOSTypeMCFD<<", "<<iLOSTypeTAMEX<<endl;
    //    cout<<"VFTX 1-8 "<<time_t_V<<", "<<time_lt_V<<", "<<time_l_V<<", "<<time_lb_V<<", "<<time_b_V<<",
    //    "<<time_rb_V<<", "<<time_r_V<<", "<<time_rt_V<<endl; cout<<"TAMEX L 1-8 "<<time_t_L<<", "<<time_lt_L<<",
    //    "<<time_l_L<<", "<<time_lb_L<<", "<<time_b_L<<", "<<time_rb_L<<", "<<time_r_L<<", "<<time_rt_L<<endl;
    //    cout<<"TAMEX T 1-8 "<<time_t_T<<", "<<time_lt_T<<", "<<time_l_T<<", "<<time_lb_T<<", "<<time_b_T<<",
    //    "<<time_rb_T<<", "<<time_r_T<<", "<<time_rt_T<<endl;
    ...
    if (iLOSType == 2)
    {
        ...
        if (tott > walk_par[0][0] && totlt > walk_par[1][0] && totl > walk_par[2][0] && totlb > walk_par[3][0] &&
            totb > walk_par[4][0] && totrb > walk_par[5][0] && totr > walk_par[6][0] && totrt > walk_par[7][0])
            Igood_event = 1;
        ...
        fht5Q5M->Fill(totb, (time_b_V - ref_timeM));
        fht7Q7M->Fill(totr, (time_r_V - ref_timeM));
        // MCFD 2,4,6,8
        fht2Q2M->Fill(totlt, (time_lt_V - ref_timeM));
        fht4Q4M->Fill(totlb, (time_lb_V - ref_timeM));
        fht6Q6M->Fill(totrb, (time_rb_V - ref_timeM));
        fht8Q8M->Fill(totrt, (time_rt_V - ref_timeM));
        // TAMEX 1,3,5,7
        ...
        dx2 = (time_rb_V_corr - time_lt_V_corr);
        dy2 = (time_lb_V_corr - time_rt_V_corr);

        if (tott < walk_par[0][0] || tott > walk_par[0][1])
            dy1 = 0.0 / 0.0;
        if (totl < walk_par[2][0] || totl > walk_par[2][1])
            dx1 = 0.0 / 0.0;
        if (totlb < walk_par[3][0] || totlb > walk_par[3][1])
            dy2 = 0.0 / 0.0;
        if (totlt < walk_par[1][0] || totlt > walk_par[1][1])
            dx2 = 0.0 / 0.0;
    ...
    if (iLOSType == 2)
    {
        ...
        if (iSciType == 0 || iSciType == 2 || iSciType == 4)
        {
            ...
            rw_t =
                atan((abs(xvar) + l_PM / 2.) / (l_SC / 2. - yvar)) - atan((abs(xvar) - l_PM / 2.) / (l_SC / 2. - yvar));
            rw_b =
                atan((abs(xvar) + l_PM / 2.) / (l_SC / 2. + yvar)) - atan((abs(xvar) - l_PM / 2.) / (l_SC / 2. + yvar));
            rw_l =
                atan((abs(yvar) + l_PM / 2.) / (l_SC / 2. - xvar)) - atan((abs(yvar) - l_PM / 2.) / (l_SC / 2. - xvar));
            rw_r =
                atan((abs(yvar) + l_PM / 2.) / (l_SC / 2. + xvar)) - atan((abs(yvar) - l_PM / 2.) / (l_SC / 2. + xvar));
        ...
        if (iSciType == 1 || iSciType == 3 || iSciType == 5 || iSciType == 6)
        {
            ...
            LosTresT =
                (time_r_L + time_l_L + time_t_L + time_b_L) / 4. - (time_rb_L + time_lt_L + time_rt_L + time_lb_L) / 4.;
            if (time_r_L_corr > 0. && !(IS_NAN(time_r_L_corr)) && time_t_L_corr > 0. && !(IS_NAN(time_t_L_corr)) &&
                time_l_L_corr > 0. && !(IS_NAN(time_l_L_corr)) && time_b_L_corr > 0. && !(IS_NAN(time_b_L_corr)) &&
                !(IS_NAN(time_rt_L_corr)) && !(IS_NAN(time_rb_L_corr)) && !(IS_NAN(time_lt_L_corr)) &&
                !(IS_NAN(time_lb_L_corr)) && time_rt_L_corr > 0. && time_lt_L_corr > 0. && time_lb_L_corr > 0. &&
                time_rb_L_corr > 0.)
            {
            ...
            if (!(IS_NAN(tott)) && !(IS_NAN(totlt)) && !(IS_NAN(totl)) && !(IS_NAN(totlb)) && !(IS_NAN(totb)) &&
                !(IS_NAN(totrb)) && !(IS_NAN(totr)) && !(IS_NAN(totrt)) && tott > 0. && totlt > 0. && totl > 0. &&
                totlb > 0. && totb > 0. && totrb > 0. && totr > 0. && totrt > 0.)
            {
            ...
            if (time_r_V_corr > 0. && !(IS_NAN(time_r_V_corr)) && time_t_V_corr > 0. && !(IS_NAN(time_t_V_corr)) &&
                time_l_V_corr > 0. && !(IS_NAN(time_l_V_corr)) && time_b_V_corr > 0. && !(IS_NAN(time_b_V_corr)) &&
                !(IS_NAN(time_rt_V_corr)) && !(IS_NAN(time_rb_V_corr)) && !(IS_NAN(time_lt_V_corr)) &&
                !(IS_NAN(time_lb_V_corr)) && time_rt_V_corr > 0. && time_lt_V_corr > 0. && time_lb_V_corr > 0. &&
                time_rb_V_corr > 0.)
            {
            ...
            if (IS_NAN(tott))
                cout << "IS_NAN(tott)" << endl;
            if (IS_NAN(totlt))
                cout << "IS_NAN(totlt)" << endl;
            if (IS_NAN(totl))
                cout << "IS_NAN(totl)" << endl;
            if (IS_NAN(totlb))
                cout << "IS_NAN(totlb)" << endl;
            if (IS_NAN(totb))
                cout << "IS_NAN(totb)" << endl;
            if (IS_NAN(totrb))
                cout << "IS_NAN(totrb)" << endl;
            if (IS_NAN(totr))
                cout << "IS_NAN(totr)" << endl;
            if (IS_NAN(totrt))
                cout << "IS_NAN(totrt)" << endl;
            ...
            */
            ...
            {
                rw_t = atan((abs(xvar) + l_PM / 2.) / (l_SC / 2. - yvar)) -
                       atan((abs(xvar) - l_PM / 2.) / (l_SC / 2. - yvar));
                rw_b = atan((abs(xvar) + l_PM / 2.) / (l_SC / 2. + yvar)) -
                       atan((abs(xvar) - l_PM / 2.) / (l_SC / 2. + yvar));
                rw_l = atan((abs(yvar) + l_PM / 2.) / (l_SC / 2. - xvar)) -
                       atan((abs(yvar) - l_PM / 2.) / (l_SC / 2. - xvar));
                rw_r = atan((abs(yvar) + l_PM / 2.) / (l_SC / 2. + xvar)) -
                       atan((abs(yvar) - l_PM / 2.) / (l_SC / 2. + xvar));
                rw_rt = atan((abs(xvar) + l_PM / 2.) / (l_SC / 2. - yvar)) -
                        atan((abs(xvar) - l_PM / 2.) / (l_SC / 2. - yvar));
                rw_lb = atan((abs(xvar) + l_PM / 2.) / (l_SC / 2. + yvar)) -
                        atan((abs(xvar) - l_PM / 2.) / (l_SC / 2. + yvar));
                rw_lt = atan((abs(yvar) + l_PM / 2.) / (l_SC / 2. - xvar)) -
                        atan((abs(yvar) - l_PM / 2.) / (l_SC / 2. - xvar));
                rw_rb = atan((abs(yvar) + l_PM / 2.) / (l_SC / 2. + xvar)) -
                        atan((abs(yvar) - l_PM / 2.) / (l_SC / 2. + xvar));
            }
            else
            {
                rw_t = atan((abs(xvar) + l_PM1 / 2.) / (l_SC / 2. - yvar)) -
                       atan((abs(xvar) - l_PM1 / 2.) / (l_SC / 2. - yvar));
                rw_b = atan((abs(xvar) + l_PM1 / 2.) / (l_SC / 2. + yvar)) -
                       atan((abs(xvar) - l_PM1 / 2.) / (l_SC / 2. + yvar));
                rw_l = atan((abs(yvar) + l_PM1 / 2.) / (l_SC / 2. - xvar)) -
                       atan((abs(yvar) - l_PM1 / 2.) / (l_SC / 2. - xvar));
                rw_r = atan((abs(yvar) + l_PM1 / 2.) / (l_SC / 2. + xvar)) -
                       atan((abs(yvar) - l_PM1 / 2.) / (l_SC / 2. + xvar));

                rw_rt = atan((abs(xvar1) + l_PM2 / 2.) / (l_SC / 2. - yvar1)) -
                        atan((abs(xvar1) - l_PM2 / 2.) / (l_SC / 2. - yvar1));
                rw_lb = atan((abs(xvar1) + l_PM2 / 2.) / (l_SC / 2. + yvar1)) -
                        atan((abs(xvar1) - l_PM2 / 2.) / (l_SC / 2. + yvar1));
                rw_lt = atan((abs(yvar1) + l_PM2 / 2.) / (l_SC / 2. - xvar1)) -
                        atan((abs(yvar1) - l_PM2 / 2.) / (l_SC / 2. - xvar1));
                rw_rb = atan((abs(yvar1) + l_PM2 / 2.) / (l_SC / 2. + xvar1)) -
                        atan((abs(yvar1) - l_PM2 / 2.) / (l_SC / 2. + xvar1));
            }
            ...
            tott_scal = tott * ((2. * sin(rw_t / 2.)) / rr_t) /
                        (0.5 / 6.); //* (0.38 / rw_t) / (6. / rr_t); // /(rr_t/(2.*sin(rw_t/2.))/10.);
            totl_scal = totl * ((2. * sin(rw_l / 2.)) / rr_l) /
                        (0.5 / 6.); //*(0.38 / rw_l) / (6. / rr_l); // //(rr_l/(2.*sin(rw_l/2.))/5.);
            totb_scal = totb * ((2. * sin(rw_b / 2.)) / rr_b) /
                        (0.5 / 6.); //*(0.38 / rw_b) / (6. / rr_b); // //(rr_b/(2.*sin(rw_b/2.))/10.);
            totr_scal = totr * ((2. * sin(rw_r / 2.)) / rr_r) /
                        (0.5 / 6.); //*(0.38 / rw_r) / (6. / rr_r); // //(rr_r/(2.*sin(rw_r/2.))/5.);
            totlt_scal = totlt * ((2. * sin(rw_lt / 2.)) / rr_lt) /
                         (0.5 / 6.); //*(0.38 / rw_lt) / (6. / rr_lt); // //(rr_lt/(2.*sin(rw_lt/2.))/20.);
            totlb_scal = totlb * ((2. * sin(rw_lb / 2.)) / rr_lb) /
                         (0.5 / 6.); //*(0.38 / rw_lb) / (6. / rr_lb); // //(rr_lb/(2.*sin(rw_lb/2.))/20.);
            totrb_scal = totrb * ((2. * sin(rw_rb / 2.)) / rr_rb) /
                         (0.5 / 6.); //*(0.38 / rw_rb) / (6. / rr_rb); // //(rr_rb/(2.*sin(rw_rb/2.))/20.);
            totrt_scal = totrt * ((2. * sin(rw_rt / 2.)) / rr_rt) /
                         (0.5 / 6.); //*(0.38 / rw_rt) / (6. / rr_rt); // //(rr_rt/(2.*sin(rw_rt/2.))/20.);
           ...
           */
            ...
            {
                tot_corr = (totl_corr + totlt_corr + totlb_corr + tott_corr + totr_corr + totrb_corr + totb_corr +
                            totrt_corr) /
                           8.;
            }
        ...
        if (Igood_event == 1)
        {
            ...
            fhTres_M_oddCh->Fill((t_hor_M - t_ver_M));
            fhTresMvsIcount->Fill(Icount, LosTresM);
            // MCFD walk corrected times:
            fhTres_M_corr->Fill(LosTresM_corr);
            fhTres_M_oddCh_corr->Fill((time_r_V_corr + time_l_V_corr) / 2. - (time_t_V_corr + time_b_V_corr) / 2.);
            fhTres_M_evCh_corr->Fill((time_rb_V_corr + time_lt_V_corr) / 2. - (time_rt_V_corr + time_lb_V_corr) / 2.);
            fhTreswcMvsIcount->Fill(Icount, LosTresM_corr);
            // TAMEX times:
            fhTres_T->Fill(LosTresT);
            fhTres_T_evCh->Fill((t_45_T - t_135_T));
            fhTres_T_oddCh->Fill((t_hor_T - t_ver_T));
            fhTresMvsIcount->Fill(Icount, LosTresM);
            // TAMEX walk corrected times:
            fhTres_T_corr->Fill(LosTresT_corr);
            fhTres_T_oddCh_corr->Fill((time_r_L_corr + time_l_L_corr) / 2. - (time_t_L_corr + time_b_L_corr) / 2.);
            fhTres_T_evCh_corr->Fill((time_rb_L_corr + time_lt_L_corr) / 2. - (time_rt_L_corr + time_lb_L_corr) / 2.);
            ...
            fhQ2vsIcount->Fill(Icount, totlt);
            fhQ4vsIcount->Fill(Icount, totlb);
            fhQ6vsIcount->Fill(Icount, totrb);
            fhQ8vsIcount->Fill(Icount, totrt);

            fhQ_oddCh->Fill((tott + totl + totb + totr) / 4.);
            fhQ_evCh->Fill((totlt + totlb + totrb + totrt) / 4.);
            fhQ_oddCh_corr->Fill((tott_corr + totl_corr + totb_corr + totr_corr) / 4.);
            fhQ_evCh_corr->Fill((totlt_corr + totlb_corr + totrb_corr + totrt_corr) / 4.);

            fhQ1_vs_Q5->Fill(tott, totb);
            fhQ3_vs_Q7->Fill(totl, totr);
            fhQ2_vs_Q6->Fill(totlt, totrb);
            fhQ4_vs_Q8->Fill(totlb, totrt);
            fhQ1_vs_Q5_corr->Fill(tott_corr, totb_corr);
            fhQ3_vs_Q7_corr->Fill(totr_corr, totl_corr);
            fhQ2_vs_Q6_corr->Fill(totlt_corr, totrb_corr);
...
void R3BLosCal2Hit::FinishTask()
{
    ...
    if (fhTres_M_corr)
        fhTres_M_corr->Write();
    if (fhTres_MT_corr)
        fhTres_MT_corr->Write();
    if (fhTres_M_oddCh)
        fhTres_M_oddCh->Write();
    if (fhTres_T_oddCh)
        fhTres_T_oddCh->Write();
    if (fhTres_T_oddCh_corr)
        fhTres_T_oddCh_corr->Write();
    if (fhTres_M_oddCh_corr)
        fhTres_M_oddCh_corr->Write();
    if (fhTres_M_evCh)
        fhTres_M_evCh->Write();
    if (fhTres_T_evCh)
        fhTres_T_evCh->Write();
    if (fhTres_T_evCh_corr)
        fhTres_T_evCh_corr->Write();
    if (fhTres_M_evCh_corr)
        fhTres_M_evCh_corr->Write();
    if (fhQ_L)
        fhQ_L->Write();
    if (fhQ_B)
        fhQ_B->Write();
    ...
    if (fhXY_ToT_corr)
        fhXY_ToT_corr->Write();
    if (fhXY1_ToT_corr)
        fhXY1_ToT_corr->Write();
    if (fhXY2_ToT_corr)
        fhXY2_ToT_corr->Write();
    if (fhXY1_corr)
        fhXY1_corr->Write();
    if (fhXY2_corr)
        fhXY2_corr->Write();
