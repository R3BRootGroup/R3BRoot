...
InitStatus R3BTrackingS515::Init()
{
    ...
    if(GladCurrent<0 || GladReferenceCurrent<0 ||
            FrsCutMeanZ<0 || FrsCutMeanAoZ<0 || FrsCutRadiusZ<0  || FrsCutRadiusAoZ<0 ||
            FrsBrhoMin<0 || FrsBrhoMax<0 || MusicZMin<0 || MusicZMax<0 ||
...
void R3BTrackingS515::Exec(Option_t* option)
{
    if (fNEvents / 1000. == (int)fNEvents / 1000)
        std::cout << "\rEvents: " << fNEvents 
            << " / " <<maxevent << " (" << (int)(fNEvents * 100./maxevent) << " %) " << std::flush;
    fNEvents += 1;
    FairRootManager* mgr = FairRootManager::Instance();
    if (NULL == mgr)
        LOG(fatal) << "FairRootManager not found";
    ...
    if( mwpc_DataItems->GetEntriesFast()!=1 || frs_DataItems->GetEntriesFast()<1 ||
            ...
            fDataItems.at(DET_FI10)->GetEntriesFast() == 0 ||
            fDataItems.at(DET_FI11)->GetEntriesFast() == 0 ||
            fDataItems.at(DET_FI12)->GetEntriesFast() == 0 ||
            tofd_DataItems->GetEntriesFast()<1
      ) return;
