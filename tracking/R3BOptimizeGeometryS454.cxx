...
// For the moment one has to swap manually between the setups 
    ...
    if (tof->fArrayHits->GetEntriesFast() > 0)
    {
		...
		cout << "Copy Hits: " 
		<< fi3b->events[fNEvents].size() << "  " << fi12->events[fNEvents].size() << "  " 
		...
		if(tof->hits.at(i)->GetX() > 0 && fi10->hits.size() > 0 && fi12->hits.size() > 0){
			...
			do // fi10
			{
				do // fi12
				{
                 ...
                 //                               cout << "Fi3b  # " << " x: " << fi3b->hits.at(0)->GetX() << endl;
			...
			do // fi13
			{
				...
				do // fi11
				{
					...
					do // fi3a
					{
						...
						cout << "Hit Tofd # " << i << " x: " << tof->hits.at(i)->GetX() 
							<< " y: " << tof->hits.at(i)->GetY() << endl;
						if(ifi13 > -1) cout << "Fi13 # " <<  ifi13 << " x: "<< fi13->hits.at(ifi13)->GetX() << endl;
						if(ifi11 > -1) cout << "Fi11 # " <<  ifi11 << " x: "<< fi11->hits.at(ifi11)->GetX() << endl;
						cout << "Hit target # " << " x: " << target->hits.at(0)->GetX() << endl;
...
void R3BOptimizeGeometryS454::Finish()
{
	...
	if(fLeft)
	{
		...
		//Double_t variable[9] = {-94.98,  667.6,  -67.87,  583.4,  -17.64, -17.35,  1., 5.55, 80.53}; // best with mag=1
		//Double_t variable[9] = {-94.85,  667.6,  -67.87,  583.4,  -17.67, -17.23,  1.019, 5.55, 80.53}; //best with mag!=0
		//Double_t variable[9] = {-94.66,  667.7,  -67.73,  583.4,  -17.95, -17.08,  1.0, 5.35, 80.59}; //chi=739
		//Double_t variable[9] = {-94.46,  667.8,  -67.73,  583.4,  -18.15, -17.10,  1.0, 5.31, 80.49}; //chi=613
		//Double_t variable[9] = {-94.46,  667.8,  -67.73,  583.4,  -18.19, -17.08,  1.0, 5.31, 80.49}; //chi=
		...
		//Double_t variable[9] = {-94.58,  667.8 , -67.73,  583.0,  -18.78, -17.27,  0.9997,  5.52 , 80.59}; //chi=919 best with 4He and 12C
		//Double_t variable[9] = {-94.72,  667.8 , -67.62,  583.0,  -19.00, -16.95,  0.9997,  5.6 , 80.65}; //chi=532 only 4He
		//Double_t variable[9] = {-94.62,  667.8 , -67.62,  583.0,  -18.97, -16.95,  0.9997,  5.6 , 80.65}; //chi=532 only 4He
		//Double_t variable[9] = {-94.62,  668.2 , -67.38,  582.9,  -19.37, -16.88,  1.019,  5.59 , 80.47}; //chi=3313 only 4He
		//Double_t variable[9] = {-94.46,  667.8 , -67.73,  583.4,  -18.15, -17.10,  1.0,  5.31 , 80.49}; //chi=3313 only 12C
		//Double_t variable[9] = {-94.56,  667.8 , -67.73,  583.4,  -18.25, -17.06,  1.0,  5.31 , 80.49}; //chi=3693 only 12C
		//Double_t variable[9] = {-94.63,  667.8 , -67.73,  583.4,  -18.68, -17.28,  1.009,  5.45 , 80.50}; //chi=3754 only 12C
		
		
		//Double_t variable[9] = {-94.67,  668.1 , -67.77,  583.4,  -17.7, -17.7,  1.000,  5.83 , 80.5}; //chi=13.51 only 12C
		//Double_t variable[9] = {-94.64,  667.8 , -67.73,  583.2,  -18.68, -17.48,  1.009,  5.45 , 80.48}; //chi=13.51 only 12C
		//Double_t variable[9] = {-94.82,  668.1 , -67.77,  583.4,  -17.83, -17.69,  1.019,  5.7 , 80.51}; //chi=15.94 only 12C
		//Double_t variable[9] = {-94.93,  668.1 , -67.77,  583.4,  -17.64, -17.51,  1.019,  5.7 , 80.51}; //chi=14.55 only 12C
		//Double_t variable[9] = {-94.93,  668.1 , -67.77,  583.4,  -17.64, -17.51,  0.990,  5.7 , 80.51}; //chi=14.52 only 12C
		//Double_t variable[9] = {-94.77,  668.1 , -67.77,  583.4,  -17.83, -17.71,  0.980,  5.5 , 80.50}; //chi=16.01 only 12C
		//Double_t variable[9] = {-94.69,  668.1 , -67.74,  583.6,  -17.83, -17.80,  0.970,  5.3 , 80.64}; //chi=15.8 only 12C
		//Double_t variable[9] = {-94.49,  668.3 , -67.69,  583.5,  -17.88, -17.87,  0.960,  5.24 , 80.7}; //chi=15.9 only 12C
		//Double_t variable[9] = {-94.3,  668.3 , -67.69,  583.5,  -17.88, -17.68,  0.950,  5.07 , 80.69}; //chi=15.0 only 12C
		//Double_t variable[9] = {-94.14,  668.3 , -67.69,  583.5,  -17.72, -17.73,  0.9570,  4.91 , 80.77}; //chi=15.56 only 12C
		//Double_t variable[9] = {-94.00,  668.3 , -67.57,  583.5,  -17.52, -17.59,  0.9570,  4.91 , 80.77}; //chi=15.15 only 12C
		
		
		
		
		
		//Double_t variable[9] = {-94.67,  668.1 , -67.77,  583.4,  -17.83, -17.71,  1.000,  5.7 , 80.50}; //chi=16.01 only 12C
		//Double_t variable[9] = {-94.82,  668.1 , -67.77,  583.4,  -17.83, -17.69,  1.010,  5.56 , 80.47}; //chi=14.64 only 12C
		//Double_t variable[9] = {-94.88,  668.1 , -67.77,  583.4,  -17.93, -17.63,  1.010,  5.56 , 80.47}; //chi=14.88 only 12C
		//Double_t variable[9] = {-94.88,  668.1 , -67.77,  583.4,  -18.13, -17.69,  1.010,  5.56 , 80.47}; //chi=14.85 only 12C
		//Double_t variable[9] = {-94.98,  668.1 , -67.93,  583.4,  -18.09, -17.83,  1.010,  5.56 , 80.47}; //chi=14.69 only 12C
		//Double_t variable[9] = {-94.98,  668.1 , -67.93,  583.4,  -18.08, -17.83,  1.010,  5.56 , 80.47}; //chi=14.69 only 12C
		
		
		//Double_t variable[9] = {-94.64,  667.8 , -67.73,  583.2,  -18.68, -17.48,  1.009,  5.45 , 80.48}; //chi=13.51 only 12C
		//Double_t variable[9] = {-94.76,  667.8 , -67.73,  583.2,  -18.31, -17.68,  1.009,  5.45 , 80.65}; //chi=14.47 only 12C
		//Double_t variable[9] = {-94.76,  667.8 , -67.73,  583.2,  -18.12, -17.58,  1.009,  5.45 , 80.65}; //chi=14.35 only 12C


//----------------------------------------------------
		// neuer Datensatz
		// Optimierung mit 12C ohne Bedingung auf Magnetfeld
		//Double_t variable[9] = {-94.75,  667.8 , -67.73,  583.2,  -18.23, -17.46,  1.009,  5.45 , 80.65}; //chi=10.24 only 12C
		//Double_t variable[9] = {-94.75,  667.8 , -67.73,  583.2,  -18.23, -17.46,  1.009,  5.45 , 80.65}; //chi=10.24 only 12C
		
		
		// 12C und mit Zwang auf mag = 0.961
		//Double_t variable[9] = {-94.55,  668.0 , -67.55,  583.0,  -18.24, -17.27,  0.961,  5.25 , 80.68}; //chi=25.26 only 12C
		//Double_t variable[9] = {-94.38,  668.0 , -67.42,  583.0,  -18.40, -17.24,  0.961,  5.05 , 80.73}; //chi=19.45 only 12C
		//Double_t variable[9] = {-94.18,  667.8 , -67.42,  582.8,  -18.58, -17.43,  0.961,  4.850 , 80.63}; //chi=13.70 only 12C
		//Double_t variable[9] = {-93.98,  668.0 , -67.40,  582.9,  -18.45, -17.28,  0.961,  4.850 , 80.50}; //chi=10.57 only 12C
		//Double_t variable[9] = {-93.96,  668.1 , -67.40,  582.9,  -18.47, -17.20,  0.961,  4.850 , 80.50}; //chi=10.52 only 12C
		...
		//Double_t variable[9] = {-94.86,  668.1 , -67.73,  583.4,  -17.74, -17.69,  0.9690,  5.620 , 80.40}; //chi=3.28
		//Double_t variable[9] = {-94.80,  668.1 , -67.73,  583.4,  -17.74, -17.79,  0.92760,  5.620 , 80.40}; //chi=3.10
		//Double_t variable[9] = {-94.78,  668.1 , -67.73,  583.4,  -17.67, -17.69,  0.9076,  5.624 , 80.42}; //chi=2.92
		//Double_t variable[9] = {-94.80,  668.1 , -67.73,  583.4,  -17.62, -17.75,  0.9076,  5.624 , 80.42}; //chi=2.92
		//Double_t variable[9] = {-94.80,  668.1 , -67.73,  583.4,  -17.57, -17.75,  0.9076,  5.624 , 80.42}; //chi=2.92
		
		// 4He und ohne Bedingung auf Magnetfeld aber mit Impuls im chi2 mitzuzählen (100*)
		//Double_t variable[9] = {-94.90,  667.9 , -67.65,  583.5,  -17.61, -17.75,  0.9476,  5.615 , 80.43}; //chi=31.54
		//Double_t variable[9] = {-94.81,  667.9 , -67.65,  583.5,  -17.43, -17.93,  0.9656,  5.620 , 80.43}; //chi=28.06
		//Double_t variable[9] = {-94.80,  667.9 , -67.65,  583.5,  -17.31, -17.81,  0.9656,  5.620 , 80.43}; //chi=28.00
		//Double_t variable[9] = {-94.80,  667.9 , -67.65,  583.5,  -17.31, -17.81,  0.9656,  5.620 , 80.43}; //chi=28.00

		// 4He und ohne Bedingung auf Magnetfeld aber mit Impuls im chi2 mitzuzählen (1000*)
		//Double_t variable[9] = {-94.61,  668.1 , -67.67,  583.7,  -17.18, -17.75,  0.9846,  5.581 , 80.42}; //chi=40.73
		//Double_t variable[9] = {-94.44,  668.1 , -67.67,  583.7,  -17.31, -17.94,  0.9846,  5.578 , 80.43}; //chi=39.81
		//Double_t variable[9] = {-94.46,  668.2 , -67.67,  583.7,  -17.25, -17.99,  0.9846,  5.578 , 80.43}; //chi=39.83
		//Double_t variable[9] = {-94.47,  668.2 , -67.67,  583.7,  -17.24, -17.93,  0.9846,  5.578 , 80.43}; //chi=39.84
		...
		//Double_t variable[9] = {-94.58,  668.2 , -67.65,  583.9,  -16.97, -18.23,  1.001,  5.578 , 80.43}; //chi=484.6
	...
	else
	{
		...
		//Double_t variable[9] = { -129.8,  589.6,  -157.8,  676.0,  -198.4,  -196.9,  1.0, -5.58, 80.68}; // best with mag=1
		//Double_t variable[9] = { -129.8,  589.7,  -157.9,  676.0,  -198.7,  -196.6,  0.9890, -5.46, 80.65}; // best with mag!=1
		//Double_t variable[9] = { -129.7,  589.7,  -157.9,  676.0,  -198.7,  -196.5,  1.0, -5.31, 80.67}; // chi=248.7
		//Double_t variable[9] = { -129.7,  589.7,  -157.9,  676.0,  -198.8,  -196.4,  1.0, -5.31, 80.67}; // chi=247
		//Double_t variable[9] = { -129.8,  589.7,  -158.0,  676.0,  -198.9,  -196.3,  1.0, -5.23, 80.68}; // chi=1040
		//Double_t variable[9] = { -129.8,  589.7,  -158.0,  676.0,  -198.9,  -196.2,  1.0, -5.35, 80.71}; // chi=
		//Double_t variable[9] = { -129.7,  589.7,  -158.1,  676.0,  -198.9,  -196.2,  1.000,  -5.350,  80.71}; // chi=7456
		//Double_t variable[9] = { -129.7,  589.7,  -158.1,  676.0,  -198.9,  -196.2,  1.000,  -5.350,  80.71}; // chi=7456
		//Double_t variable[9] = { -129.7,  589.7,  -158.1,  676.0,  -198.9,  -196.2,  1.000,  -5.350,  80.71}; // chi=7643
		//Double_t variable[9] = { -129.4,  589.5,  -158.1,  676.4,  -198.6,  -196.3,  0.9997,  -5.270,  80.68}; // best chi=5514
		//Double_t variable[9] = { -129.6,  589.5,  -158.1,  676.4,  -198.6,  -196.4,  0.98,  -5.270,  80.68}; // chi=5514
		//Double_t variable[9] = { -129.6,  589.5,  -158.1,  676.4,  -198.8,  -196.4,  0.9997,  -5.270,  80.67}; // chi=5700
		//Double_t variable[9] = { -129.6,  589.5,  -158.1,  676.4,  -198.7,  -196.4,  0.9997,  -5.270,  80.67}; // chi=5439  best with 4He and 12C
		//Double_t variable[9] = { -129.5,  589.5,  -158.0,  676.4,  -198.2,  -196.4,  0.9997,  -5.210,  80.64}; // chi=2291  only 4He
		//Double_t variable[9] = { -129.6,  589.5,  -158.0,  676.5,  -198.4,  -196.5,  1.013,  -5.11,  80.61}; // chi=37677  only 12C
		//Double_t variable[9] = { -129.7,  589.6,  -158.1,  676.5,  -198.4,  -196.6,  1.013,  -5.28,  80.58}; // chi=7418  only 4He
		//Double_t variable[9] = { -129.7,  589.6,  -158.1,  676.0,  -198.8,  -196.4,  1.0,  -5.31,  80.67}; // chi=7418  only 4He
		//Double_t variable[9] = { -129.6,  589.5,  -158.1,  676.0,  -198.9,  -196.5,  1.011,  -5.2,  80.54}; // chi=7955  only 4He
		
		//Double_t variable[9] = { -127.9,  587.7,  -156.5,  674.7,  -197.7,  -197.7,  1.000,  -5.83,  80.50}; // chi=13.16  only 12C
		//Double_t variable[9] = { -129.6,  589.5,  -158.1,  676.2,  -199.2,  -196.5,  0.981,  -5.197,  80.53}; // chi=13.16  only 12C
		//Double_t variable[9] = { -128.1,  587.5,  -156.3,  674.7,  -197.9,  -197.7,  0.980,  -5.51,  80.59}; // chi=20.15  only 12C
		//Double_t variable[9] = { -128.1,  587.5,  -156.3,  674.7,  -197.9,  -197.8,  0.9770,  -5.51,  80.58}; // chi=18.65  only 12C
		//Double_t variable[9] = { -128.1,  587.5,  -156.3,  674.7,  -197.9,  -198.0,  0.9570,  -5.45,  80.74}; // chi=12.66  only 12C
		//Double_t variable[9] = { -128.1,  587.5,  -156.3,  674.7,  -198.1,  -198.1,  0.9570,  -5.45,  80.89}; // chi=12.08  only 12C
		
		
//----------------------------------------------------
		// neuer Datensatz
		// Optimierung mit 12C ohne Bedingung auf Magnetfeld
		//Double_t variable[9] = { -128.1,  587.6,  -156.3,  674.7,  -198.1,  -197.9,  0.961,  -5.46,  80.91}; // chi=10.77  only 12C
		//Double_t variable[9] = { -128.1,  587.6,  -156.3,  674.7,  -198.1,  -197.9,  0.961,  -5.46,  80.91}; // chi=10.77  only 12C

		// 12C und mit Zwang auf mag = 1.009
		//Double_t variable[9] = { -128.3,  587.8,  -156.5,  674.6,  -198.0,  -197.9,  1.009,  -5.27,  81.06}; // chi=20.47  only 12C
		//Double_t variable[9] = { -128.2,  587.8,  -156.6,  674.7,  -198.2,  -197.7,  1.009,  -5.09,  81.06}; // chi=15.72  only 12C
		//Double_t variable[9] = { -128.1,  587.8,  -156.7,  674.7,  -198.4,  -197.7,  1.009,  -4.89,  81.02}; // chi=12.31  only 12C
		//Double_t variable[9] = { -128.1,  587.8,  -156.7,  674.7,  -198.4,  -197.7,  1.009,  -4.89,  80.95}; // chi=11.93  only 12C
		//Double_t variable[9] = { -128.1,  587.8,  -156.7,  674.7,  -198.4,  -197.5,  1.009,  -4.89,  80.96}; // chi=11.85  only 12C

		// 4He oder 12C und ohne Bedingung auf Magnetfeld
		//Double_t variable[9] = { -128.1,  587.6,  -156.3,  674.7,  -198.1,  -197.7,  1.003,  -5.46,  80.97}; // chi=7.91  
		//Double_t variable[9] = { -128.1,  587.6,  -156.3,  674.7,  -198.2,  -197.7,  1.003,  -5.46,  80.95}; // chi=7.47  
		//Double_t variable[9] = { -128.1,  587.6,  -156.3,  674.7,  -198.3,  -197.3,  1.006,  -5.44,  80.88}; // chi=7.56  
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.5,  -197.4,  1.006,  -5.44,  80.88}; // chi=7.50  
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.7,  -197.5,  1.006,  -5.44,  80.88}; // chi=7.50  

		// 4He oder 12C und Mit Zwang auf 1.006
		//Double_t variable[9] = { -128.1,  587.6,  -156.3,  674.7,  -198.3,  -197.5,  1.006,  -5.44,  80.95}; // chi=7.73  
		//Double_t variable[9] = { -128.1,  587.6,  -156.3,  674.7,  -198.5,  -197.4,  1.006,  -5.44,  80.95}; // chi=7.55  

		// 4He und ohne Bedingung auf Magnetfeld
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.7,  -197.5,  1.006,  -5.44,  80.88}; // chi=7.35  
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.7,  -197.5,  1.006,  -5.44,  80.88}; // chi=7.35  

		// 4He und ohne Bedingung auf Magnetfeld und ohne Impuls im chi2 mitzubrücksichtigen
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.7,  -197.5,  1.006,  -5.44,  80.88}; // chi=5.23  
		//Double_t variable[9] = { -128.1,  587.8,  -156.3,  674.6,  -198.6,  -197.7,  0.9456,  -5.29,  80.90}; // chi=5.64  
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.7,  -197.6,  0.9276,  -5.33,  80.78}; // chi=3.98  
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.6,  -197.4,  0.9276,  -5.33,  80.78}; // chi=3.87  
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.6,  -197.4,  0.9276,  -5.33,  80.78}; // chi=3.83  
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.6,  -197.3,  0.9276,  -5.330,  80.78}; // chi=3.83  
		//Double_t variable[9] = { -128.1,  587.7,  -156.3,  674.6,  -198.6,  -197.3,  0.8956,  -5.330,  80.78}; // chi=3.77  
		
		// 4He und ohne Bedingung auf Magnetfeld aber mit Impuls im chi2 mitzuzählen (100*)
		//Double_t variable[9] = { -128.1,  587.7,  -156.4,  674.6,  -198.7,  -197.2,  0.9356,  -5.350,  80.79}; // chi=28.28  
		//Double_t variable[9] = { -128.0,  587.7,  -156.4,  674.6,  -198.8,  -197.0,  0.9516,  -5.356,  80.80}; // chi=27.42  
		//Double_t variable[9] = { -128.1,  587.7,  -156.4,  674.6,  -198.7,  -196.8,  0.9516,  -5.356,  80.80}; // chi=27.22  
		//Double_t variable[9] = { -128.1,  587.7,  -156.4,  674.6,  -198.7,  -196.7,  0.9516,  -5.356,  80.80}; // chi=27.21  

		// 4He und ohne Bedingung auf Magnetfeld aber mit Impuls im chi2 mitzuzählen (1000*)
		//Double_t variable[9] = { -128.1,  587.7,  -156.4,  674.6,  -199.0,  -196.9,  0.9846,  -5.365,  80.80}; // chi=50.07  
		//Double_t variable[9] = { -128.1,  587.7,  -156.4,  674.6,  -199.2,  -196.9,  0.9846,  -5.365,  80.80}; // chi=50.35  
		//Double_t variable[9] = { -128.1,  587.7,  -156.4,  674.6,  -199.4,  -197.0,  0.9846,  -5.365,  80.80}; // chi=50.18  
		//Double_t variable[9] = { -128.1,  587.7,  -156.4,  674.6,  -199.4,  -197.0,  0.9846,  -5.365,  80.80}; // chi=50.34  

		// 4He und ohne Bedingung auf Magnetfeld aber mit Impuls im chi2 mitzuzählen (10000*)
		//Double_t variable[9] = { -128.2,  587.7,  -156.4,  674.6,  -199.4,  -197.0,  0.9846,  -5.354,  80.80}; // chi=470  
		//Double_t variable[9] = { -128.1,  587.7,  -156.4,  674.6,  -199.5,  -197.2,  0.9846,  -5.354,  80.80}; // chi=469  
		//Double_t variable[9] = { -128.2,  587.7,  -156.4,  674.6,  -199.6,  -197.3,  0.9846,  -5.354,  80.80}; // chi=469.2  

		// 12C und ohne Bedingung auf Magnetfeld aber mit Impuls im chi2 mitzuzählen (10000*)
		//Double_t variable[9] = { -128.4,  587.7,  -156.4,  674.6,  -199.5,  -197.1,  0.9776,  -5.365,  80.80}; // chi=411  
		//Double_t variable[9] = { -128.4,  587.7,  -156.4,  674.6,  -199.5,  -197.1,  0.9776,  -5.365,  80.80}; // chi=404  
