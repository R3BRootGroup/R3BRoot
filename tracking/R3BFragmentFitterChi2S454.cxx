...
//	double z0 = 0.;
    ...
    for (Int_t i = 0; i < (gSetup->GetArray().size() - 0); i++)
    {
        ...
        if (hit && det->res_x > 1e-6)
        {
            ...
            chi2 += TMath::Power((x_l - hit->GetX()) / det->res_x, 2);
            // LOG(INFO) << nchi2 << "  " << chi2 << ",  dev: " << (x_l - det->hit_x);
            LOG(DEBUG) << "chi2calc x: " << det->GetDetectorName().Data() << " res: " << det->res_x << " pos: " 
				<< x_l << " hit: " << hit->GetX() << " dev: " << x_l - hit->GetX() << " chi2: " << chi2;
            nchi2 += 1;
        }
        if (hit && det->res_y > 1e-6)
        {
            ...
            // LOG(INFO) << nchi2 << "  " << chi2 << ",  dev: " << (x_l - det->hit_x);
            LOG(DEBUG) <<"chi2calc y: " << det->GetDetectorName().Data() << " res: " << det->res_y << " pos: " 
				<< y_l << " hit: " << hit->GetY() << " dev: " << y_l - hit->GetY() << " chi2: " << chi2;
            nchi2 += 1;
        }
    }
...
double Chi2MomentumBackward(const double* xx)
{
	...
	TVector3 pos2;
    TVector3 pos3;
    //LOG(DEBUG3) << "Test: " << gCandidate->GetHitIndexByName("fi12") << "  " << gCandidate->GetHitIndexByName("fi10") << endl;
...
*/	
    ...
    for (Int_t i = (gSetup->GetArray().size() - 2); i >= 0; i--)
    {
        ...
        if (hit && det->res_x > 1e-6)
        {
            chi2 += TMath::Power((x_l - hit->GetX()) / det->res_x, 2);
            // LOG(INFO) << nchi2 << "  " << chi2 << ",  dev: " << (x_l - det->hit_x);
            LOG(DEBUG2) << "chi2calc x: " << det->GetDetectorName().Data() << " res: " << det->res_x << " pos: " 
				<< x_l << " hit: " << hit->GetX() << " dev: " << x_l - hit->GetX() << " chi2: " << chi2;
            nchi2 += 1;
        }
        if (hit && det->res_y > 1e-6)
        {
            chi2 += TMath::Power((y_l - hit->GetY()) / det->res_y, 2);
            // LOG(INFO) << nchi2 << "  " << chi2 << ",  dev: " << (x_l - det->hit_x);
            LOG(DEBUG2) <<"chi2calc y: " << det->GetDetectorName().Data() << " res: " << det->res_y << " pos: " 
				<< y_l << " hit: " << hit->GetY() << " dev: " << y_l - hit->GetY() << " chi2: " << chi2;
            nchi2 += 1;
        }
    }
...
//    double variable[3] = { gCandidate->GetMomentum().X(), gCandidate->GetMomentum().Y(), gCandidate->GetMomentum().Z()};
...
//    double step[3] = { 0.01, 0.01, 0.1 };
    ...
    minimum_m->SetLimitedVariable(2, "pz", variable[2], step[2], gCandidate->GetStartMomentum().Z() - 2., gCandidate->GetStartMomentum().Z() + 2.);
    ...
    TVector3 startMomentum(gCandidate->GetStartMomentum().X(), gCandidate->GetStartMomentum().Y(), gCandidate->GetStartMomentum().Z());	
    TVector3 startPosition(gCandidate->GetStartPosition().X(), gCandidate->GetStartPosition().Y(), gCandidate->GetStartPosition().Z());
    gCandidate->SetStartPosition(startPosition);
    gCandidate->SetStartMomentum(startMomentum);
    gCandidate->Reset();
...
//    TVector3 startPositionOptimized(0., 0., 0.);
    ...
    LOG(DEBUG3) << "current momentum: " <<  gCandidate->GetMomentum().X() << "  " 
		<< gCandidate->GetMomentum().Y() << "  " <<  gCandidate->GetMomentum().Z() ; 
    LOG(DEBUG3) << "current position: " <<  gCandidate->GetPosition().X() << "  " 
		<< gCandidate->GetPosition().Y() << "  " <<  gCandidate->GetPosition().Z() ; 
...
//			gSetup->GetHit("fi13", gCandidate->GetHitIndexByName("fi13"))->GetY());
    ...
    Double_t variable[3] = { gCandidate->GetMomentum().X(), gCandidate->GetMomentum().Y(), gCandidate->GetMomentum().Z()};
    ...
    minimum_a->SetLimitedVariable(2, "pz", variable[2], step[2], gCandidate->GetMomentum().Z() -5., gCandidate->GetMomentum().Z() + 5.);
