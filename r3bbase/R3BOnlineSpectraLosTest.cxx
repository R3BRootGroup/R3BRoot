...
#include "R3BOnlineSpectraLosTest.h"
...
R3BOnlineSpectraLosTest::R3BOnlineSpectraLosTest()
    : FairTask("OnlineSpectraLosTest", 1)
...
R3BOnlineSpectraLosTest::R3BOnlineSpectraLosTest(const char* name, Int_t iVerbose)
    ...
    if (fMappedItems.at(DET_LOS))
    {
        ...
        if (fNofLosDetectors == 2)
        {
            ...
            for (Int_t i = 0; i < 8; i++)
            {
                ...
                fh_test_corr[i] = new TH2F(strName1, strName2, 1000, 0, 10, 400, 1, 400.);
            }
        ...
        for (Int_t iloscount = 0; iloscount < fNofLosDetectors; iloscount++)
        {
            ...
            fh_los_ihit_ToT[iloscount] =
                new TH2F(Form("%s_tot_ihit", detName), Form("%s ToT vs ihit", detName), 1000, 0, 10, 600, 0., 300.);
            ...
            fh_los_tot_mean_corr[iloscount]->Draw();
            cLos[iloscount]->cd(6);
            gPad->SetLogy();
            ...
            cLos[iloscount]->cd(9);
            gPad->SetLogz();
            fh_los_tot_S8[iloscount]->Draw("colz");
            ...
            cLos[iloscount]->cd(15); // gPad->SetLogy();
            fh_los_rw[iloscount]->Draw();
            cLos[iloscount]->cd(0);
...
void R3BOnlineSpectraLosTest::Reset_TWIN_Histo()
{
    if (fMappedItemsTwim)
    {
        ...
        fh2_twim_ESum->Reset();
        fh1_Twimmap_mult->Reset();
    }
}
void R3BOnlineSpectraLosTest::Reset_LOS_Histo()
{
    ...
    if (fMappedItems.at(DET_LOS))
    {
        for (Int_t iloscount = 0; iloscount < fNofLosDetectors; iloscount++)
        {
            ...
            if (fMappedItemsTwim)
                fh_los_twin[iloscount]->Reset();
            fh_los_rw[iloscount]->Reset();
            fh_los_tot_mean_corr[iloscount]->Reset();
        }
        if (fNofLosDetectors > 1)
        {
            fh_los_ToT_vs_ToT->Reset();
            fh_los_nPart_vs_nPart->Reset();
            fh_dt_los1_los2->Reset();
        }
    }
}

void R3BOnlineSpectraLosTest::Reset_ROLU_Histo()
{
    fh_rolu_channels->Reset();
    fh_rolu_tot->Reset();
}

void R3BOnlineSpectraLosTest::Reset_BMON_Histo()
{
    fhTrigger->Reset();
    fhTpat->Reset();
...
void R3BOnlineSpectraLosTest::Reset_SCI8_Histo()
{
...
void R3BOnlineSpectraLosTest::Reset_TOFD_Histo()
{
...
void R3BOnlineSpectraLosTest::Reset_FIBERS_Histo()
{
    if (fMappedItems.at(DET_PSPX))
    {
        for (UInt_t i = 0; i < N_PSPX; i++)
...
void R3BOnlineSpectraLosTest::Exec(Option_t* option)
{
    ...
    for (Int_t idet = 0; idet < fNofLosDetectors; idet++)
    {
        for (Int_t imult = 0; imult < 32; imult++)
        {
            ...
            xToT_cm[idet][imult] = 0.0 / 0.0;
            yToT_cm[idet][imult] = 0.0 / 0.0;
                ...
                if (iLOSType[iDet - 1][iPart])
                {
                    ...
                    for (int ipm = 0; ipm < 8; ipm++)
                    {
                            ...
                            !(IS_NAN(time_T[iDet - 1][iPart][ipm])) && !(IS_NAN(time_L[iDet - 1][iPart][ipm])))
                        {
                            ...
                            if (tot[iDet - 1][iPart][ipm] > 100.)
                                iLOSType[iDet - 1][iPart] = false; // pileup rejection
                        }
                    ...
                    if (foptcond == 0)
                    {
                        ...
                        //   if(iDet==1) fh_los_tres_MTDC->Fill(LosTresMTDC[iPart]);
    ...
    Double_t yfib, xfib, tfibMax = 0. / 0.;
        ...
        if (detHit)
        {
            ...
            for (Int_t ihit = 0; ihit < nHits; ihit++)
            {
                ...
                /*
                                ...
                                if(timeLos[0][0]>0. && timeLos[0][0] < 8192.*5. && !(IS_NAN(timeLos[0][0])))
                                {
                                    while(tSPMT - timeLos[0][0] < 2048.*5./2.)
                                    {
                                        tSPMT = tSPMT + 2048.*5.;
                                    }
                                    while(tSPMT - timeLos[0][0] > 2048.*5./2.)
                                ...
                                tfib = (tMAPMT + tSPMT) / 2.;
                               // tfib = tSPMT;
                                if(tfib > 0. && !(IS_NAN(tfib)) ){
                                 for(Int_t imlos = 0; imlos < nPartc[0]; imlos++){
                                  if((tfib - timeLos[0][imlos]) > 2575. && (tfib - timeLos[0][imlos]) < 2600)  tof_fib =
                tfib - timeLos[0][imlos];
                                  }
                                 }
                               // Not-calibrated position:
                                xpos=iFib;
                ...
                */
                ...
                for (Int_t lhit = 0; lhit < nPartc[1]; lhit++)
                {
                    ...
                    // Los coarse counter has 8192 channels with 5 ns each
                    // SPMT coarse counter has 2048 channels with 5 ns each
                    if (timeLos[1][lhit] > 0. && timeLos[1][lhit] < 8192. * 5. && !(IS_NAN(timeLos[1][lhit])))
                    {
                        while (tSPMT - timeLos[1][lhit] < 2048. * 5. / 2.)
                        {
                            tSPMT = tSPMT + 2048. * 5.;
                        }
                        while (tSPMT - timeLos[1][lhit] > 2048. * 5. / 2.)
                    ...
                    // tfib = tSPMT;
                    // tfib = tMAPMT;
                    if (tfib > 0. && !(IS_NAN(tfib)) && timeLos[1][lhit] > 0. && !(IS_NAN(timeLos[1][lhit])))
            ...
            if (tof_fibMax > tof_fiber[ifibcount] - 10. && tof_fib < tof_fiber[ifibcount] + 10.)
                los_fib_cor[ifibcount] = true;

            randx = (std::rand() / (float)RAND_MAX);
            if (iFibMax_MA > 0)
                posfib = (-n_fiber[ifibcount] / 2. + iFibMax_MA + (0.5 - randx)) * 0.221 / 10.; // in cm
            ...
            //	if(iFibMax_MA > 0)cout<<"FIB POS: "<<fNEvents<<", "<<ifibcount<<"; "<<iFibMax_MA<<",
            ...
            /*
                   if (ifibcount==0  && ((iFibMax_MA >68 && iFibMax_MA<88) || (iFibMax_MA >168 && iFibMax_MA<188))){
                    fib1a_cut=true;
                }
                   if (ifibcount==1  && ((iFibMax_MA >68 && iFibMax_MA<88) || (iFibMax_MA >168 && iFibMax_MA<188))){
                    fib1b_cut=true;
                }
            */
            // Full fib1a, 1b:

            if (ifibcount == 0 && (iFibMax_MA > 1 && iFibMax_MA < 265))
            {
                fib1a_cut = true;
            }
            if (ifibcount == 1 && (iFibMax_MA > 1 && iFibMax_MA < 265))
            {
                fib1b_cut = true;
            }
            ...
            /*
                if (ifibcount==0  && (iFibMax_MA >110 && iFibMax_MA<146)){
                    fib1a_cut=true;
                }
                   if (ifibcount==1  && (iFibMax_MA >110 && iFibMax_MA<146)){
                    fib1b_cut=true;
                }
              */
            if (nHits > 0)
    ...
    if (foptcond == 1 && (fib1a_cut == true && fib1b_cut == true && rfib < 2.) && twin)
    {
        for (Int_t ilos = 0; ilos < fNofLosDetectors; ilos++)
        {
            ...
            {
                if (iLOSType[ilos][imultlos] && nPartc[ilos] == 1)
                {
                    fh_los_pos_MCFD[ilos]->Fill(xV_cm[ilos][imultlos], yV_cm[ilos][imultlos]);
                    fh_los_pos_TAMEX[ilos]->Fill(xT_cm[ilos][imultlos], yT_cm[ilos][imultlos]);
                    fh_los_pos_ToT[ilos]->Fill(xToT_cm[ilos][imultlos], yToT_cm[ilos][imultlos]);
                    fh_los_tres_MCFD[ilos]->Fill(LosTresV[ilos][imultlos]);
                    fh_los_tres_TAMEX[ilos]->Fill(LosTresT[ilos][imultlos]);
                    // fh_los_tot_mean[ilos]->Fill(totsum[ilos][imultlos]);
                    for (int ipm = 0; ipm < 8; ipm++)
                    {
                        fh_los_tot[ilos]->Fill(ipm + 1, tot[ilos][imultlos][ipm]);
                    }
                }
            }
        }
        if (fCalItems.at(DET_LOS) && fNofLosDetectors == 2)
        {
            for (Int_t iPart1 = 0; iPart1 < nPartc[0]; iPart1++)
            {
                for (Int_t iPart2 = 0; iPart2 < nPartc[1]; iPart2++)
                {
                    if ((timeLos[1][iPart2] - timeLos[0][iPart1]) > 2.2 &&
                        (timeLos[1][iPart2] - timeLos[0][iPart1]) < 2.45 && timeLos[1][iPart2] > 0. &&
                        timeLos[0][iPart1] > 0.)
                    {
                        // fh_dt_los1_los2->Fill(timeLos[1][iPart2]-timeLos[0][iPart1]);
                        //  fh_los_ToT_vs_ToT->Fill(totsum[0][iPart1],totsum[1][iPart2]);
                    ...
                    if (iLOSType[idet][imul])
                    {
                        ...
                        */
                        ...
                        if (rfib < 3. && totsum[idet][imul] > 0. && twin)
                        {
                            ...
                            fh_los_ihit_ToT[idet]->Fill(rimpact, totsum_corr[idet][imul]);
                            fh_los_rw[idet]->Fill(sum_alpha[idet][imul]);
                            fh_los_twin[idet]->Fill(totsum_corr[idet][imul], dE_TWIN);

                            /* if(imul == lhitmem[idet])*/ fh_los_tot_S8[idet]->Fill(tof_mem[idet] - 7852.64,
                                                                                     totsum_corr[idet][imul]);
                        }
...
void R3BOnlineSpectraLosTest::FinishEvent()
{
...
void R3BOnlineSpectraLosTest::FinishTask()
{
...
ClassImp(R3BOnlineSpectraLosTest)
