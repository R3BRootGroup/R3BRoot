...
*/
        ...
        {            
            ...
            fh_los_pos_MCFD_cond[iloscount] =
                new TH2F(Form("%s_pos_MCFD_cond", detName), Form("%s MCFD Position_cond ", detName), 500, -5., 5., 500, -5., 5.);
            ...
            fh_los_pos_ToT[iloscount] =
                new TH2F(Form("%s_pos_ToT", detName), Form("%s ToT Position ", detName), 1000, -10., 10., 1000, -10., 10.);
            fh_los_pos_ToT[iloscount]->GetXaxis()->SetTitle("X position / cm");
            fh_los_pos_ToT[iloscount]->GetYaxis()->SetTitle("Y position / cm");
            
            fh_los_pos_ToT_cond[iloscount] =
                new TH2F(Form("%s_pos_ToT_cond", detName), Form("%s ToT Position cond", detName), 1000, -10., 10., 1000, -10., 10.);
            ...
            cLos[iloscount]->cd(4);gPad->SetLogz();
            ...
            cLos[iloscount]->cd(13);gPad->SetLogz();
            fh_xlos_xfib[iloscount]->Draw("colz");
            cLos[iloscount]->cd(14);gPad->SetLogz();
            fh_ylos_yfib[iloscount]->Draw("colz");
            cLos[iloscount]->cd(15);gPad->SetLogz();
    ...
    for (Int_t ifibcount = 0; ifibcount < NOF_FIB_DET; ifibcount++)
    {
        ...
        if (fMappedItems.at(DET_FI_FIRST + ifibcount))
        {
            ...
            fh_Fib_pos[ifibcount] =
                new TH2F(Form("%s_pos", detName), Form("%s Not-calibrated position", detName), 2000,-100,100,2000,-100,100);
...
*/
             ...
             //        cout<<tot[iDet - 1][iPart][1]<<", "<<tot[iDet - 1][iPart][2]<<", "<<tot[iDet - 1][iPart][5]<<", "<<tot[iDet - 1][iPart][6] <<", "<<
             //          tot[iDet - 1][iPart][0]<<", "<<tot[iDet - 1][iPart][3]<<", "<<tot[iDet - 1][iPart][4]<<", "<<tot[iDet - 1][iPart][7] <<endl;
    ...
    for (Int_t ifibcount = 0; ifibcount < NOF_FIB_DET; ifibcount++)
    {
        ...
        if (detMapped)
        {
            ...
            for (int i = 0; i < 16; ++i)
            {
                ...
                if (s > 0) {fh_multihit_s_Fib[ifibcount]->Fill(i + 1, s); }// multihit of a given PADI cha
            }
        }
            ...
            //    if (!(tMAPMT > 0.) || !(tSPMT > 0.))  continue;
              ...
              //cout<<"TEST: "<<fNofLosDetectors<<", "<<fNofLosDetectors-1<<endl;
                ...
                //    if (!iLOSType[1][lhit])  continue;
                    ...
                    // SPMT coarse counter has 2048 channels with 5 ns each
                    if (timeLos[fNofLosDetectors-1][lhit] > 0. &&  !(IS_NAN(timeLos[fNofLosDetectors-1][lhit])))   //timeLos[1][lhit] < 8192. * 5. &&
                    {
        ...
        for (Int_t ilos = 0; ilos < fNofLosDetectors; ilos++)
        {
            ...
            {				
                ...
                {   
					...
					if(dx > -1 && dx < -0.4 && dy > -1.7 && dy < -0.7) //dx > -0.8 && dx < 0.6 && dy > -1.1 && dy < 0.4)
					{
						if(ilos>0) fh_ylos_yfib[ilos]->Fill(xToT_cm[ilos][imultlos]-xToT_cm[ilos-1][imultlos],yToT_cm[ilos][imultlos]-yToT_cm[ilos-1][imultlos]);           
