...
//TofD detector
	...
	if(fMappedItemsTofd){
		...
		for (Int_t j = 0; j < 4; j++)
		{
		     ...
		     fh_tofd_ToF[j] = new TH2F(strName7, strName8, 15,0,15,100000, 5500., 6500.);
			...
			if(j<3)
			{
			    ...
			    sprintf(strName6, "Tofd ToF plane %d and %d ", jk, jk+1); 
			     fh_tofd_dt[j] = new TH2F(strName5, strName6, 50,0,50,80000, -4., 36.);
...
//----------------------------------------------------------------------
    ...
    if(fMappedItemsTofd )
    {
        ...
        for (Int_t ihit = 0; ihit < nHits; ihit++)     
        {
            ...
            Int_t iSide  = hit->GetSideId();   // 1..n
            Int_t iEdge  = hit->GetEdgeId(); 
          ...
          //  cout<<ihit<<", "<<iPlane<<"; "<<iPlaneMem<<"; "<<iBar<<"; "<<iBarMem<<", "<<NumPaddles[iPlane-1]<<endl;
    ...
    if(fCalItemsTofd)
    {
     ...
     /*                 
        cout<<"TOFD Online: "<<fNEvents<<", "<<nHits<<"; "<<ihit<<"; "<<iPlane<<", "<<iBar<<", "<<iBarMem<<", "<<", "<<
                                   hit->fTime1L_ns<<", "<<hit->fTime1T_ns<<", "<<
                                   hit->fTime2L_ns<<", "<<hit->fTime2T_ns<<endl;
       */        
            ...
            if(iBar != iBarMem || NumPaddles[iPlane-1] == 1) // condition on NumPaddles to be removed when more paddles in plane 3 and 4 activ!!! 
            {
                ...
                if(!(IS_NAN(hit->fTime1L_ns))) t1l[0][iPlane-1][iBar-1]=hit->fTime1L_ns;
                if(!(IS_NAN(hit->fTime1T_ns))) t1t[0][iPlane-1][iBar-1]=hit->fTime1T_ns;
                if(!(IS_NAN(hit->fTime2L_ns))) t2l[0][iPlane-1][iBar-1]=hit->fTime2L_ns;
                if(!(IS_NAN(hit->fTime2T_ns))) t2t[0][iPlane-1][iBar-1]=hit->fTime2T_ns;  
            ...
            else
            {
			    ...
			    if(!(IS_NAN(hit->fTime1L_ns))) t1l[jm][iPlane-1][iBar-1]=hit->fTime1L_ns;
                if(!(IS_NAN(hit->fTime1T_ns))) t1t[jm][iPlane-1][iBar-1]=hit->fTime1T_ns;
                if(!(IS_NAN(hit->fTime2L_ns))) t2l[jm][iPlane-1][iBar-1]=hit->fTime2L_ns;
                if(!(IS_NAN(hit->fTime2T_ns))) t2t[jm][iPlane-1][iBar-1]=hit->fTime2T_ns;
       ...
       */ 
        ...
        {  
	      ...
	      // calculate time over threshold and check if clock counter went out of range
                ...
                while(t1t[jm][iPlane-1][iBar-1] - t1l[jm][iPlane-1][iBar-1] < 0.) 
               {
	                t1t[jm][iPlane-1][iBar-1]=t1t[jm][iPlane-1][iBar-1]+2048.*fClockFreq; 
	            }

                while(t2t[jm][iPlane-1][iBar-1]-t2l[jm][iPlane-1][iBar-1] < 0.) 
                {
	                t2t[jm][iPlane-1][iBar-1]=t2t[jm][iPlane-1][iBar-1]+2048.*fClockFreq; 
                }
	            ...
	            if( !(IS_NAN(timeLos[ilc])) && timeLos[ilc]>0.)
	            {
	                ...
	                while(t1l[jm][iPlane-1][iBar-1]-timeLos[ilc] < 0.)
	                {
	                    t1t[jm][iPlane-1][iBar-1]=t1t[jm][iPlane-1][iBar-1]+2048.*fClockFreq; 
	                    t1l[jm][iPlane-1][iBar-1]=t1l[jm][iPlane-1][iBar-1]+2048.*fClockFreq; 
	                    t2t[jm][iPlane-1][iBar-1]=t2t[jm][iPlane-1][iBar-1]+2048.*fClockFreq; 
	                    t2l[jm][iPlane-1][iBar-1]=t2l[jm][iPlane-1][iBar-1]+2048.*fClockFreq; 			  
		            }
	// ToF	  
        ...
        //  between 2 bars in 2 planes  
		 ...
		 // between LOS and paddle          		            
		            ToF[jm][iPlane-1][iBar-1] = t_paddle[jm][iPlane-1][iBar-1] - timeLos[ilc];
