...
TCanvas* cLos[fNofLosDetectors];
    ...
    fh_spill_times_fib_fine = new TH1F("CountsPerSpillFiber_fine", "counts per spill fine", 500000, 0., 5.);//s, 10µs bin ///?
    fh_spill_times_fib_Coarse = new TH1F("CountsPerSpillFiber_coarse", "counts per spill coarse", 500, 0., 5.);//s, 10ms bin ///?
	...
	fh_FIB_TrgW_dt_coarse = new TH1F("dt_TrggW_Fib_coarse","Time-difference within Trigger window", 1000, 0., 10.);//µs, 10ns bin
	fh_LOS_TrgW_dt_coarse = new TH1F("dt_TrggW_LOS_coarse","Time-difference within Trigger window", 1000, 0., 10.);//µs, 10ns bin
	fh_SAMP_TrgW_dt_coarse = new TH1F("dt_TrggW_SAMP_coarse","Time-difference within Trigger window", 1000, 0., 10.);//µs, 10ns bin
	...
	fh_spill_times_LOS = new TH1F("CountsPerSpillLOSRaw", "counts per spill RAW [100us bin]", 120000, 0., 12.);//bin 100µs
	fh_spill_times_LOS_Fine = new TH1F("CountsPerSpillLOSfine", "counts per spill fine [10us bin] ", 1000000, 0., 10.);//10µs bin
	fh_spill_times_LOS_Fine_adj = new TH1F("CountsPerSpillLOSfineadj", "counts per spill fine [10us bin] ", 1000000, 0., 10.);//10µs bin
	fh_spill_times_LOSFIB_Fine = new TH1F("CountsPerSpillLOSFIBfine", "counts per spill fine Sampler + FIB [10us bin] ", 1000000, 0., 10.);//10µs bin
	fh_spill_times_LOSFIB_Fine_adj = new TH1F("CountsPerSpillLOSFIBfineadj", "counts per spill fine Sampler + FIB [10us bin] ", 1000000, 0., 10.);//10µs bin
	fh_spill_times_LOS_Coarse = new TH1F("CountsPerSpillLOScoarse", "counts per spill coarse [10ms bin] ", 1200, 0., 12.);//10ms bin
	fh_SAMPFIB_tDiff = new TH1F("Time_difference_SAMPFIB", "Time-difference Sampler + Fiber", 200000, 0., 1.);//1ms, 5ns bin
	...
	fh_spill_times_LOS_pois = new TH1F("Random_distributed_times","Random_tDiff", 10000000*fSpillLength, 0., fSpillLength);//0-5s, 100ns bin
    ...
    fh_spill_times_daq_first = new TH1F("CountsPerSpill_daq_first", "counts per spill daq first hit only", 50000, 0., 5.);
	...
	for(Int_t ifibcount = 0; ifibcount < NOF_FIB_DET; ifibcount++){
		...
		if(fMappedItems.at(DET_FI_FIRST + ifibcount)) {
			...
			// ToT MAPMT:  
			//fh_ToT_m_Fib[ifibcount] = new TH2F(Form("%s_tot_m",detName), Form("%s ToT of MAPMT",detName), N_FIBER_PLOT, 0., N_FIBER_PLOT, 100, 0., 100.);   	   
			fh_ToT_m_Fib[ifibcount] = new TH2F(Form("%s_tot_m",detName), Form("%s ToT of MAPMT",detName),512, 0., 512, 300, 0., 100.);   	   
			fh_ToT_m_Fib[ifibcount]->GetXaxis()->SetTitle("Fiber number");
			fh_ToT_m_Fib[ifibcount]->GetYaxis()->SetTitle("ToT / ns");

			// ToT SAPMT:  
			fh_ToT_s_Fib[ifibcount] = new TH2F(Form("%s_tot_s",detName), Form("%s ToT of single PMT",detName), 512, 0., 512, 400, 0., 40.);   	   
			fh_ToT_s_Fib[ifibcount]->GetXaxis()->SetTitle("Fiber number");
			fh_ToT_s_Fib[ifibcount]->GetYaxis()->SetTitle("ToT / ns");

			// Time of fiber: 
			fh_time_Fib[ifibcount]= new TH2F(Form("%s_TimevsFiber",detName), Form("%s Time vs Fiber",detName), 512, 0., 512, 2048, -1024., 1024.);
			fh_time_Fib[ifibcount]->GetXaxis()->SetTitle("Fiber number");
			fh_time_Fib[ifibcount]->GetYaxis()->SetTitle("tMAPMT-tSPMT");

			// ToF LOS -> Fiber:
			fh_Fib_ToF[ifibcount] = new TH2F(Form("%s_tof",detName), Form("%s ToF LOS to Fiber",detName), 512, 0., 512,1000, -50000., 50000.);	   
			fh_Fib_ToF[ifibcount]->GetYaxis()->SetTitle("ToF / ns");
			fh_Fib_ToF[ifibcount]->GetXaxis()->SetTitle("Fiber number");

			// Not-calibrated position:
			fh_Fib_pos_xy[ifibcount] = new TH2F(Form("%s_pos",detName), Form("%s xy position",detName), 500, 0., 500, 1000, 0., 200.);	   
	...
	fh_DutyFactor_MtA = new TH1F("DutyFactor_MtA", "Duty Factor MtA", 1200., 0., 12./*, 2000., 0., 2.*/);//timebin in 10ms
	fh_DutyFactor_LOS_pois = new TH1F("DutyFactor_pois", "Duty Factor", 1200., 0., 12./*, 2000., 0., 2.*/);//timebin in 10ms
	fh_DutyFactor_MaxToAvg = new TH1F("MaxToAvg", "Duty Factor - Maximum to Average", 360., 0., 180./*, 2000., 0., 2.*/);//timebin in 10ms
	fh_DutyFactor_MaxRun = new TH1F("DFMaxRun", "Duty Factor - Maximum over run", 360., 0., 180./*, 2000., 0., 2.*/);//timebin in 10ms
	fh_DutyFactor_AvgRun = new TH1F("DFAvgRun", "Duty Factor - Average over run", 360., 0., 180./*, 2000., 0., 2.*/);//timebin in 10ms
	...
	fh_DutyFactor_SAMP_clean = new TH1F("DutyFactor_SAMPFIB_clean", "Duty Factor meassured/poisson", 1200., 0., 12./*, 2000., 0., 2.*/);//timebin in 10ms
   ...
   // fh_spill_Q_E_pois->Draw("SAME B");
	...
	fh_DutyFactor_SAMPFIB = new TH1F("DutyFactor_SAMPFIB", "Duty Factor", 1200., 0., 12./*, 2000., 0., 2.*/);//timebin in 10ms
	fh_DutyFactor_SAMPFIB_pois = new TH1F("DutyFactor_SAMPFIB_pois", "Duty Factor", 1200., 0., 12./*, 2000., 0., 2.*/);//timebin in 10ms
	fh_rate_sum_SAMPFIB = new TH1F("rate_sum_SAMPFIB", "Rate accepted relative to poisson", 200, -100.,100.);
	fh_rate_sum_DTmean_SAMPFIB = new TH1F("rate_sum_SAMPFIB_DTmean", "Rate accepted relative to poisson", 200, 0.,200.);
	...
	fh_DutyFactor_SAMPFIB_clean = new TH1F("DutyFactor_SAMPFIB_clean", "Duty Factor meassured/poisson", 1200., 0., 12./*, 2000., 0., 2.*/);//timebin in 10ms
  ...
  if(fMappedItems.at(DET_SAMPLER)){
		...
		for (Int_t ihit = 0; ihit < nHitsSamp; ihit++)
		{
			...
			if(spill_on_sampler && ihit == 0) {
				...
				//cout<<"sampSpill: "<<samplerSpill<<" samplerCurr: "<<samplerCurr<<"  "<<samplerSpill-samplerCurr<<endl;
			...
			if(spill_ctr_LOS>0){
				...
				if(dt<0)
					dt += 1073741823.;
/**Only for the LaserRUN!!!!!!*///if(dt==11.){ ///Only for the Laser Run! There seems to be some Reflection-effect for the Laser, so that 
///														Very repeatedly a Hit occurs after 110ns. Filter out these hits.
  ...
  if(header->GetTrigger() == 13){
	...
	else if(spill_off_calc) spill_off = true;
	
	else spill_off = false;
  ...
  if(spill_off == true ){  
		...
		Int_t pps_LOS = fh_spill_times_LOS->Integral(1,fh_spill_times_LOS->GetSize() - 2);///pps_fib = parts per spill?    
		...
		if(time_spill_start > 0 && pps_LOS > 0 && spill_ctr_LOS > 0){
			...
			/**The choice of how to calculate the deadtime is not yet completed. Most accurate way appears to be DeadTime by calibration->Deadtime by Maximum gets closest. */
			...
			Double_t DeadTime_set = fDAQ_dead_time;//Set a deadtime, possibility if no calculation should be done -> not dynamic
			...
			Double_t TotZeit_geeicht = TotZeitEichung(fSpillLength, (double)pps_LOS);//Calibrate DeadTime via function. Got parameters through calibration with known poisson dist. 
			...
			Double_t accepted_LOS = ((double)pps_LOS / fSpillLength) / (1 + ((double)pps_LOS / fSpillLength) * deadTime * 1.e-6);//accepted rate! Calibrated DeadTime
			/**
			Double_t accepted_LOS_set = ((double)pps_LOS / fSpillLength) / (1 + ((double)pps_LOS / fSpillLength) * DeadTime_set * 1.e-6);//accepted rate! Fix DeadTime
			///Double_t accepted_LOS_adj = ((double)pps_LOS / fSpillLength) / (1 + ((double)pps_LOS / fSpillLength) * DeadAdj * 1.e-6);//accepted rate! Mean DeadTime
			Double_t accepted_LOS_adj = ((double)pps_LOS / fSpillLength) / (1 + ((double)pps_LOS / fSpillLength) * TotZeit_geeicht * 1.e-6);//accepted rate! calibrated DeadTime
			*/
			Double_t accepted_SAMPFIB = ((double)pps_SAMPFIB / fSpillLength) / (1 + ((double)pps_SAMPFIB / fSpillLength) * deadTime * 1.e-6);//accepted rate!
			/**
			Double_t accepted_SAMPFIB_set = ((double)pps_SAMPFIB / fSpillLength) / (1 + ((double)pps_SAMPFIB / fSpillLength) * DeadTime_set * 1.e-6);//accepted rate!
			Double_t accepted_SAMPFIB_adj = ((double)pps_SAMPFIB / fSpillLength) / (1 + ((double)pps_SAMPFIB / fSpillLength) * TotZeit_geeicht * 1.e-6);//accepted rate!
			*/
			cout  << "Ratio pois LOS: " << R_Poisson_LOS << "  " <<
			" Accepted LOS trigger: "<<accepted_LOS / (double)pps_LOS * fSpillLength  << "  " << //accepted rate relative to full rate
			/*(double)pps_daq_first / (double)pps_fib << "  " <<
			(double)pps_daq / (double)pps_fib<<*/ endl;
			...
			//cout<<"value PPSdf:  "<< (double)pps_daq_first_LOS<<" accepted: "<<accepted_LOS<<" t_spill: "<<fSpillLength<<endl;
/**Calibrated DeadTime */fh_rate->SetBinContent(2*spillCounter-1,(double)pps_daq_first_LOS/accepted_LOS/fSpillLength*100. - 100.); //Number of event-trigger / (accepted rate * t_spill)
/**Calibrated DeadTime */fh_rate_SAMPFIB->SetBinContent(2*spillCounter-1,(double)pps_daq_first_LOS/accepted_SAMPFIB/fSpillLength*100. - 100.); //Number of event-trigger / (accepted rate * t_spill)

/**Fixed DeadTime*/ // fh_rate_set->SetBinContent(2*spillCounter-1,(double)pps_daq_first_LOS/accepted_LOS_set/fSpillLength*100. - 100.); //Number of event-trigger / (accepted rate * t_spill)
/**Fixed DeadTime*/ // fh_rate_set_SAMPFIB->SetBinContent(2*spillCounter-1,(double)pps_daq_first_LOS/accepted_SAMPFIB_set/fSpillLength*100. - 100.); //Number of event-trigger / (accepted rate * t_spill)
/**Mean DeadTime*/ // fh_rate_adj->SetBinContent(2*spillCounter-1,(double)pps_daq_first_LOS/accepted_LOS_adj/fSpillLength*100. - 100.); //Number of event-trigger / (accepted rate * t_spill)
/**Mean DeadTime*/ // fh_rate_adj_SAMPFIB->SetBinContent(2*spillCounter-1,(double)pps_daq_first_LOS/accepted_SAMPFIB_adj/fSpillLength*100. - 100.); //Number of event-trigger / (accepted rate * t_spill)
			// => #event-trigger / #accepted hits = ratio 
			/** With the given dead-time, accepted_LOS gives us the rate of hits that are (theoretically) expected to be accepted. By dividing the number of daq_first
			 * hits (these are the hits, that trigger the readout window), we get the number of events / expected accepted hits. For a periodic (and/or low 
			 * intesity) spill, the number of recorded events is equal to the number of real events.*/
			...
			//cout<<"Size: "<<Size<<" SizePois: "<<Size_pois<<" pps_LOS: "<<pps_LOS<<" pps_LOS_pois: "<<pps_LOS_pois<<endl;
			Int_t real_count = 0;
			Int_t pois_count = 0;
...
for(int k1 = 0; k1<2; k1++){//Loop over "only sampler"(k1=0) & "sampler & fiber1" (k1=1)	
	int Xmax = 0;
	if(k1==0) Xmax = fh_spill_times_LOS_Fine->FindLastBinAbove(0,1);
	if(k1==1) Xmax = fh_spill_times_LOSFIB_Fine->FindLastBinAbove(0,1);
	...
	for(int i = 0; i<=Xmax; i++){
		...
		if(k1==0) N_count = fh_spill_times_LOS_Fine->GetBinContent(i);
		if(k1==1) N_count = fh_spill_times_LOSFIB_Fine->GetBinContent(i);
		if(N_MtA < N_count) N_MtA = N_count;
		Np += N_count;
		Nps += N_count*N_count;
		NCTR++;
		if((((i+1)%1000 == 0) || i == Xmax) && (Nps > 0)){
			...
			Np = 0;
			Nps = 0;
			Double_t Filler = ((double)(i))/1000.;///Not exactly sure why, but need to reduce i by 1, otherwise there occurs a problem in the binning and some bins arent getting filled, while the ones before are filled twice.

			if(Filler<0.) Filler += fSpillLength;
			if(F_duty > -1000. && F_duty < 1000.){ 
				if(k1==0) fh_DutyFactor_LOS->SetBinContent(Filler, F_duty);
				if(k1==1) fh_DutyFactor_SAMPFIB->SetBinContent(Filler, F_duty);
			}
			if(FD/(FD+1.) > -1000. && FD/(FD+1.) < 1000.){	
				if(k1==0) fh_DutyFactor_LOS_pois->SetBinContent(Filler, FD/(FD+1.));///Get the Poisson-Limit for a given N (=FD). This is the highest possible value. If DutyFactor is lower, then there is a problem with a device.
				if(k1==1) fh_DutyFactor_SAMPFIB_pois->SetBinContent(Filler, FD/(FD+1.));///Get the Poisson-Limit for a given N (=FD). This is the highest possible value. If DutyFactor is lower, then there is a problem with a device.
			}
			if(k1==0) fh_DutyFactor_SAMP_clean->SetBinContent(Filler, ( F_duty - FD/(FD+1.) )/( FD/(FD+1.) ) * 100. );
			if(k1==1) fh_DutyFactor_SAMPFIB_clean->SetBinContent(Filler, ( F_duty - FD/(FD+1.) )/( FD/(FD+1.) ) * 100. );
			...
			if(k1==0) fh_DutyFactor_MtA->SetBinContent(Filler, (double)N_MtA/(double)pps_LOS);
			N_MtA = 0;
		}
	...
	Double_t DutyMax_clean = fh_DutyFactor_SAMPFIB_clean->GetBinContent(fh_DutyFactor_SAMPFIB->GetMaximumBin());
...
for(int ft = 0; ft<2; ft++){	
		...
		if(spillCounter > 1)
		fh_FFT->Reset();
		if(ft==0) fh_spill_times_LOS_Fine_adj->Reset();
		if(ft==1) fh_spill_times_LOSFIB_Fine_adj->Reset();
		if(ft==0) fh_FFT_adj->Reset();
		if(ft==1) fh_FFT_adj_SAMPFIB->Reset();
		
		Double_t mean_of_spill = 0;
		Int_t Last_of_spill = 0;
		Int_t First_of_spill = 0;
		Int_t Int_mean = 0;
		if(ft==0) Last_of_spill = fh_spill_times_LOS_Fine->FindLastBinAbove(0,1);
		if(ft==1) Last_of_spill = fh_spill_times_LOSFIB_Fine->FindLastBinAbove(0,1);
		if(ft==0) First_of_spill = fh_spill_times_LOS_Fine->FindFirstBinAbove(0,1);
		if(ft==1) First_of_spill = fh_spill_times_LOSFIB_Fine->FindFirstBinAbove(0,1);
		if(ft==0) Int_mean = fh_spill_times_LOS_Fine->Integral(First_of_spill, Last_of_spill);
		if(ft==1) Int_mean = fh_spill_times_LOSFIB_Fine->Integral(First_of_spill, Last_of_spill);
		Int_t mean_count = 0;
		for(int k1=First_of_spill;k1<Last_of_spill+1;k1++){
			if(ft==0) mean_of_spill += fh_spill_times_LOS_Fine->GetBinContent(k1);
			if(ft==1) mean_of_spill += fh_spill_times_LOSFIB_Fine->GetBinContent(k1);
			mean_count++;
		}

		
		///Substract the mean (here calculated via integral) from the spill to eliminate the lowest freq. (0th bin of FFT)
		for(int i = First_of_spill; i<Last_of_spill+1; i++){
			if(ft==0) fh_spill_times_LOS_Fine_adj->SetBinContent(i, fh_spill_times_LOS_Fine->GetBinContent(i) - (double)mean_of_spill/(double)mean_count);
			if(ft==1) fh_spill_times_LOSFIB_Fine_adj->SetBinContent(i, fh_spill_times_LOSFIB_Fine->GetBinContent(i) - (double)mean_of_spill/(double)mean_count);
		}
		
		TH1 *fft = 0;
		//actual FFT
		TVirtualFFT::SetTransform(0);
		if(ft==0) fh_FFT_adj = fh_spill_times_LOS_Fine_adj->FFT(fh_FFT_adj, "MAG");
		if(ft==1) fh_FFT_adj_SAMPFIB = fh_spill_times_LOSFIB_Fine_adj->FFT(fh_FFT_adj, "MAG");
		///Add FFTs
		if(ft==0) fh_FFT_add->Add(fh_FFT_adj);
		if(ft==1) fh_FFT_add_SAMPFIB->Add(fh_FFT_adj_SAMPFIB);
	...
	Double_t nPPS = 0.;
	if(pps_LOS<100000.) nPPS = pps_LOS*1000.;
	if(pps_LOS>=100000.) nPPS = pps_LOS*10.;
...
for(int k2=0; k2<2; k2++){	//Loop over "only sampler"(k2=0) & "sampler & fiber1" (k2=1)
	//Now get the Integral of the real, and the poisson dt. Get the maximum of pois. Ratio of Area  until the Max is quality
	//Int_t max_pois = fh_LOS_tDiff_pois->GetMaximumBin();
	//Int_t max_real = fh_LOS_tDiff->GetMaximumBin();
	...
	if(k2==0) int_pois = fh_LOS_tDiff_pois->Integral(1, 200);//Get Integral of tDiffs equal and below 1µs
	if(k2==1) int_pois = fh_LOS_tDiff_pois->Integral(1, 200);//Get Integral of tDiffs equal and below 1µs
	if(k2==0) int_real = fh_LOS_tDiff->Integral(1, 200);	   //with 5ns bin from 0th to 200th bin
	if(k2==1) int_real = fh_SAMPFIB_tDiff->Integral(1, 200);	   //with 5ns bin from 0th to 200th bin
	if(k2==0) int_pois_byMax = fh_LOS_tDiff_pois->Integral(1, max_pois);//Get Integral of tDiffs equal and below 1µs
	if(k2==1) int_pois_byMax = fh_LOS_tDiff_pois->Integral(1, max_pois);//Get Integral of tDiffs equal and below 1µs
	if(k2==0) int_real_byMax = fh_LOS_tDiff->Integral(1, max_real);	   //with 5ns bin from 0th to 200th bin
	if(k2==1) int_real_byMax = fh_SAMPFIB_tDiff->Integral(1, max_real);	   //with 5ns bin from 0th to 200th bin
	Double_t int_real_LOS = fh_los_dt_hits[0]->Integral(1, 200);	   //with 5ns bin from 0th to 200th bin
	/**One can either use 1µs tDiff as Integration limit or the mean -> both show the same course, but the absolute values are a bit different. Both seem valid and independent of particle rate, because we compare it to a poisson distribution
	 * with the same mean value => the same particle rate. */
	
	cout<<"int_pois: "<<int_pois<<" int_real: "<<int_real<<endl;
	Double_t frac_int_real = 0;
	if(k2==0) frac_int_real = int_real / fh_LOS_tDiff->Integral(1, fh_LOS_tDiff->GetSize() -2);
	if(k2==1) frac_int_real = int_real / fh_SAMPFIB_tDiff->Integral(1, fh_SAMPFIB_tDiff->GetSize() -2);
	...
	if(k2==0) frac_int_real_byMax = int_real_byMax / fh_LOS_tDiff->Integral(1, fh_LOS_tDiff->GetSize() -2);
	if(k2==1) frac_int_real_byMax = int_real_byMax / fh_SAMPFIB_tDiff->Integral(1, fh_SAMPFIB_tDiff->GetSize() -2);
	Double_t  frac_int_pois_byMax = int_pois_byMax / fh_LOS_tDiff_pois->Integral(1, fh_LOS_tDiff_pois->GetSize() -2);
	...
	/* Since the exponential distribution has many entries (high statistics) compare the fractions of dt's under 1µs */
	...
	if(!IS_NAN(Hans)&&k2==0){
		...
		if(!IS_NAN(Hans2)){
			if(k2==0) fh_spill_hans->SetBinContent(2*spillCounter-1, 100.*Hans2);
			if(k2==1) fh_spill_hans_SAMPFIB->SetBinContent(2*spillCounter-1, 100.*Hans2);
			if(k2==0) fh_hans_sum->Fill(100.*Hans2);
			if(k2==1) fh_hans_SAMPFIB_sum->Fill(100.*Hans2);
		}
		if(!IS_NAN(Hans)){
			if(k2==0) fh_spill_hans_byMax->SetBinContent(2*spillCounter-1, 100.*Hans);
			if(k2==1) fh_spill_hans_SAMPFIB_byMean->SetBinContent(2*spillCounter-1, 100.*Hans);
			if(k2==0) fh_hans_sum_byMax->Fill(100.*Hans2);
			if(k2==1) fh_hans_SAMPFIB_sum_byMean->Fill(100.*Hans2);
		}
		
}		
	/**Test if time diffence distribution is a exponential distribution (therefore particles are poisson distributed) with F = 2 * µ²/(µ² + sigma²) of tDiffs (Enis Lorenz)*/	
	/**If spill is poisson distr. then F = 100%, for better (sharper around mean) F>100%, for higher stddev (worse) F<100% */
for(int k3=0; k3<2; k3++){	
	...
	if(k3==0) mean_pois = fh_LOS_tDiff_pois->GetMean();
	if(k3==1) mean_pois = fh_LOS_tDiff_pois->GetMean();
	if(k3==0) mean_real = fh_LOS_tDiff->GetMean();
	if(k3==1) mean_real = fh_SAMPFIB_tDiff->GetMean();
	if(k3==0) stdDev_pois = fh_LOS_tDiff_pois->GetStdDev();
	if(k3==1) stdDev_pois = fh_LOS_tDiff_pois->GetStdDev();
	if(k3==0) stdDev_real = fh_LOS_tDiff->GetStdDev();
	if(k3==1) stdDev_real = fh_SAMPFIB_tDiff->GetStdDev();
	
	Double_t FQ_pois = (mean_pois*mean_pois)/(mean_pois*mean_pois + stdDev_pois*stdDev_pois);//Qualityfactor

	Double_t FQ_real = 1./( 1. + (stdDev_real*stdDev_real)/(mean_real*mean_real) );//Qualityfactor of poisson distribution -> must be 100%

	if(!IS_NAN(FQ_real)){
		if(k3==0) fh_spill_Q_E_real -> SetBinContent(2*spillCounter-1, 200*FQ_real - 100.);
		if(k3==1) fh_spill_Q_E_real_SAMPFIB -> SetBinContent(2*spillCounter-1, 200*FQ_real - 100.);
		if(k3==0) fh_EL_sum->Fill(200.*FQ_real - 100.);
		if(k3==1) fh_EL_SAMPFIB_sum->Fill(200.*FQ_real - 100.);
	}
	if(!IS_NAN(FQ_pois)){
		if(k3==0) fh_spill_Q_E_pois -> SetBinContent(2*spillCounter, 200*FQ_pois - 100.);
		if(k3==1) fh_spill_Q_E_pois_SAMPFIB -> SetBinContent(2*spillCounter, 200*FQ_pois - 100.);
	}
	//When FQ is Filled subtract 100 to normalize it to other methods of analysis => 0 = poisson, lower than 0 = worse, higher than 0 = better
}
...
// End sorting    
    ...
    { 
		...
		for(int ihit=nHits-1;ihit>-1; ihit--){///Try going backwards through nHits -> Time order should be conserved this way

			if(ihit < 0) break; ///Failsafe, if nHits == 0, or other bug
...
#define USE_MAPMT
				...
				if(ifibcount<2){//only look at Fiber1a & Fiber1b
					 tHit[ihit] = hit->GetTime();
					 tHitFibID[ihit] = hit->GetFiberId();
					 tHitToT[ihit] = hit->GetMAPMTToT_ns();
					//Collect HitTime, FiberID & ToT
					//cout<<"ihit: "<<ihit<<" iFib: "<<iFib<<" tMA: "<<tMAPMT<<" tS: "<<tSPMT<<" tHit: "<<hit->GetTime()<<endl;//for Debugging
				}
				
				///Loop over LOS-Hits to make Coincidence between LOS & Fiber1, not directly needed for micro spill structure
...
//                if(iFib > 0) xpos = ((-n_fiber[ifibcount]/2.+iFib+(0.5-randx)))*0.21; 
           ...
           Int_t nHits_corrected = 0;
				if(sort_array(&nHits, tHit, tHitFibID, tHitToT)){//Sorting function in header -> if(tHit[i] == -1.e9) -> need to sort these out. Let following loop start at nHits_corrected
					for(int i=0; i<nHits; i++){
						if(tHit[i] == -1.e9) nHits_corrected++;
						else break;
					}
				}
				...
				/**Now check hits for crosstalk, merge if necessary and fill them into tHitFib1a/b to be able to make coincidence
				 * --Checking Hits within 5ns interval, since MAPMT channels have delays upto 5ns. Hence it's possible for CT within 5ns */
						...
						if(tHit[k+1] > -9000. && tHit[k+1] < 9000.){
							for(int i=k; i<nHits-1; i++){
								...
								if(tHit[i+1] - tHit[k] < 5.){
									...
									if(check_anode_CT(tHitFibID[i+1], tHitFibID[k])) no_ACT = false;
									if(check_fiber_CT(tHitFibID[i+1], tHitFibID[k])) no_FCT = false;
									//if(!no_ACT) cout<<"Found Anode CT"<<endl;//For Debugging
									//if(!no_FCT) cout<<"Found Fiber CT"<<endl;//For Debugging
									
									if(no_ACT && no_FCT) break;
								}
								//std::cout<<"arr["<<k<<"]: "<<tHit[k]<<" arr["<<i+1<<"]: "<<tHit[i+1]<<std::endl;//For Debugging
								merge_counter++;
							}
							if(merge_counter == 0){
								if(ifibcount==0)
							...
							//	cout<<"Merge Counter: "<<merge_counter<<endl;
								...
								if(ifibcount==0){
							//		cout<<"Merged Hit to tHitFib["<<point<<"]: "<<tHitFib1a[point]/(double)(merge_counter+1)<<endl;//For Debugging
									tHitFib1a[point] = tHitFib1a[point]/(double)(merge_counter+1);
								}
								if(ifibcount==1){
								//	cout<<"Merged Hit to tHitFib["<<point<<"]: "<<tHitFib1b[point]/(double)(merge_counter+1)<<endl;//For Debugging
    	...
    	/* Since it's possible for Fib1a & Fib1b a different amount of hits, we need to adjust the length of the arrays to compare them.
    	 * Take the length of the longer array.*/
    	Int_t numctr = 0;
    	if(numHits1a < numHits1b)
    	numctr = numHits1a;
    	else numctr = numHits1b;
    	Int_t numctr2 = 0;
    	if(numHits1a > numHits1b)
    	numctr2 = numHits1a;
    	else numctr2 = numHits1b;
    	...
    	if(numHits1a < numHits1b){
			...
			for(int t=0; t<numHits1b; t++){
				if(t<numHits1a) tHitFib1a[t] = tmp[t];
				else tHitFib1a[t] = 0./0.;
				tHitCoince[t] = 0./0.;
			}
			delete[] tmp;
			tmp = NULL;
			...
			for(int t=0; t<numHits1a; t++){
				if(t<numHits1b) tHitFib1b[t] = tmp[t];
				else tHitFib1b[t] = 0./0.;
				tHitCoince[t] = 0./0.;
			}
			delete[] tmp;
			tmp = NULL;
		...
		for(int i=0; i<numctr2; i++){
			if(use_fib1a){
				if(!IS_NAN(tHitFib1a[i])) tHitCoince[i] = tHitFib1a[i];
			}
			if(use_fib1b){
				if(!IS_NAN(tHitFib1b[i])) tHitCoince[i] = tHitFib1b[i];
			}
		}
    	...
    	for(int h = 0; h<numctr2; h++){
			...
			if(!IS_NAN(tHitCoince[h])) FibHitCtr++;
		}		
    	...
    	for(int i=0; i<FibHitCtr; i++){
		  ...
		  fh_FIB_count_t->Fill(tHitCoince[i]/1000.);//µs
			if(is_first){
				if(spillon_fib){///First in Spill?
					FIB_time_spill = 0.;///Time of first Hit in Spill->Set to 0
					spill_ctr_FIB++;
					spillon_fib = false;
				}
				else{
					Double_t time_lastInPrevWindow_minus_firstInNewWindow = tLastInWindow - tHitCoince[i];///Calculate tDiff between two TriggerWindows
			...
			if(timedifference < 100.){
				 fh_spill_times_LOSFIB_Fine->Fill(FIB_time_spill/1.e9);//Fill hits, that are registered by Fiber, but not by Sampler	
		...
		tHitFib1b = NULL;
		tHitCoince = NULL;
		/**At this point tDiffs and HitTimes over spill are calculated. For Spillquality analysis go to if(spill_off){...}, like for LOS/Sampler*/
...
{    
	...
	cout<<"RS Mean: "<<fh_DutyFactor_Avg->GetMean()<<" std: "<<fh_DutyFactor_Avg->GetStdDev()<<" RS peak: "<<fh_DutyFactor_Max->GetMean()<<" std: "<<fh_DutyFactor_Max->GetStdDev()<<endl;
	cout<<"RS Mean normalized: "<<fh_DutyFactor_Avg_clean->GetMean()<<" std: "<<fh_DutyFactor_Avg_clean->GetStdDev()<<" RS peak normalized: "<<fh_DutyFactor_Max_clean->GetMean()<<" std: "<<fh_DutyFactor_Max_clean->GetStdDev()<<endl;
	cout<<"PPT Mean: "<<fh_spillQuality_ppt_stat->GetMean()<<" std: "<<fh_spillQuality_ppt_stat->GetStdDev()<<endl;
	cout<<"DeadTimeAdj: "<<fh_deadTime_adj->GetMean()<<"µs with "<<fh_deadTime_adj->GetStdDev()<<"µs stdd"<<endl;
...
//	cout<<"MH DTMEAN Mean: "<<fh_rate_sum_DTmean->GetMean()<<" std: "<<fh_rate_sum_DTmean->GetStdDev()<<endl;
	cout<<"RS Mean: "<<fh_DutyFactor_Avg_SAMPFIB->GetMean()<<" std: "<<fh_DutyFactor_Avg_SAMPFIB->GetStdDev()<<" RS peak: "<<fh_DutyFactor_Max_SAMPFIB->GetMean()<<" std: "<<fh_DutyFactor_Max_SAMPFIB->GetStdDev()<<endl;
	cout<<"RS Mean normalized: "<<fh_DutyFactor_Avg_SAMPFIB_clean->GetMean()<<" std: "<<fh_DutyFactor_Avg_SAMPFIB_clean->GetStdDev()<<" RS peak normalized: "<<fh_DutyFactor_Max_SAMPFIB_clean->GetMean()<<" std: "<<fh_DutyFactor_Max_SAMPFIB_clean->GetStdDev()<<endl;
