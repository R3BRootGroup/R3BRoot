...
InitStatus R3BOnlineSpectraToFD_S494::Init()
{
    ...
    if (fMappedItems.at(DET_TOFD))
    {
        ...
        for (Int_t j = 0; j < 4; j++)
        {
            ...
            fh_tofd_multihit[j] = new TH2F(strName7, strName8, 90, -45., 45., 20, 0, 20);
            ...
            fh_tofd_multihit_coinc[j] = new TH2F(strName9, strName10, 45, 0., 45., 20, 0, 20);
        ...
        cTofd_planes->cd(3);
        gPad->SetLogz();
        fh_tofd_multihit[0]->SetAxisRange(1, 20, "Y");
        ...
        cTofd_planes->cd(5);
        gPad->SetLogz();
        fh_tofd_multihit_coinc[0]->SetAxisRange(1, 20, "Y");
        ...
        cTofd_planes->cd(9);
        gPad->SetLogz();
        fh_tofd_multihit[1]->SetAxisRange(1, 20, "Y");
        ...
        cTofd_planes->cd(11);
        gPad->SetLogz();
        fh_tofd_multihit_coinc[1]->SetAxisRange(1, 20, "Y");
        ...
        cTofd_planes->cd(15);
        gPad->SetLogz();
        fh_tofd_multihit[2]->SetAxisRange(1, 20, "Y");
        ...
        cTofd_planes->cd(17);
        gPad->SetLogz();
        fh_tofd_multihit_coinc[2]->SetAxisRange(1, 20, "Y");
        ...
        cTofd_planes->cd(21);
        gPad->SetLogz();
        fh_tofd_multihit[3]->SetAxisRange(1, 20, "Y");
        ...
        cTofd_planes->cd(23);
        gPad->SetLogz();
        fh_tofd_multihit_coinc[3]->SetAxisRange(1, 20, "Y");
    ...
    if (fHitItems.at(DET_TOFD))
    {
        ...
        for (Int_t j = 0; j < 4; j++)
        {
            ...
            fh_tofd_Tot_hit[j] = new TH2F(strName3, strName4, 200, -50, 50, 3000, 0., 300.);
            fh_tofd_Tot_hit[j]->GetXaxis()->SetTitle("x / cm");
            ...
            if (j < 3)
            {
                ...
                fh_tofd_dt_hit[j] = new TH2F(strName11, strName12, 200, -50, 50, 2000, -1000., 1000);
                char strName9[255];
                sprintf(strName9, "x plane %d / cm", j + 1);
...
void R3BOnlineSpectraToFD_S494::Reset_TOFD_Histo()
{
    ...
    if (fHitItems.at(DET_TOFD))
    {
        ...
        for (int i = 0; i < N_PLANE_MAX_TOFD - 1; i++)
        {
...
void R3BOnlineSpectraToFD_S494::Exec(Option_t* option)
{
    ...
    if (fCalItems.at(DET_TOFD))
    {
        ...
        UInt_t vmultihits[N_PLANE_MAX_TOFD + 1][N_PADDLE_MAX_TOFD];
        UInt_t vmultihits_top[N_PLANE_MAX_TOFD + 1][N_PADDLE_MAX_TOFD];
        UInt_t vmultihits_bot[N_PLANE_MAX_TOFD + 1][N_PADDLE_MAX_TOFD];
        Double_t time_bar[N_PLANE_MAX_TOFD + 1][N_PADDLE_MAX_TOFD][100];
        ...
        {
            for (Int_t ibr = 1; ibr < N_PADDLE_MAX_TOFD + 1; ibr++)
            {
        ...
        for (auto it = bar_map.begin(); bar_map.end() != it; ++it)
        {
            ...
            for (; topc_i < topc_vec.size() && botc_i < botc_vec.size();)
            {
                ...
                {
                    auto topc_trig = (R3BTofdCalData const*)fCalTriggerItems->At(topc_trig_i);
                    auto botc_trig = (R3BTofdCalData const*)fCalTriggerItems->At(botc_trig_i);
    ...
    if (fHitItems.at(DET_TOFD))
    {
        ...
        Double_t x[N_PLANE_MAX_TOFD][10], y[N_PLANE_MAX_TOFD][10], t[N_PLANE_MAX_TOFD][10], q[N_PLANE_MAX_TOFD][10];
        for (Int_t i = 0; i < N_PLANE_MAX_TOFD; i++)
        {
        ...
        Int_t nMulti[N_PLANE_MAX_TOFD] = { 0 }, iCounts[N_PLANE_MAX_TOFD] = { 0 };
        for (Int_t ihit = 0; ihit < nHits; ihit++)
        {
            ...
            fh_tofd_Tot_hit[iPlane - 1]->Fill(x[iPlane - 1][ictemp], q[iPlane - 1][ictemp]);
            iCounts[iPlane - 1] += 1;
            nMulti[iPlane - 1] += 1;
        }
            ...
            if (i > 0)
            {
                for (Int_t im1 = 0; im1 < iCounts[i]; im1++)
                {
                    for (Int_t im2 = 0; im2 < iCounts[i - 1]; im2++)
                    {
                        ...
                        fh_tofd_dt_hit[i - 1]->Fill(x[i][im1], tdif);
                    }
                }
            }
        }
