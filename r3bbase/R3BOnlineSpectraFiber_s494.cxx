...
R3BOnlineSpectraFiber_s494::~R3BOnlineSpectraFiber_s494()
{
    ...
    for (int i = 0; i < NOF_FIB_DET; i++)
    {
        ...
        if(fh_channels_single_Fib[i]) delete fh_channels_single_Fib[i];
        if(fh_Fib_pos[i]) delete fh_Fib_pos[i];
        if(fh_Fib_vs_Events[i]) delete fh_Fib_vs_Events[i];
        if(fh_ToTmax_Fibmax[i])  delete fh_ToTmax_Fibmax[i];
        if(fh_chan_corell[i]) delete fh_chan_corell[i];
        if(fh_raw_tot_up[i]) delete fh_raw_tot_up[i];
        if(fh_raw_tot_down[i]) delete fh_raw_tot_down[i];
...
InitStatus R3BOnlineSpectraFiber_s494::Init()
{
 ...
 //       }
   ...
   // Hit level
            ...
            // ToT MAPMT:
            fh_ToT_m_Fib[ifibcount] = new TH2F(Form("%sHit_tot_up", detName),
                                               Form("%sHit ToT of up", detName),
                                               N_FIBER_PLOT,
                                               0.,
                                               N_FIBER_PLOT,
                                               400,
            ...
            // ToT SAPMT:
            fh_ToT_s_Fib[ifibcount] = new TH2F(Form("%sHit_tot_down", detName),
                                               Form("%sHit ToT of down", detName),
                                               N_FIBER_PLOT,
                                               0.,
                                               N_FIBER_PLOT,
                                               400,
            ...
            fh_Fib_pos[ifibcount] =
                new TH2F(Form("%sHit_pos", detName), Form("%sHit Not-calibrated position", detName), 2000,-100,100,2000,-100,100);
            ...
            FibCanvas[ifibcount]->cd(4);gPad->SetLogz();
            fh_multihit_s_Fib[ifibcount]->Draw("colz");
            FibCanvas[ifibcount]->cd(5);gPad->SetLogz();
            ...
            FibCanvas[ifibcount]->cd(11);gPad->SetLogz();
            fh_ToT_m_Fib[ifibcount]->Draw("colz");
            FibCanvas[ifibcount]->cd(12);gPad->SetLogz();
            fh_ToT_s_Fib[ifibcount]->Draw("colz");
            FibCanvas[ifibcount]->cd(13);gPad->SetLogz();
            fh_time_Fib[ifibcount]->Draw("colz");
            FibCanvas[ifibcount]->cd(14);gPad->SetLogz();
            fh_Fib_vs_Events[ifibcount]->Draw("colz");
            FibCanvas[ifibcount]->cd(15);gPad->SetLogz();
            fh_Fib_pos[ifibcount]->Draw("colz");
            FibCanvas[ifibcount]->cd(16);gPad->SetLogz();
   ...
   for (Int_t ifibcount = 0; ifibcount < NOF_FIB_DET; ifibcount++)
    {
		...
		Double_t totMax;
		Double_t tofMax;
		Double_t tleading_up=0., tleading_down=0.;
        ...
        if (detCal)
        {
            ...
            {
                R3BBunchedFiberCalData* hit = (R3BBunchedFiberCalData*)detCal->At(ical);
                if (!hit)
                    continue;
                ...
                if (iSide == 1 && hit->IsTrailing())
                {
					tot_up = fmod(hit->GetTime_ns()- tleading_up + c_period + c_period / 2, c_period) - c_period / 2;
					fh_raw_tot_up[ifibcount]->Fill(iCha,tot_up);
				}

                if (iSide == 0 && hit->IsLeading())
                {
					iCha_down = iCha;
					tleading_down = hit->GetTime_ns();
                ...
                if (iSide == 0 && hit->IsTrailing())
                {
					tot_down = fmod(hit->GetTime_ns()- tleading_down + c_period + c_period / 2, c_period) - c_period / 2;
					fh_raw_tot_down[ifibcount]->Fill(iCha,tot_down);
				}
            ...
            for (int i = 0; i < 512; ++i)
            {
                ...
                if (s > 0) {fh_multihit_s_Fib[ifibcount]->Fill(i + 1, s); }// multihit of a given down killom channel 
            }
     }       
