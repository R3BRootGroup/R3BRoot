...
#ifndef R3BONLINESPECTRALOSENGRUN2019
#define R3BONLINESPECTRALOSENGRUN2019
 ...
 * for online checks.
 */
class R3BOnlineSpectraLosEngRun2019 : public FairTask
{
  ...
  public:
     ...
     */
    R3BOnlineSpectraLosEngRun2019(const char* name, Int_t iVerbose = 1);

    /**
     * Destructor.
     * Frees the memory used by the object.
     */
    virtual ~R3BOnlineSpectraLosEngRun2019();
	...
	fh_los_tres_MCFD_cond.resize(nDets);
	fh_los_tres_TAMEX.resize(nDets);
	fh_los_tot_mean.resize(nDets);
	fh_los_tot_mean_cond.resize(nDets);
	fh_los_tot.resize(nDets);
	fh_los_tot_cond.resize(nDets);
	fh_los_multihit.resize(nDets);
	fh_xlos_xfib.resize(nDets);
	fh_ylos_yfib.resize(nDets);
	...
	fh_los_pos_MCFD_cond.resize(nDets);
	fh_los_pos_TAMEX.resize(nDets);
	fh_los_pos_ToT.resize(nDets);
	fh_los_pos_ToT_cond.resize(nDets);
...
#define NOF_FIB_DET (DET_FI_LAST - DET_FI_FIRST + 1)
    ...
    Double_t n_fiber[NOF_FIB_DET]={256.,256.};    
    Double_t tof_fiber[NOF_FIB_DET]={2580.,2580.};        
 ...
 //   TClonesArray *fbmonMappedItems;
    ...
    Double_t pile[8]={110,85,85,95,92,120,110,120};
    ...
    std::vector<TH1F *> fh_los_tres_MCFD_cond;
    std::vector<TH1F *> fh_los_tres_TAMEX;
    std::vector<TH1F *> fh_los_tot_mean;
    std::vector<TH1F *> fh_los_tot_mean_cond;
    std::vector<TH2F *> fh_los_tot;
    std::vector<TH2F *> fh_los_tot_cond;
    std::vector<TH2F *> fh_xlos_xfib;
    std::vector<TH2F *> fh_ylos_yfib;
    ...
    std::vector<TH2F *> fh_los_pos_MCFD_cond;
    std::vector<TH2F *> fh_los_pos_TAMEX;
    std::vector<TH2F *> fh_los_pos_ToT;
    std::vector<TH2F *> fh_los_pos_ToT_cond;
  ...
  public:
    ClassDef(R3BOnlineSpectraLosEngRun2019, 2)
};
