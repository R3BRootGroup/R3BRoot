...
R3BOnlineSpectraS494::R3BOnlineSpectraS494()
    : R3BOnlineSpectraS494("OnlineSpectra", 1)
{
}

R3BOnlineSpectraS494::R3BOnlineSpectraS494(const char* name, Int_t iVerbose)
...
InitStatus R3BOnlineSpectraS494::Init()
{
    ...
    for (Int_t ifibcount = 0; ifibcount < NOF_FIB_DET; ifibcount++)
    {
        ...
        if (fMappedItems.at(DET_FI_FIRST + ifibcount))
        {
            ...
            fh_multihit_s_Fib[ifibcount] = new TH2F(
                Form("%s_multihit_s", detName), Form("%s multihits single PMT", detName), 512, 0., 512., 20, 0., 20.);
...
void R3BOnlineSpectraS494::Reset_LOS_Histo()
{
...
void R3BOnlineSpectraS494::Reset_BMON_Histo()
{
    fhTrigger->Reset();
    fhTpat->Reset();
...
void R3BOnlineSpectraS494::Reset_SCI8_Histo()
{
...
void R3BOnlineSpectraS494::Reset_TOFD_Histo()
{
...
void R3BOnlineSpectraS494::Reset_FIBERS_Histo()
{
    if (fMappedItems.at(DET_PSPX))
    {
        for (UInt_t i = 0; i < N_PSPX; i++)
...
void R3BOnlineSpectraS494::Exec(Option_t* option)
{
    ...
    //                    fh_los_dt_hits_ToT->Fill(
    //                        (header->GetTimeStamp() - time_V_mem + timeLosM[iPart] - time_first) / 1000.,
    //                        totsum[iPart]);
    ...
    for (Int_t ifibcount = 0; ifibcount < NOF_FIB_DET; ifibcount++)
    {
        ...
        if (detMapped)
        {
            ...
            for (Int_t ihit = 0; ihit < nHits; ihit++)
            {
                R3BFiberMAPMTMappedData* hit = (R3BFiberMAPMTMappedData*)detMapped->At(ihit);
                if (!hit)
                    continue;
    ...
    //        else
    //        {
    //            fh_pspx_cal_energy_frontback[i]->Fill(
    //                -energy_front[i], -energy_back[i]); // s444: psp 2 is read out with Munich cards, inverts polarity
    //        }
    //    }
    //}
...
void R3BOnlineSpectraS494::FinishEvent()
{
...
void R3BOnlineSpectraS494::FinishTask()
{
