...
*/ 
	...
	// define histograms	
		...
		fh1_RawTofFromS2 =
        new TH1D("CalTofNs_m1_Sci2_Los", "CalTofNs_m1_Sci2_Los", 10000, 0, 1000);
        ...
        fh1_Mushit_z_cond = new TH1F("fh1_Music_charge_z_cond", "Music: Charge Z cond on Los", 3000, 10, 70); //60 * 20., 0, 60);
        ...
        cPID->cd(5);gPad->SetLogz();
        fh2_Zcorel->Draw("colz");
        cPID->cd(6);gPad->SetLogz();
        fh2_ZvsBeta_m1->Draw("colz");
        cPID->cd(7);gPad->SetLogz();
        fh2_ZvsBeta1_m1->Draw("colz");
        cPID->cd(8);gPad->SetLogz();
        fh2_ZvsAoQ_m1->Draw("colz");
        cPID->cd(9);gPad->SetLogz();
        fh2_AoQ_vs_xLOS->Draw("colz");
        cPID->cd(10);gPad->SetLogz();
        fh2_AoQ_vs_xLOS_corr->Draw("colz");
        cPID->cd(11);gPad->SetLogz();
        fh2_ZmusicvsAoQ_m1->Draw("colz");
        cPID->cd(12);gPad->SetLogz();
        fh2_ZvsAoQ_test->Draw("colz");
        cPID->cd(13);gPad->SetLogz();
        fh2_ZvsX_m1->Draw("colz");
        cPID->cd(14);gPad->SetLogz();
        fh2_ZvsY_m1->Draw("colz");
        cPID->cd(15);gPad->SetLogz();
        fh2_Brho_vs_xLOS->Draw("colz");
        run->AddObject(cPID);

       run->GetHttpServer()->RegisterCommand("Reset_PID",
                                              Form("/Tasks/%s/->Reset_PID()", GetName()));
 ...
 */    
    ...
    */
    ...
    if (fTcalItems.at(DET_SCI2) && fTcalItems.at(DET_SCI2)->GetEntriesFast())
    {
        ...
        if(multTcal[0] == 1 && multTcal[1] == 1) sci_mult1 = true;;
    }
   ...
   //----------------------------------------------------------------------
    ...
    if (fHitItems.at(DET_LOS) && fHitItems.at(DET_LOS)->GetEntriesFast() > 0 && fHitItems.at(DET_LOS)->GetEntriesFast() < 32)  // fHitItems.at(DET_LOS) && fHitItems.at(DET_LOS)->GetEntriesFast() > 0 &&
    {		
   ...
   //     cout<<"ICH FAND NPARTLOS: "<<nPartLos <<endl;
        ...
        for (Int_t iPart = 0; iPart < nPartLos; iPart++)
        {
            ...
            R3BLosHitData* hitData = (R3BLosHitData*)detLos->At(iPart);
               ...
               if(totLos[iDet][iPart] > 0. && timeLos[iDet][iPart] > 0.) nLosNonZero += 1;
               
      //  if(totLos[iDet][iPart] > 0) cout<<"LOS input "<<fNEvents<<", "<<nPartLos<<", "<<iPart<<"; "<<iDet<<"; "<<timeLos[iDet][iPart]<<", "<<totLos[iDet][iPart]<<endl;
