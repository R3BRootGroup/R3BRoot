...
void R3BSci8Mapped2Cal::Exec(Option_t* option)
{
	...
	for (Int_t ihit = 0; ihit < nHits; ihit++)  // nHits = Nchannel_Sci8 * NTypes = 2 * 3
	{
		...
		if (times_raw_ns < 0. || times_raw_ns > fClockFreq || IS_NAN(times_raw_ns) )
		{
			...
			LOG(INFO) << 
				"R3BSci8Mapped2Cal::Exec : Bad time in ns: det= " << iDet << 
				", ch= " << iCha << 
				", type= "<< iType <<
		...
		for (iCal=0;iCal<fNofCalItems;iCal++)
		{
			...
			if(iType == 0) {
				...
				if(Tdev < SCI8_COINC_WINDOW_NS) SCI8_COINC = true;	
			}	
			if(iType == 1 ) {
				SCI8_COINC_WINDOW_NS = SCI8_COINC_WINDOW_T_NS;   
                Tdev = fabs(aCalItem->GetMeanTimeTAMEXL()-times_ns);
                if(Tdev < SCI8_COINC_WINDOW_NS && aCalItem->GetTAMEXLNcha() > 0 ) SCI8_COINC = true;
                if(IS_NAN(Tdev) && aCalItem->GetTAMEXLNcha() == 0) SCI8_COINC = true; // First Tamex leading time
            }
			if(iType == 2) {
				SCI8_COINC_WINDOW_NS = SCI8_COINC_WINDOW_T_NS;   
                Tdev = fabs(aCalItem->GetMeanTimeTAMEXT()-times_ns);               
                if(Tdev < SCI8_COINC_WINDOW_NS && aCalItem->GetTAMEXTNcha() > 0 ) SCI8_COINC = true;
                if(IS_NAN(Tdev) && aCalItem->GetTAMEXTNcha() == 0) SCI8_COINC = true; // First Tamex trailing time
            }            
		...
		continue;
skip_event_pileup:
				LOG(WARNING) << "R3BSci8Mapped2Cal::Exec : " << fNEvent
					<< " iCha: " << iCha 
					<< " iType: " << iType
					<< " iCal: " << iCal
