...
TGraph createLookupGraph(std::function<Double_t(Double_t)> distribution, const Double_t lower_bound, const Double_t upper_bound, const Int_t samples = 1000){
...
TGraph createLookupGraph(const TH1& distribution, const Double_t lower_bound, const Double_t upper_bound){
    ...
    Double_t integral = 0;
    integralGraph.SetPoint(integralGraph.GetN(), lower_bound, 0);
    Int_t startbin = distribution.GetXaxis()->FindBin(lower_bound), endbin = distribution.GetXaxis()->FindBin(upper_bound);

    if(distribution.GetBinContent(startbin) < 0 || distribution.GetBinContent(endbin) < 0)
        throw std::underflow_error("Found negative value inside data!");

    //handle first bin --> might be "splitted"
    integral += (distribution.GetXaxis()->GetBinUpEdge(startbin) - lower_bound)/distribution.GetXaxis()->GetBinWidth(startbin)*distribution.GetBinContent(startbin);
    ...
    integral += (upper_bound - distribution.GetXaxis()->GetBinLowEdge(endbin))/distribution.GetXaxis()->GetBinWidth(endbin)*distribution.GetBinContent(endbin);
...
TGraph createLogLogLookupGraph(const TGraph& distribution, const Double_t lower_bound, const Double_t upper_bound, const Int_t samples = 1000){
...
R3BDistribution<Dim> R3BDistribution1D::Delta(const Double_t value){
    return R3BDistribution<Dim>({value});
}
...
R3BDistribution<Dim> R3BDistribution1D::Gaussian(const Double_t mean, const Double_t sigma){
    ...
    return R3BDistribution<Dim>([g](Arr values) -> Arr { 
            return {g.Eval(values[0])};
        });
}

R3BDistribution<Dim> R3BDistribution1D::Function(const std::function<Double_t(const Double_t)> func, const Double_t lower_bound, const Double_t upper_bound){
    auto g = createLookupGraph([func] (Double_t val) {return func(val);}, lower_bound, upper_bound);
    return R3BDistribution<1>([g](Arr values) -> Arr { 
        return {g.Eval(values[0])};
    });
}
...
R3BDistribution<Dim> R3BDistribution1D::Data(const TH1& data){
    return R3BDistribution1D::Data(data, data.GetXaxis()->GetBinLowEdge(1), data.GetXaxis()->GetBinUpEdge(data.GetNbinsX()));
}
...
R3BDistribution<Dim> R3BDistribution1D::DataLogLog(const TGraph& data, const Double_t lower_bound, const Double_t upper_bound){
    Double_t xmin, xmax, ymin, ymax;
    data.ComputeRange(xmin, ymin, xmax, ymax);
    if(lower_bound < xmin || upper_bound > xmax)
        throw std::range_error(std::string(__func__) + " : bounds outsie of data-range");
