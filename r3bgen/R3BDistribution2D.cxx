...
TGraph createLookupGraph(std::function<Double_t(Double_t)> distribution, const Double_t lowerBound, const Double_t upperBound, const Int_t samples = 1000);
...
R3BDistribution<Dim> R3BDistribution2D::Rectangle(const Arr center, const Arr edgeLengths){
    return R3BDistribution<Dim>([center, edgeLengths](const Arr values) -> Arr {
        return {center[0] + edgeLengths[0] * (values[0]-0.5),
                center[1] + edgeLengths[1] * (values[1]-0.5)};
    });
}
