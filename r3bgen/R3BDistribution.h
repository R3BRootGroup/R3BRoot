...
public:
    R3BDistribution() : fLookupFunction([] (const Array vals) { return vals;}) {CreateValues();} // Required by Root Dict
    ...
    R3BDistribution(const std::function<Array(const Array)> lookupFunction) : fLookupFunction(lookupFunction){CreateValues();}
