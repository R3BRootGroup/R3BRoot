...
R3BFi63DigitizerCal::R3BFi63DigitizerCal()
    : FairTask("R3B Fi63 Digitization scheme ", 1)
    , fFi63Points(NULL)
{
}

R3BFi63DigitizerCal::R3BFi63DigitizerCal(const char* name, Int_t iVerbose)
...
InitStatus R3BFi63DigitizerCal::Init()
{
    ...
    fFi63Cals = new TClonesArray("R3BFiberMAPMTCalData", 1000);
    ioman->Register("Fi63Cal", "Digital response in Fi63", fFi63Cals, kTRUE);

    fFi63TriggerCals = new TClonesArray("R3BFiberMAPMTCalData", 1000);
    ioman->Register("Fi63TriggerCal", "Digital response in Fi63", fFi63TriggerCals, kTRUE);
...
void R3BFi63DigitizerCal::Exec(Option_t* opt)
{
    Reset();
                ...
                if (energyl > 0.0001)
                {
                    ...
                    /*
                      ...
                      if (i > 255)
                          ichanSA = 2;
                      */
                    // cout << "Fi63, Fiber: " << i << " MAPMT channel: " << ichanMA << " SA channel: " << ichanSA <<
                    // endl;
    ...
    if (fFi63Points)
    {
        Digitize(fFi63Points, fFi63Cals, fFi63TriggerCals, fiber_nbr);
    }
}
// -------------------------------------------------------------------------

void R3BFi63DigitizerCal::Reset()
{
    if (fFi63Cals)
        fFi63Cals->Clear();
    if (fFi63TriggerCals)
        fFi63TriggerCals->Clear();
}

void R3BFi63DigitizerCal::Finish() {}

ClassImp(R3BFi63DigitizerCal)
