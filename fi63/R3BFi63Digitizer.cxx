...
R3BFi63Digitizer::R3BFi63Digitizer()
...
R3BFi63Digitizer::R3BFi63Digitizer(Double_t e, Double_t t, Double_t y)
    : FairTask("R3B Fi63 Digitization scheme ")
    , fFi63Points(NULL)
{
...
void R3BFi63Digitizer::SetEnergyResolution(Double_t e) { esigma = e; }

void R3BFi63Digitizer::SetTimeResolution(Double_t t) { tsigma = t; }

void R3BFi63Digitizer::SetYPositionResolution(Double_t y) { ysigma = y; }

InitStatus R3BFi63Digitizer::Init()
{
    ...
    fFi63Hits = new TClonesArray("R3BBunchedFiberHitData", 1000);
...
void R3BFi63Digitizer::Exec(Option_t* opt)
{
    Reset();
    ...
    auto Digitize = [this](TClonesArray* Points, TClonesArray* Hits, Int_t NumOfFibers) {
        ...
        /*
                ...
                {
                    R3BFibPoint* aTrack = (R3BFibPoint*)fFi63Points->At(l);

                if(aTrack->GetModule()==43 || aTrack->GetModule()==45 || aTrack->GetModule()==52)
           cout<<setprecision(10)<<"From Fi63Points: "<<l<<", "<<aTrack->GetModule()<<", "<<aTrack->GetXIn()<<";
           "<<aTrack->GetXOut()<<endl;
...
void R3BFi63Digitizer::Reset()
{
    if (fFi63Hits)
        fFi63Hits->Clear();
}

void R3BFi63Digitizer::Finish() {}

ClassImp(R3BFi63Digitizer)
