...
R3BFi60DigitizerHit::R3BFi60DigitizerHit()
...
R3BFi60DigitizerHit::R3BFi60DigitizerHit(Double_t e, Double_t t, Double_t x, Double_t y)
    : FairTask("R3B Fi60 Digitization scheme ")
    , fFi60Points(NULL)
{
...
void R3BFi60DigitizerHit::SetEnergyResolution(Double_t e) { esigma = e; }
void R3BFi60DigitizerHit::SetTimeResolution(Double_t t) { tsigma = t; }
void R3BFi60DigitizerHit::SetXPositionResolution(Double_t x) { xsigma = x; }
void R3BFi60DigitizerHit::SetYPositionResolution(Double_t y) { ysigma = y; }

InitStatus R3BFi60DigitizerHit::Init()
{
    ...
    fFi60Hits = new TClonesArray("R3BFiberMAPMTHitData", 1000);
...
void R3BFi60DigitizerHit::Exec(Option_t* opt)
{
    Reset();
    ...
    auto Digitize = [this](TClonesArray* Points, TClonesArray* Hits, Int_t NumOfFibers) {
        ...
        for (Int_t i = 0; i < entryNum; ++i)
        {
            ...
            LOG(debug) << "Point Data fi60 - x: " << (data_element->GetXIn() + data_element->GetXOut()) / 2.
        ...
        for (TempHit& Hit : TempHits)
        {
            ...
            LOG(debug) << "Hit Fi60 in: fiber: " << Hit.fiberID << " x: " << Hit.X << " y: " << Hit.Y << " z: " << Hit.Z
        ...
        for (Int_t i = 0; i < NumOfFibers; ++i)
        {
            for (Double_t& energyl : energy[i])
            {
                if (energyl > 1.e-12)
                {
                    ...
                    // Double_t xx = -detector_width / 2. + fiber_thickness / 2. + double(fiber_id - 1) *
                    // fiber_thickness;

                    // cout<<setprecision(10) << "Hit Fi60 local-x from fiberNum: " << xx<<" from transf: " << x_local
                    ...
                    // cout<<setprecision(10)<< "Hit Fi60 global-dx,dy,dz: "<<posGlobalcheck.X()-posGlobal.X()<<",
...
void R3BFi60DigitizerHit::Reset()
{
    if (fFi60Hits)
        fFi60Hits->Clear();
}

void R3BFi60DigitizerHit::Finish() {}

ClassImp(R3BFi60DigitizerHit)
