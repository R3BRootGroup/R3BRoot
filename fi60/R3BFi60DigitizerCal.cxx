...
R3BFi60DigitizerCal::R3BFi60DigitizerCal()
    : FairTask("R3B Fi60 Digitization scheme ", 1)
    , fFi60Points(NULL)
{
}

R3BFi60DigitizerCal::R3BFi60DigitizerCal(const char* name, Int_t iVerbose)
...
InitStatus R3BFi60DigitizerCal::Init()
{
    ...
    fFi60Cals = new TClonesArray("R3BFiberMAPMTCalData", 1000);
    ioman->Register("Fi60Cal", "Digital response in Fi60", fFi60Cals, kTRUE);

    fFi60TriggerCals = new TClonesArray("R3BFiberMAPMTCalData", 1000);
    ioman->Register("Fi60TriggerCal", "Digital response in Fi60", fFi60TriggerCals, kTRUE);
...
void R3BFi60DigitizerCal::Exec(Option_t* opt)
{
    Reset();
                ...
                if (energyl > 0.0001)
                {
                    ...
                    /*
                      ...
                      if (i > 255)
                          ichanSA = 2;
                      */
                    // cout << "Fi60, Fiber: " << i << " MAPMT channel: " << ichanMA << " SA channel: " << ichanSA <<
                    // endl;
    ...
    if (fFi60Points)
    {
        Digitize(fFi60Points, fFi60Cals, fFi60TriggerCals, fiber_nbr);
    }
}
// -------------------------------------------------------------------------

void R3BFi60DigitizerCal::Reset()
{
    if (fFi60Cals)
        fFi60Cals->Clear();
    if (fFi60TriggerCals)
        fFi60TriggerCals->Clear();
}

void R3BFi60DigitizerCal::Finish() {}

ClassImp(R3BFi60DigitizerCal)
