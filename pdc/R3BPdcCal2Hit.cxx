...
void R3BPdcCal2Hit::Exec(Option_t* option)
{
    if (0 == fnEvents % 10000)
    {
        std::cout << "\rEvents: " << fnEvents << " / " << maxevent << " (" 
			<< (int)(fnEvents * 100. / maxevent) << " %) " << std::flush;
	}
	for (auto it = fPlaneArray.begin(); fPlaneArray.end() != it; ++it)
	{
		auto &ch_arr = *it;
    ...
    std::vector<R3BPdcCalData const*> trig_table(
		LENGTH(EXT_STR_h101_PDC_onion::PDC_P));
    ...
    for (size_t j = 0; j < cal_num; ++j)
    {
        ...
        if (cur_cal->GetEdgeId() == 2)
        {
            ...
            auto lead_ns = fmod(lead->GetTime_ns() - cur_cal_trig_ns + c_period + c_period / 2, c_period) - c_period / 2;
...
*/			           
    ...
    for (auto it = fPlaneArray.begin(); fPlaneArray.end() != it; ++it)
    {
		...
		for (auto it2 = ch_arr.begin(); ch_arr.end() != it2; ++it2)
		{
			...
			for (auto it_pdc_tot = channel.tot_list.begin();
				channel.tot_list.end() != it_pdc_tot;
				++it_pdc_tot) // over ihit(fiber)
			{
				...
				//cout << "Hit level ID: " << ID << " x: " << x << " y: " << y << " ToT: " << tot_pdc << " t: " << t_pdc << endl;
				new ((*fHitItems)[fNofHitItems++])
				R3BPdcHitData(t_pdc, x, y, tot_pdc, ID, wire);
			}
		}
    }
