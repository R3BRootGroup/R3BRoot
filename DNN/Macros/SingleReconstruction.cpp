...
void SingleReconstruction(Int_t const TotalNumberOfThreads = 1)
{
    ...
    if (Inputs->ContainsNoErrors()==kTRUE)
    {
        ...
        run->SetOutputFile(SingleFile);
        ConnectParFileToRuntimeDb(ParFile,run->GetRuntimeDb());
  
        // Create the Reconstruction translator:
        R3BSingleReconstructor* SingleReco = new R3BSingleReconstructor();
        SingleReco->LinkInputClass(Inputs);
        SingleReco->SetNevents(nEvents);
        
        // Add it to the Mother FairTask:
        run->AddTask(SingleReco);
