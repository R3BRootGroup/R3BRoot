void GenerateTrainingInput()
{
    ...
    OutputPath = OutputPath + "/DNN_Training/";
    Inputs->ModifyString("TheOutputPath",OutputPath);
    
    Inputs->ModifyInteger("ParticleGun_StartEventReading_SnDataFiles",0);
    Inputs->ModifyInteger("ParticleGun_EndEventReading_SnDataFiles",8999);
    Inputs->ModifyBoolian("NeuLAND_TranslateToSignals_Mark_TruePrimarySignals",kTRUE);
    Inputs->ModifyBoolian("NeuLAND_DNNTextFile_ValidationMode",kFALSE);
    ...
    if (Inputs->GetInputString("NeuLAND_DNNTextFile_NormalizationMethod")=="Elena")
    {
        ...
        Inputs->ModifyBoolian("ParticleGun_Uniformize_Detected_Multiplicity",kTRUE);
    }
