TString RoundOff(Double_t const our_input, Int_t const significance)
{
    ...
    while (test == kFALSE)
    {
        ...
        low = TMath::Power(10.0,(Int_t) k);
        up = TMath::Power(10.0,(Int_t) (k+1));
        if ((input>=low)&&(input<up)) {exp = k; test = kTRUE;}
        
        // check negative exponents:
        up = TMath::Power(10.0,-1.0*((Int_t) k));
        low = TMath::Power(10.0,-1.0*((Int_t) (k+1)));
        if ((input>=low)&&(input<up)) {exp = -(k+1); test = kTRUE;}
