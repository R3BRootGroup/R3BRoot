...
C                                                                       
  100 CONTINUE                        
      ...
      WRITE(*,*) 'VSOI = ',2*(F3/AI)/(THISR*(1.0+F3)**2)
      WRITE(*,*) 'VSOR = ',2*(F1/AR)/(THISR*(1.0+F1)**2)
      U(MK+1)=U(MK+1)-WS*F3*4/(1.0+F3)**2
      U(MK+1)=U(MK+1)-VSOI*2*(F3/AI)/(THISR*(1.0+F3)**2)
      U(MK  )=U(MK  )-WR*F1*4/(1.0+F1)**2
      U(MK  )=U(MK  )-VSOR*2*(F1/AR)/(THISR*(1.0+F1)**2)
...
C     IF(DABS(VSOR)+DABS(VSOI).EQ.0.0) GO TO 2009                       
      IF(0.0.EQ.0.0) GO TO 2009
C     The above line is in place of the commented one so that all use of VSOR & VSOI is eliminated. Then we can use these parameters!
      NX=NX+3                                                           
