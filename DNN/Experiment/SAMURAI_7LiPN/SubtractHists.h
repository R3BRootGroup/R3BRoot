TH1D* CrossSections_7LiPN::SubtractHists(TString const NewHistName, TH1D* SignalHist, TH1D* BackHist, Double_t const BackScale, Double_t const LowerBoundary, Double_t const UpperBoundary, Bool_t const DoSubtraction)
{
    ...
    if (Test==kTRUE)
    {
        ...
        for (Int_t ThisBin = 1; ThisBin<(Nbins_New+1); ++ThisBin)
        {
            ...
            {
                BinContent = SignalContent;
                BinError = TMath::Sqrt(SignalContent);
            }
