TString CrossSections_7LiPN::RoundOff(Double_t const our_input, Int_t const significance)
{
    ...
    while (test == kFALSE)
    {
        ...
        low = TMath::Power(10.0,(Int_t) Thisk);
        up = TMath::Power(10.0,(Int_t) (Thisk+1));
        if ((input>=low)&&(input<up)) {exp = Thisk; test = kTRUE;}
        
        // check negative exponents:
        up = TMath::Power(10.0,-1.0*((Int_t) Thisk));
        low = TMath::Power(10.0,-1.0*((Int_t) (Thisk+1)));
        if ((input>=low)&&(input<up)) {exp = -(Thisk+1); test = kTRUE;}
        
        // increase k:
        Thisk = Thisk+1;
        
        // terminate the loop in case of a really big or small number:
        if (Thisk>100) {test = kTRUE;}
    }
    
    // in case the loop terminated:
    if ((Thisk>100)&&(input<1.0)) {return "0";}
    if ((Thisk>100)&&(input>1.0))
    {
    ...
    // in case we have more then 1 digit:
    if (significance>1)
    {
        Answer = Answer + ".";
