...
TCanvas* DrawHistDouma2(TH1D* const Raw, TString const Title, TString const xlabel, TString const ylabel, Int_t const Color, Bool_t const UseLogScale, Double_t const ScaleFactor, Double_t const max)
{
  ...
  Raw->SetAxisRange(-0.1,1.8,"x");
  
  Raw->SetMaximum(1.2*max);
