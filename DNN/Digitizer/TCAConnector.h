...
class TCAInputConnector
{
  ...
  public:
    ...
    void Init()
    {
        ...
        fTCA = (TClonesArray*)ioman->GetObject(fBranchName+OutputNameTag);
        if (fTCA == nullptr)
        {
...
class TCAOutputConnector
{
  ...
  public:
    ...
    void Init()
    {
        ...
        fTCA = ioman->Register(fBranchName+OutputNameTag, fClassName, "", kTRUE);
    }
    ...
    void Store(std::vector<T>& v)
    {
        ...
        fTCA->Clear("C"); // NOTE: C is supposed to solve memory leak problem. Else, just make it empty again.
