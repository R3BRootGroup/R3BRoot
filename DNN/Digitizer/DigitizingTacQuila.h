...
#include "Validated_CAD.h"
    ...
    class DigitizingTacQuila : public DigitizingEngine
    {
        struct Params
        {
            ...
            Bool_t PrintParameters;          // Decides whether the other parameters will be printed yes/no (very slow operation!)
      ...
      public:
        class PMT : public DigitizingEngine::PMT
        {
          private:
            ...
            mutable Validated_CAD<std::vector<PMTHit>::const_iterator> cachedFirstHitOverThresh;
            mutable Validated_CAD<Double_t> cachedQDC;
            mutable Validated_CAD<Double_t> cachedTDC;
            mutable Validated_CAD<Double_t> cachedEnergy;
            const Params& par;
      ...
      public:
        // Override functions for all 9 digitizer parameters:  
        void SetPaddleHalfLength(const Double_t v) override { par.fPaddleHalfLength = v; }
        void SetPMTThreshold(const Double_t v) override { par.fPMTThresh = v; }
        void SetSaturationCoefficient(const Double_t v) override { par.fSaturationCoefficient = v; }
        void SetTimeRes(const Double_t v) override { par.fTimeRes = v; }
        void SetERes(const Double_t v) override { par.fEResRel = v; }
        void SetIntegrationTime(const Double_t v) override { par.fIntegrationTime = v; }
        void SetcMedium(const Double_t v) override { par.fcMedium = v; }
        void SetAttenuation(const Double_t v) override { par.fAttenuation = v; }
        void SetLambda(const Double_t v) override { par.fLambda = v; }
        void SetBatchMode() override {par.PrintParameters = kFALSE;}
        void SetPrintMode() override {par.PrintParameters = kTRUE;}
        void ReInitializeRandomSeed() override {par.fRnd = new TRandom3(0);}
