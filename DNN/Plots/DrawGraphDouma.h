...
TCanvas* R3B_DNN_Plots::DrawGraphDouma(TGraph* const Raw, TString const Title, TString const xlabel, TString const ylabel, Int_t const Color, Bool_t const UseLogScale, Double_t const Ymini, Double_t const Ymaxi)
{
  ...
  {
      if (Ymin<=0.0) {Ymin = 0.3;}
      if (Ymax<=0.0) {Ymax = 1.0;}
      if (Ymax<=Ymin) {Ymax = 2.0*Ymin;}
      Ylength = Ymax - Ymin;
  ...
  // Draw the graph:
  c->cd(); // To select our newly created canvas.
  Raw->Draw("APL*"); // To actually draw.
  //text->Draw("same");
  //text2->Draw("same");
