...
TCanvas* R3B_DNN_Plots::DrawHistDouma(TH1D* const Raw, TString const Title, TString const xlabel, TString const ylabel, Int_t const Color, Bool_t const UseLogScale)
{
  ...
  // Double_t max = Raw->GetMaximum(); // Y-axis
  
  // Calculate the max on the z-scale ourselves:
  Int_t nbinsX = Raw->GetXaxis()->GetNbins();
  Double_t bincontent = 0.0;
  Double_t max = 0.0;
  
  for (Int_t kx = 1; kx<(nbinsX+1); ++kx)
  {
      bincontent = Raw->GetBinContent(kx);
      if (bincontent>max) {max = bincontent;}
  }  
  
  if (max == 0.0) {max = 0.06;}
  // if (max == 0.0) {max = 1.0;}
  Raw->SetMaximum(1.25*max);
  Raw->SetMinimum(-0.05*max); // must be positive for log-scale!
  if (UseLogScale==kTRUE) {Raw->SetMinimum(LogScaleMinimum);}
