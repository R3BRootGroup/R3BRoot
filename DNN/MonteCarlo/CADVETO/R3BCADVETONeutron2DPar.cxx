...
R3BCADVETONeutron2DPar::R3BCADVETONeutron2DPar(const char* name, const char* title, const char* context)
    : FairParGenericSet(name, title, context)
    , fNeutronCuts(nullptr)
{
}

R3BCADVETONeutron2DPar::~R3BCADVETONeutron2DPar()
{
    // Note: Deleting stuff here or in clear() causes segfaults?
}

void R3BCADVETONeutron2DPar::clear() {}

void R3BCADVETONeutron2DPar::putParams(FairParamList* l)
{
    if (!l)
    {
        return;
    }
    l->addObject("VETONeutronCuts", fNeutronCuts);
}

Bool_t R3BCADVETONeutron2DPar::getParams(FairParamList* l)
{
    if (!l)
    {
        return kFALSE;
    }
    if (!l->fillObject("VETONeutronCuts", fNeutronCuts))
    {
        return kFALSE;
    }
    return kTRUE;
...
void R3BCADVETONeutron2DPar::printParams()
{
    std::cout << "R3BCADVETONeutron2DPar: VETO Neutron Cuts ..." << std::endl;
...
void R3BCADVETONeutron2DPar::SetNeutronCuts(const std::map<UInt_t, TCutG*>& cuts)
{
...
std::map<UInt_t, TCutG*> R3BCADVETONeutron2DPar::GetNeutronCuts() const
{
    if (fNeutronCuts == nullptr)
    {
        throw std::runtime_error("R3BCADVETONeutron2DPar: NeutronCuts not set!");
    }
...
TCutG* R3BCADVETONeutron2DPar::GetNeutronCut(const Int_t n) const { return GetNeutronCuts().at(n); }

UInt_t R3BCADVETONeutron2DPar::GetNeutronMultiplicity(const Double_t energy, const Double_t nClusters) const
{
...
ClassImp(R3BCADVETONeutron2DPar);
