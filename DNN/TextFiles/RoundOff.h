TString R3BTextFileGenerator::RoundOff(Double_t const our_input, Int_t const significance)
{
    ...
    if ((kk>100)&&(input<1.0)) 
    {
        ...
        if (significance>2)
        {
            for (Int_t k = 2; k<significance; ++k) {stt = stt + "0";}
        }
        stt = stt + "e0";
        return stt;
    }
    ...
    // Failsafe:
    if (Answer.Length()>(3*significance)) {Answer = "0.00e0";}
