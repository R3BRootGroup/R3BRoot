void R3BTextFileGenerator::ProcessOneEvent_SingleCluster()
{
    // This function does filling of output neurons and writing to textfile
    // all at once for a single event. This is, because multiple writings
    // per event are required (a new one for each cluster).
    
    // Set proper flags:
    CutOff_ClusterVector = kTRUE;
    ...
    Int_t N_DesiredInputs = TheScorers->GetNumberOfClusterScores() + 4;
    Int_t N_DesiredOutputs = nOutputFlags_PerCluster;
    ...
    else
    {
        ...
        if (Preselection_AllowEvent==kTRUE)
        {
            ...
            // And then re-shuffle the order, if required:
            ...
            // Eliminate the events if we lost primary clusters:
            ...
            {
                // Then, for all clusters left in the vector, we must fill/write to the TextFile.
                
                // So loop over the clusters:
                for (Int_t kclus = 0; kclus<TheClusters.size(); ++kclus)
                {
                    // Begin by resetting:
                    InputNeurons.clear();
                    OutputNeurons.clear();
                    
                    // Then, fill in the cluster:
                    FillOneCluster(TheClusters.at(kclus).TheCluster);
                    
                    // Then, write to the textfiles:
                    WriteEvent();
