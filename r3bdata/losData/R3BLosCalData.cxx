...
UInt_t R3BLosCalData::GetDetector() const
{
  return fDetector;
}    
    ...
    time_width, Double_t c_range)
{
 ...
 //std::cout<<"SET TIME START: "<<i<<", "<<icha<<", "<<", "<<time_ns<<", "<<c_range<<", "<<fTime_ns[icha][i]<<std::endl;
  ...
  if (fFirst < 0) {
    ...
    std::cout<<"Nothing yet set; start new item with time for channel "<<icha<<", type "<<i<<" time: "<<time_ns<<std::endl;
  } else {
    auto fFirst_time_ns = GetTimeDiff(fFirst, time_ns, c_range);
    auto time_ns_fLast = GetTimeDiff(time_ns, fLast, c_range);
