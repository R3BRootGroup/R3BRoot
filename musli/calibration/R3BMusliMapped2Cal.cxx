...
void R3BMusliMapped2Cal::Exec(Option_t* option)
{
    ...
    if (fNumGroupsAnodes <= fNumSignals)
    {
        ...
        {
            if (map_mult[i] == 0 || map_mult[i] >= (fMaxMult - 1))
            {
                continue;
            }
            ...
            {
                if (fUseMultHit && 0 < map_mult[16] && map_mult[16] < fMaxMult)
                {
                ...
                } // end of muti-hit analysis called
            }     // end of else multi hits
        }         // end of if (fNumGroups <= fNumAnodes)
