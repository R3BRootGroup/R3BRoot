...
Bool_t R3BStartrackReader::Init(ext_data_struct_info *a_struct_info)
{
	...
	EXT_STR_h101_STAR_ITEMS_INFO(ok, *a_struct_info, fOffset,
	    EXT_STR_h101_STAR_t, 0);
		...
		if(starN){ 
		  ...
		  for (int i=0;i<starN;i=i+2){ 
		    ...
		    if( ((starWord1 >> 30) == 0x2) && (((starWord1 & 0xFFFFFFFF) != 0xFFFFFFFF)))
		    {
		      ...
		      if(info_code == 4 || info_code == 7) fts_hb=info_field;  //  high bit (47:28) time from startrack hit strip
		      if(info_code == 5 || info_code == 8) fts_vhb=info_field; //  very high bit (63:48)time from startrack hit strip
	  	      if(info_code == 14) tsExt_hb=info_field;  // high bit (47:28) time from the external Master
		      if(info_code == 15) tsExt_vhb=info_field; // very high bit (63:48)time from the external Master
		      ...
		      if(info_code == 14 || info_code == 15)
			{
		      ...
		      //if(tsExt_vhb && tsExt_hb && tsExt_lb ) TSExt= ((uint64_t) tsExt_vhb << 47) | ((uint64_t) tsExt_hb << 28 ) | tsExt_lb; // Reconstructed full timestamp for Ext signal
		      if(tsExt_vhb && tsExt_hb && tsExt_lb ) TSExt= ((uint64_t) tsExt_hb << 28 ) | tsExt_lb; // Reconstructed timestamp from MSB (most significant bits) and sLSB (slightly lower significant bits) for a startrack signal

		    // Case of wordtype=3:
		    }else if(((starWord1 >> 30) & 0x3) == 0x3 && ( (starWord1 & 0xFFFFFFFF) != 0xFFFFFFFF ))  // = case of wordtype=3
		      {
			...
			UInt_t ts_lb_part2_inv=  (~(ts_lb_part2)) & 0x00000FFF; // invert and take only the first 12 inverted bits;
			...
			//if(ts_vhb && ts_hb && ts_lb) TS= ((uint64_t) ts_vhb << 47) | ((uint64_t) ts_hb << 28) |  ts_lb ; // Reconstructed full timestamp for a startrack signal
			if(fts_hb && ts_lb) TS= ((uint64_t) fts_hb << 28) |  ts_lb ; // Reconstructed timestamp from MSB (most significant bits) and sLSB (slightly lower significant bits) for a startrack signal
		      }
		    ...
		    if(hitbit==1){
		      //new ((*fArray)[fArray->GetEntriesFast()]) R3BStartrackMappedData(WRvhb, WRhb, WRlb, wordtype, hitbit, ladder_id, side, asic_id, strip_id, adcData, ts_vhb, ts_hb, ts_lb, tsExt_vhb, tsExt_hb, tsExt_lb,info_field, info_code); // 
		      new ((*fArray)[fArray->GetEntriesFast()]) R3BStartrackMappedData(TS, TSExt, wordtype, hitbit, ladder_id, side, asic_id, strip_id, adcData, fts_vhb, fts_hb, ts_lb, tsExt_vhb, tsExt_hb, tsExt_lb,info_field, info_code); // 
		    }
