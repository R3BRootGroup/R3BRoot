...
R3BPWReader::R3BPWReader(EXT_STR_h101_PW* data, size_t offset)
    ...
    , fData(data)
    , fArrayPW(new TClonesArray("R3BPWMappedData"))
    , fOnline(kFALSE)
    , fOffset(offset)
    , fNEvent(0)
{
...
Bool_t R3BPWReader::Init(ext_data_struct_info* a_struct_info)
{
    Int_t ok;
    R3BLOG(info, "");
    EXT_STR_h101_PW_ITEMS_INFO(ok, *a_struct_info, fOffset, EXT_STR_h101_PW, 0);
    if (!ok)
    {
    ...
    FairRootManager::Instance()->Register("R3BPWMappedData", "PW Mapped Data", fArrayPW, !fOnline);
    Reset();
