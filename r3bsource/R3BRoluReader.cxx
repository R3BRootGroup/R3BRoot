...
Bool_t R3BRoluReader::Init(ext_data_struct_info *a_struct_info)
{
  ...
  EXT_STR_h101_ROLU_ITEMS_INFO(ok, *a_struct_info, fOffset,
      EXT_STR_h101_ROLU, 0);
  if (!ok) {
...
{   
  ...
  for (uint32_t d = 0; d < NUM_ROLU_DETECTORS; d++) {
   ...
   if(data->ROLU[d].TTCL < 1) return kFALSE;
...
//   cout<<" **** NEW EVENT **** "<<endl;
 ...
 //  cout<<"NUM DATA: "<<data->ROLU[d].TTCL<<"; NUM CHANNELS: "<<data->ROLU[d].TTCLM<<endl;
   ...
   //           cout<<"TAMEX trailing: "<< channel<<", "<<data->ROLU[d].TTFTv[j]<<", "<< coarse_trailing<<endl;    
