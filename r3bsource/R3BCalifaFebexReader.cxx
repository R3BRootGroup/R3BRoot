...
Bool_t R3BCalifaFebexReader::Read()
{
    ...
    for (int crystal = 0; crystal < fData->CALIFA_ENE; ++crystal)
    {
        ...
        uint32_t ov = fData->CALIFA_OVv[crystal];
        uint16_t pu = fData->CALIFA_PILEUPv[crystal];
        uint16_t dc = fData->CALIFA_DISCARDv[crystal];

        uint16_t tot = fData->CALIFA_TOTv[crystal];
    ...
    {
        UShort_t channelNumber = fData->CALIFA_TRGI[i];
        int16_t energy = fData->CALIFA_TRGv[i];
