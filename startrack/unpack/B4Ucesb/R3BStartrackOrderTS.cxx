...
InitStatus R3BStartrackOrderTS::Init()
{
	...
	output_Tree->Branch("StartrackOrderTS", &s_entry.tm_stp,"tm_stp/L:tm_stp_ext/L:nhit/I:type/I:hit/I:ModuleId/I:Side/I:AsicId/I:StripId/I:ADCdata/I:sync_flag/O:pause_flag/O");
...
void R3BStartrackOrderTS::Exec(Option_t *option)
{
	...
	Info4=false; // Set Info 4 to false at the beginning of each block reading so the first word type 3 (adc) are ignored until code=4 is found
	...
	for(Int_t i=0; i<nItems; i++)  
	{
		...
		if(my_wordtype==3)my_det_id=item->GetModuleId();
      
		if(my_wordtype==3 && !Info4)fCount_rejectA++;

		if(my_wordtype==3 && Info4){ //ie:  my_wordtype=11   and  with the condition Info4=true, we ignore data before the first info_code=4 (or 7) appeared

		  if(Info4) Word3=true;  //  Info4 is already true (ie: a word of type 2 has appeared already in the datastream) and now we found a word of type 3 so we put Word3=true


		  difflb= (item->GetTimelb()) - tm_stp_code47_lb; // calculate the difference of adc low-bit ts and the last low-bit ts from info_code 4/5 or 7/8. 
		  
		  if( abs(difflb) >= 1.e8 ){ 
		    // abs(difflb) >= 1.e8: Sometime (not sure why?), in between 2 sets of word of type 2 (ie:code4/5 or 7/8), the first ADC word (type 3) has a low-bit time stamp which is close to the max range 
                    // of the low-bit time stamp (28 bit) suggesting that this adc word should be recombined with the previous high-bit ts (ie: bit 20 to 48).  
		    // So here we check that the difference is not the same order of magnitude than the low-bit range (28 bits <=> 2^28=268435456)
		    // if it is we ignore that adc word.  
		    //cout << "!!!!!!!!!!!! hit: " <<  fTotalHits++ << " " << item->GetTimelb()  << " " << tm_stp_code47_lb << " " << diff << endl;
		    ts_sort_flag=false;
		    fCount_rejectB++;
		  }else
		  {
		    ...
		    my_tm_stp_lb=(item->GetTimelb()); // this includes the factor 5 as Vic already * by 5 before sending to MBS		  
		    ...
		    cout << "my_tm_stp from lmd before divided by 5=" << my_tm_stp << endl;
		      ...
		      if((  (my_tm_stp_vhb_cor<< 48 ) | ((my_tm_stp_hb_cor<< 28 ) | (my_tm_stp_lb_cor)) ) *5. - my_tm_stp <0) fCount_disorder++;
		      //cout << "diff my_tm_stp final=" << (  (my_tm_stp_vhb_cor<< 48 ) | ((my_tm_stp_hb_cor<< 28 ) | (my_tm_stp_lb_cor)) ) *5. - my_tm_stp << endl;
		      ...
		      if(Info4) fTotalHits++; // We start counting after Info4 is true
	 ...
	 //cout << "Info4=" << Info4 <<  "  ts_sort_flag=" << ts_sort_flag << "  high bit ts=" << my_tm_stp_hb << endl;
		...
		//
 	        ...
 	        if(my_wordtype==2 && !ts_sort_flag){        // binary: 10
			...
			if(my_info_code==14){
			  ...
			  //cout << "my_tm_stp_ext   = " << my_tm_stp_ext << endl;
			    ...
			    if((i+2)==nItems) // if it is the word2 (14) before the last word2 of the block, we'll sort data in the next loop 
			    {
			      ts_sort_flag= true;
			    }			  
			...
			if(my_info_code==4 || my_info_code==5 || my_info_code==7 || my_info_code==8){
			  ...
			  if(my_info_code==4 || my_info_code==7)my_tm_stp_hb= my_info_field ;  //  47:28 timestamp (if both 4/5 and 7/8)
			  //if(/*my_info_code==4 ||*/ my_info_code==7)my_tm_stp_hb= my_info_field ;  //  47:28 timestamp (if only 7&8)
			  ...
			  if(my_info_code==4 || my_info_code==5)  // Info 4/5 occurs periodically (every 2^17*5ns)
			      ...
			      if(my_info_code==4)  // Note: there should always info_code=5 after an info_code=4
				{
				  ...
				  cout << " new reconstructed time = " <<  ((my_tm_stp_vhb<< 48 ) | ((my_info_field<< 28 ) | (tm_stp_code47_lb)) )/5  << endl;
				  cout << " word2 previuosly reconstructed time = " <<   word2_rec_time << endl;

			          if(fTotalHits && ((my_tm_stp_vhb<< 48 ) | (( (my_info_field) << 28 ) | (tm_stp_code47_lb)) )/5  !=0) 
				    { 
                                      diff45=((my_tm_stp_vhb<< 48 ) | (( (my_info_field) << 28 ) | (tm_stp_code47_lb)))/5 - word2_rec_time;
				      if(word2_rec_time) ts_code47_diff.push_back(diff45) ; // this should be = 5ns * 2tothe17


				      word2_rec_time=((my_tm_stp_vhb<< 48 ) | (( (my_info_field)<< 28 ) | (tm_stp_code47_lb)) )/5; 
				  ...
				  if(!FirstSort_flag || (my_info_field!=my_tm_stp_hb))
				    ...
				    } else if((i+2)==nItems) // if it is the word before last in the block, we'll sort data in the next word
				    {
				      ts_sort_flag= true;
				    }
				  ...
				  if(my_info_field!=my_tm_stp_vhb)  // if The very high bit time stamp part has increased 
				    {
				      ...
				      //TS_4_5_diff->Fill( ((my_tm_stp_vhb<< 48 ) | ((my_tm_stp_hb<< 28 ) | (my_tm_stp_lb)) )/1e9 ); // in sec !!!! if the 5ns already implemented in Vic's readout
				      my_tm_stp_vhb= my_info_field;  //  63:48
				      //cout << "my_tm_stp_vhb  after  = " << my_tm_stp_vhb << endl;
				      

				      //TS_4_5->Fill( fTotalHits, ((my_tm_stp_vhb<< 48 ) | ((my_tm_stp_hb<< 28 ) | (my_tm_stp_lb)) )); // in sec !!!! if the 5ns already implemented in Vic's readout
			    ...
			    }else if( (my_info_code==7 || my_info_code==8) )   // Info 7/8 occurs when high bit timestamp is found not ordered !

			    {
				  ...
				  if(fTotalHits && ((my_tm_stp_vhb<< 48 ) | ((my_info_field<< 28 ) | (tm_stp_code47_lb)) )/5  !=0) 
				    { 
                                      diff78=((my_tm_stp_vhb<< 48 ) | (( (my_info_field) << 28 ) | (tm_stp_code47_lb)))/5 - word2_rec_time;
				      if(word2_rec_time) ts_code47_diff.push_back( diff78 ) ; // this should be = 5ns * 2tothe17
				  ...
				  if( !FirstSort_flag || (my_info_field !=my_tm_stp_hb))  // if The very high bit time stamp part has increased 
				  {

				    FirstSort_flag=true; // 

				    cout << " 7 tm_stp_msb_modules[my_det_id] (hb) before = " << tm_stp_msb_modules[my_det_id] << endl;
				    my_tm_stp_hb= my_info_field ;  //  47:28 timestamp	    	      
				    my_tm_stp_msb=  my_tm_stp_hb;	     
				    tm_stp_msb_modules[my_det_id]= my_tm_stp_msb;
				    cout << "7 tm_stp_msb_modules[my_det_id] (hb) after= " << tm_stp_msb_modules[my_det_id] << endl;
				    ...
				    }else if((i+2)==nItems) // if it is the word before last in the block, we'll sort data in the next loop
				      {
					ts_sort_flag= true;
				      }
		...
		cout << "Info4=" << Info4 <<  "  ts_sort_flag=" << ts_sort_flag << "  high bit ts=" << my_tm_stp_hb << endl;
		 ...
		 //if( (my_wordtype==2 && ts_sort_flag) /*|| (my_wordtype==3 && ts_sort_flag)*/ ) {  
		  // We add the condition: "(my_wordtype==3 && ts_sort_flag)" in case the block finishes during a sequence of word3 instead of word2
		  ...
		  for(long long j=0; j<n_loop; j++){	    //transport sorted data to output_tree
		    ...
		    cout << "my_new Type=" <<  item->GetWordType() /*branchWordType->GetValue(index_hit_temp.at(j),0,true)*/ << endl;     // when calling InsertionSort2
		    
		    
		    my_new_wordtype=item->GetWordType(); //branchWordType->GetValue(index_hit_temp.at(j),0,true);
		    my_new_hitbit=item->GetHitBit(); // branchHitValue->GetValue(index_hit_temp.at(j),0,true);     // when calling InsertionSort2
		    my_new_det_id=item->GetModuleId(); // branchModuleId->GetValue(index_hit_temp.at(j),0,true);     // when calling InsertionSort2
		    my_new_side_id=item->GetSide(); // branchEnergy->GetValue(index_hit_temp.at(j),0,true);  // when calling InsertionSort2
		    my_new_asic_id=item->GetAsicId(); // branchAsicId->GetValue(index_hit_temp.at(j),0,true);   // when calling InsertionSort2
		    my_new_ch_id= item->GetStripId(); // branchStripId->GetValue(index_hit_temp.at(j),0,true);    // when calling InsertionSort2
		    my_new_adc_data=item->GetADCdata(); // branchEnergy->GetValue(index_hit_temp.at(j),0,true);    // when calling InsertionSort2
...
void R3BStartrackOrderTS::FinishTask()
{
  ...
  cout << " " << endl;
  cout << "Number of time the timestamp was found smaller than the previous one: " << fCount_disorder << ". So: "<< fCount_disorder*100/fTotalHits << "% of accepted hits"<< endl;
  cout << "Total hits found in Si Tracker:" << fTotalHits+fCount_rejectA+fCount_rejectB << endl;
  cout << "Hits rejected because no high-bit ts information available at beginning of the block: " << fCount_rejectA << " -> " << fCount_rejectA*100./(fTotalHits+fCount_rejectA+fCount_rejectB) << "%"<< endl;
  cout << "Hits rejected because of suspected missing blocks (suspicious jump of high-bit ts values by more than 1 unit): " << fCount_rejectB << " -> " << fCount_rejectB*100./(fTotalHits+fCount_rejectA+fCount_rejectB) << "%"<< endl;
  cout << "Accepted hits: " << fTotalHits << " -> " << fTotalHits*100/(fTotalHits+fCount_rejectA+fCount_rejectB) << "%" << endl;
  cout << " " << endl;
...
void R3BStartrackOrderTS::InsertionSort2(vector<long long> & v_ts, vector<long long> & v_block_index, vector<long long> & v_hit_index){
