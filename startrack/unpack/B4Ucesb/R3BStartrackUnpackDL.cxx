...
Bool_t R3BStartrackUnpack::DoUnpack(Int_t* data, Int_t size) // used for Mbs format datafile
{
    ...
    while (l_s < size)
    {
        ...
        // LOG(debug) << "R3BStartrackerUnpack : Strip_ID IS " << strip_id << ",  Chip ID IS " << asic_id << " , Ladder
        // ID IS " << module_id << " , ADC Data IS " << adcData; new ((*fRawData)[fNHits]) R3BStartrackRawHit(module_id,
        // side, asic_id, strip_id, adcData, lclock);

        // new ((*fRawData)[fNHits]) R3BStartrackRawHit(wordtype, hitbit, module_id, side, asic_id, strip_id, adcData,
        // lclock); new ((*fRawData)[fNHits]) R3BStartrackRawHit(wordtype, hitbit, module_id, side, asic_id, strip_id,
        // adcData, ts2);
...
Bool_t R3BStartrackUnpack::DoUnpack2(Int_t* data_word0, Int_t* data_word1, Int_t size)
{
    ...
    while (l_s < size)
    {
        ...
        // LOG(info) << " --------- event "
        LOG(debug) << " --------- event "
                   << "        hitbit " << hitbit << "        Channel_id " << channel_id << "        ASIC_id "
                   << asic_id << "        Side " << side << "        Module_id " << module_id << "        energy "
        ...
        // timestamp); new ((*fRawData)[fNHits]) R3BStartrackRawHit(wordtype, hitbit, module_id, side, asic_id,
        // channel_id, energy, timestamp, info_field, info_code); fNHits++;
