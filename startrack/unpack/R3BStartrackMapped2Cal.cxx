...
void R3BStartrackMapped2Cal::Exec(Option_t* option)
{
  ...
  if(fCalItemsCA)fCalItemsCA->Delete();
    ...
    nRawHits = fMappedItemsCA->GetEntries(); // same as: Int_t nHits = fMappedItemsCA->GetEntriesFast(); = total number if words (type 2 & 3)
...
// Consider only Word of type 3 and remove hits at the beaginning of the run that have no high bits time stamp information tag. 
	    ...
	    if((ihit+1)==nRawHits){      // last hit in the block <=> ts_sort_flag= true; since TS is already fully reconstructed
		  
		  Int_t n_loop= index_hit_temp.size(); 
		  
		  //cout << "Number of hits of wordtype==3 in this block (=n_loop)=" << n_loop << endl;	          // n_loop = number of hits of wordtype==3 in the block
		  //cout << "Total number of hits of wordtype==3 in the run so far (=fN_first)=" << fN_first << endl;	  // fN_first = total number of hits of wordtype==3 in the run
		  ...
		  ts_sort_flag= false;
   	         ...
   	         for(Int_t j=0; j<n_loop; j++){	    //transport sorted data to output_tree
		    ...
		    if(j==0)cout <<" previous block last TS was =" << fW3TSlast << endl;


		      if( j>0 && ((my_new_ts - my_prev_ts)>1e8))  // To get rid of hit with probably wrongly reconstructed time stamp 
			{  
			  // ignore this hit as time reconstruction probably wrong !! 
			  //cout<< " Ignoring this hit:" << nHits <<  " because difference with previous ts is suspiciously huge:" << my_new_ts - my_prev_ts << endl;
			  nHits--;
			  NbReject++;
			  fNbReject++;
			...
			}else
			{
			  ...
			  if( ( (Int_t)(my_new_ts -  LastBlockLastTS)<0 ) )
			    {
			      //cout<< " Ignoring this hit:" << nHits <<  " because difference with previous bloc ts is negatif:" << (Int_t)(my_new_ts - LastBlockLastTS)  << endl;
			      if(j>0)my_prev_ts=ts_temp.at(j-1);
			      nHits--;
			      NbReject++;
			      fNbReject++;
			    ...
			    }else
			    {
			      ...
			      new ((*fCalItemsCA)[fNofCalItems]) R3BStartrackCalData(my_new_ts, my_new_ts_ext , my_new_ladderId, my_new_side, my_new_asicId, my_new_stripId, my_new_energy, nHits);
			      
			      fNofCalItems++;
			      fW3TSlast=ts_temp.at(j); // record last ts in the block for continuity check with TS at the beginning of the next block.
			    }
	    ...
	    cout << "Rate of StarTrack hit rejections in the run : " << (Double_t)(fNbReject*100./(fN_first+fNbReject))  << "%"<< endl;
...
void R3BStartrackMapped2Cal::InsertionSort2(vector<long long> & v_ts, vector<long long> & v_block_index, vector<long long> & v_hit_index){
