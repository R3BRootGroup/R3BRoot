...
// -------------------------------------------------------------------------
// -----                R3BStartrackCal2Hit is a substitute to         -----
// -----                     R3BStartrackDigit source file             -----
// -----                  Created 29/06/17  by M.Labiche               -----
// -----                                                               ----
...
R3BStartrackCal2Hit::R3BStartrackCal2Hit() : FairTask("R3B STaRTracker Hit Raw Sim data ") { 
...
void R3BStartrackCal2Hit::SetParContainers() {
...
// -----   Public method Init   --------------------------------------------
InitStatus R3BStartrackCal2Hit::Init() {
...
InitStatus R3BStartrackCal2Hit::ReInit() {
...
// -----   Public method Exec   --------------------------------------------
void R3BStartrackCal2Hit::Exec(Option_t* opt) {
		...
		for(Int_t i=0;i<traHitsPerEvent;i++){
			...
			if(Detector<=6){  // = inner layer
				...
				//ProjStrip= (-Length1/2-(StepZ1/2)*(2*strip+1)) - SlopA*(-WidthMax1/2);  // 1st strip proj at z=0
			        ShiftalongZ= ( Xlab1*M_Inner[0][2][0] 
					       + Ylab1*M_Inner[0][2][1] 
					       + Zlab1*M_Inner[0][2][2] 
					       +     1*M_Inner[0][2][3])-0.07; 
				// -0.07 to have 172 strips at the narrow end of the ladder //+ 0.03; // shift along z axis (-1.35855= z lab coordinate of the center of the detector after inverse transformation, 0.03 is an extra shift thought to be coinciding with the middle line of a strip: to be checked !! ).
			...
			if(Detector>=7 && Detector<=18){  // = Middle layer
				...
				ShiftalongZ= ( Xlab2*M_Mid[0][2][0] 
					       ...
					       + Zlab2*M_Mid[0][2][2] 
					       +     1*M_Mid[0][2][3])-0.07; 
// -0.07 to have 172 strips at the narrow end of the ladder //+ 0.03; // shift along z axis (z lab coordinate of the center of the detector after inverse transformation, 0.03 is an extra shift thought to be coinciding with the middle line of a strip: to be checked !! ).
...
// ---- Public method Reset   --------------------------------------------------
void R3BStartrackCal2Hit::Reset(){
	// Clear the CA structure
	// cout << " -I- R3BStartrackCal2Hit:Reset() called " << endl;
	
if (fStartrackHitCA) fStartrackHitCA->Clear();
}   


// ---- Public method Finish   --------------------------------------------------
void R3BStartrackCal2Hit::Finish()
{
	// here event. write histos
	cout << "" << endl;
	cout << " -I- Cal2Hit Finish() called " << endl;
...
// -----  Public method SetExperimentalResolution  ----------------------------------
 void R3BStartrackCal2Hit::SetExperimentalResolution(Double_t trackerRes)
{	
	fTrackerResolution=trackerRes;
    cout << "-I- R3BStartrackCal2Hit::SetExperimentalResolution to " << fTrackerResolution << " GeV" << endl;
}


// -----  Public method SetDetectionThreshold  ----------------------------------
void R3BStartrackCal2Hit::SetDetectionThreshold(Double_t thresholdEne)
{	
	fThreshold=thresholdEne;
	cout << "-I- R3BStartrackCal2Hit::SetDetectionThreshold to " << fThreshold << " GeV" << endl;
}


// -----   Private method ExpResSmearing  --------------------------------------------
Double_t R3BStartrackCal2Hit::ExpResSmearing(Double_t inputEnergy) {
...
//      R3BStartrackerDigitHit* R3BStartrackCal2Hit::AddHit(Double_t ene,Int_t det,Int_t Stripfrt, Int_t Stripbck, Double_t time){

        //  Use R3BStartrackerDigitHit to produce a root tree with raw data:    
          R3BStartrackerDigitHit* R3BStartrackCal2Hit::AddHit(Int_t det,Int_t chip, Int_t side, Int_t strip, Double_t ene, Double_t time){   
	//  Use R3BStartrackerHitData to produce a root tree with data ready to be combined with other data detectors:    
        // R3BStartrackerHitData* R3BStartrackCal2Hit::AddHit(Int_t Track, Double_t ene, Double pAngle, Double aAngle, Double_t time){   // TODO !!

	TClonesArray& clref = *fStartrackHitCA;
	Int_t size = clref.GetEntriesFast();

	return new(clref[size]) R3BStartrackerDigitHit(det, chip, side, strip, ene, time);  
	//return new(clref[size]) R3BStartrackerCal2Hit(NbTracks, ene, pAngle, aAngle, time); // TODO

}

ClassImp(R3BStartrackCal2Hit)
