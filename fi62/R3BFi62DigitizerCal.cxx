...
R3BFi62DigitizerCal::R3BFi62DigitizerCal()
    : FairTask("R3B Fi62 Digitization scheme ", 1)
    , fFi62Points(NULL)
{
}

R3BFi62DigitizerCal::R3BFi62DigitizerCal(const char* name, Int_t iVerbose)
...
InitStatus R3BFi62DigitizerCal::Init()
{
    ...
    fFi62Cals = new TClonesArray("R3BFiberMAPMTCalData", 1000);
    ioman->Register("Fi62Cal", "Digital response in Fi62", fFi62Cals, kTRUE);

    fFi62TriggerCals = new TClonesArray("R3BFiberMAPMTCalData", 1000);
    ioman->Register("Fi62TriggerCal", "Digital response in Fi62", fFi62TriggerCals, kTRUE);
...
void R3BFi62DigitizerCal::Exec(Option_t* opt)
{
    Reset();
                ...
                if (energyl > 0.0001)
                {
                    ...
                    /*
                      ...
                      if (i > 255)
                          ichanSA = 2;
                      */
                    // cout << "Fi62, Fiber: " << i << " MAPMT channel: " << ichanMA << " SA channel: " << ichanSA <<
                    // endl;
    ...
    if (fFi62Points)
    {
        Digitize(fFi62Points, fFi62Cals, fFi62TriggerCals, fiber_nbr);
    }
}
// -------------------------------------------------------------------------

void R3BFi62DigitizerCal::Reset()
{
    if (fFi62Cals)
        fFi62Cals->Clear();
    if (fFi62TriggerCals)
        fFi62TriggerCals->Clear();
}

void R3BFi62DigitizerCal::Finish() {}

ClassImp(R3BFi62DigitizerCal)
