...
R3BFi62DigitizerHit::R3BFi62DigitizerHit()
    : FairTask("R3B Fi62 Digitization scheme ")
    , fFi62Points(NULL)
{
...
R3BFi62DigitizerHit::R3BFi62DigitizerHit(Double_t e, Double_t t, Double_t x, Double_t y)
    : FairTask("R3B Fi62 Digitization scheme ")
    , fFi62Points(NULL)
{
...
void R3BFi62DigitizerHit::SetEnergyResolution(Double_t e) { esigma = e; }
void R3BFi62DigitizerHit::SetTimeResolution(Double_t t) { tsigma = t; }
void R3BFi62DigitizerHit::SetXPositionResolution(Double_t x) { xsigma = x; }
void R3BFi62DigitizerHit::SetYPositionResolution(Double_t y) { ysigma = y; }

InitStatus R3BFi62DigitizerHit::Init()
{
    ...
    fFi62Points = (TClonesArray*)ioman->GetObject("Fi62Point");

    fMCTrack = (TClonesArray*)ioman->GetObject("MCTrack");

    // Register output array DchDigi
    fFi62Hits = new TClonesArray("R3BFiberMAPMTHitData", 1000);
    ioman->Register("Fi62Hit", "Digital response in Fi62", fFi62Hits, kTRUE);
...
void R3BFi62DigitizerHit::Exec(Option_t* opt)
{
    Reset();
    ...
    auto Digitize = [this](TClonesArray* Points, TClonesArray* Hits, Int_t NumOfFibers) {
        ...
        for (Int_t i = 0; i < entryNum; ++i)
        {
            ...
            LOG(debug) << "Point Data fi62 - x: " << (data_element->GetXIn() + data_element->GetXOut()) / 2.
        ...
        for (TempHit& Hit : TempHits)
        {
            ...
            LOG(debug) << "Hit Fi62 in: fiber: " << Hit.fiberID << " x: " << Hit.X << " y: " << Hit.Y << " z: " << Hit.Z
        ...
        for (Int_t i = 0; i < NumOfFibers; ++i)
        {
            for (Double_t& energyl : energy[i])
            {
                if (energyl > 1.e-12)
                {
                    ...
                    LOG(debug) << "Hit Fi62 out: fiber: " << i << " x: " << (x[i].at(&energyl - energy[i].data()))
                    ...
                    // cout<<setprecision(10) << "Hit Fi62 local-x from fiberNum: " << xx<<" from transf: " << x_local
    ...
    if (fFi62Points)
    {
        Digitize(fFi62Points, fFi62Hits, fiber_nbr);
    }
}
// -------------------------------------------------------------------------

void R3BFi62DigitizerHit::Reset()
{
    if (fFi62Hits)
        fFi62Hits->Clear();
}

void R3BFi62DigitizerHit::Finish() {}

ClassImp(R3BFi62DigitizerHit)
