...
R3BFi62Digitizer::R3BFi62Digitizer()
...
R3BFi62Digitizer::R3BFi62Digitizer(Double_t e, Double_t t, Double_t y)
    : FairTask("R3B Fi62 Digitization scheme ")
    , fFi62Points(NULL)
{
...
void R3BFi62Digitizer::SetEnergyResolution(Double_t e) { esigma = e; }

void R3BFi62Digitizer::SetTimeResolution(Double_t t) { tsigma = t; }

void R3BFi62Digitizer::SetYPositionResolution(Double_t y) { ysigma = y; }

InitStatus R3BFi62Digitizer::Init()
{
    ...
    fFi62Hits = new TClonesArray("R3BBunchedFiberHitData", 1000);
...
void R3BFi62Digitizer::Exec(Option_t* opt)
{
    Reset();
    ...
    auto Digitize = [this](TClonesArray* Points, TClonesArray* Hits, Int_t NumOfFibers) {
        ...
        /*
                ...
                {
                    R3BFibPoint* aTrack = (R3BFibPoint*)fFi62Points->At(l);

                if(aTrack->GetModule()==43 || aTrack->GetModule()==45 || aTrack->GetModule()==52)
           cout<<setprecision(10)<<"From Fi62Points: "<<l<<", "<<aTrack->GetModule()<<", "<<aTrack->GetXIn()<<";
           "<<aTrack->GetXOut()<<endl;
...
void R3BFi62Digitizer::Reset()
{
    if (fFi62Hits)
        fFi62Hits->Clear();
}

void R3BFi62Digitizer::Finish() {}

ClassImp(R3BFi62Digitizer)
