...
void R3BCalifaUnpack::SetParContainers()
{
    ...
    if (!run)
        LOG(fatal) << "R3BCalifaUnpack::SetParContainers: No analysis run";

    FairRuntimeDb* rtdb = run->GetRuntimeDb();
    if (!rtdb)
        LOG(fatal) << "R3BCalifaUnpack::SetParContainers: No runtime database";
...
Bool_t R3BCalifaUnpack::DoUnpack(Int_t* data, Int_t size)
{
    ...
    check0 = (data[l_s] >> 16) & 0xffff;
    rabbit1 = data[l_s++] & 0xffff;
    check1 = (data[l_s] >> 16) & 0xffff;
    ...
    while (l_s < size)
    {
        ...
        switch (magic)
        {
            ...
            case 0x115A: // new event version: 1 (evsize & magic) + 9 words = 40 bytes
                ...
                if (evsize > 4 * (l_s - start))
                { // there is still traces in the
        ...
        if (overflow & 0x601)
            error |= 1; // 11000000001 Timing not valid
        if (overflow & 0x63e)
            error |= 1 << 1; // 11000111110  Energy not valid
        if (overflow & 0x78E)
            error |= 1 << 2; // 11110001110  PID not valid
        if (num_pileup)
            error |= 1 << 3;
        ...
        UShort_t crystal_id = pc_id * (max_channel * max_card * max_sfp_id) + sfp_id * (max_channel * max_card) +
                              card * max_channel + channel;
