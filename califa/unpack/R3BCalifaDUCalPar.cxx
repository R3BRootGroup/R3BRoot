...
Bool_t R3BCalifaDUCalPar::getParams(FairParamList* list)
{
    ...
    if (!list->fill("comp_id", &fCompId))
    {
        return kFALSE;
    }
    if (!list->fill("detection_unit", &fDetectionUnit))
    {
        return kFALSE;
    }
    if (!list->fill("gammacal_offset", &fGammaCal_offset))
    {
        return kFALSE;
    }
    if (!list->fill("gammacal_gain", &fGammaCal_gain))
    {
        return kFALSE;
    }
    if (!list->fill("totcal_par0", &fToTCal_par0))
    {
        return kFALSE;
    }
    if (!list->fill("totcal_par1", &fToTCal_par1))
    {
        return kFALSE;
    }
    if (!list->fill("totcal_par2", &fToTCal_par2))
    {
        return kFALSE;
    }
    if (!list->fill("rangecal_offset", &fRangeCal_offset))
    {
        return kFALSE;
    }
    if (!list->fill("rangecal_gain", &fRangeCal_gain))
    {
        return kFALSE;
    }
    if (!list->fill("quenching_factor", &fQuenchingFactor))
    {
        return kFALSE;
    }
    if (!list->fill("pid_gain", &fPidGain))
    {
        return kFALSE;
    }
...
void R3BCalifaDUCalPar::clear()
{
    ...
    fGammaCal_offset = fGammaCal_gain = fToTCal_par0 = fToTCal_par1 = fToTCal_par2 = fRangeCal_offset = fRangeCal_gain =
...
void R3BCalifaDUCalPar::Fill(FairDbResultPool& res_in, const FairDbValRecord* valrec)
{
    // Clear all structures
    clear();

    res_in >> fCompId >> fDetectionUnit >> fGammaCal_offset >> fGammaCal_gain >> fToTCal_par0 >> fToTCal_par1 >>
        fToTCal_par2 >> fRangeCal_offset >> fRangeCal_gain >> fQuenchingFactor >> fPidGain;
}

void R3BCalifaDUCalPar::Store(FairDbOutTableBuffer& res_out, const FairDbValRecord* valrec) const
{

    res_out << fCompId << fDetectionUnit << fGammaCal_offset << fGammaCal_gain << fToTCal_par0 << fToTCal_par1
            << fToTCal_par2 << fRangeCal_offset << fRangeCal_gain << fQuenchingFactor << fPidGain;
}
...
void R3BCalifaDUCalPar::Print()
{
    ...
    std::cout << "   fToTCal_par0: " << fToTCal_par0 << "   fToTCal_par1: " << fToTCal_par1
              << "   fToTCal_par2: " << fToTCal_par2 << std::endl;
...
{
    //
    Bool_t test_h = (fCompId == that.fCompId) && (fDetectionUnit == that.fDetectionUnit) &&
                    (fGammaCal_offset == that.fGammaCal_offset) && (fGammaCal_gain == that.fGammaCal_gain) &&
                    (fToTCal_par0 == that.fToTCal_par0) && (fToTCal_par1 == that.fToTCal_par1) &&
                    (fToTCal_par2 == that.fToTCal_par2) && (fRangeCal_offset == that.fRangeCal_offset) &&
                    (fRangeCal_gain == that.fRangeCal_gain) && (fQuenchingFactor == that.fQuenchingFactor) &&
...
FairDbWriter<R3BCalifaDUCalPar>* R3BCalifaDUCalPar::ActivateWriter(Int_t rid)
{
    ...
    else
    {
        ...
        fParam_Writer->Activate(
            GetValInterval(rid), GetComboNo(), GetVersion(), GetDbEntry(), "Califa Detection Unit Calib Pars");
