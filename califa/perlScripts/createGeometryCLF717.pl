...
#!/usr/local/bin/perl
#
# AUTHOR: Hector Alvarez Pol, 10/03/11  (hector.alvarez@usc.es)
# Last modification: 10/03/11
# 
# Program to create a TGeo file with the Forward ENDCAP CALIFA 7.17  (also called Version B)
# Uses the information extracted from the STEP file for the CALIFA 7.17 alveoli
...
#	@crystalLength (in cm): length of each crystal type (there are 23 crystal models)
#   2) run the perl script
#	perl createGeometryCLF717.pl
...
$OUTPUTFILE = 'CLF717_Geometry.geo';
...
print OUTPUTFILE " //Generated ",$date , " from macro createGeometryCLF717.pl by Hector Alvarez Pol (hector.alvarez@usc.es) \n";
print OUTPUTFILE "\n";
...
for ($h = 0; $h<=$#Alv_dx1; $h++) {
	...
	for($rot=0; $rot<32; $rot++) {		
		if($rot==0) { 			
			...
			print OUTPUTFILE "\t *transAlv_",$h+1,"_",$rot," =  (*transAlv_",$h+1,"_",$rot,") * (*transDef",$h+1,") ; \n";
		}
		else {
		...
		print OUTPUTFILE "\t pWorld->AddNode(Alveolus_EC_",$h+1,",",$rot,",transAlv_",$h+1,"_",$rot,"); \n\n";
	}
}		
close(INPUTFILE);
close(INPUTFILE1); 
