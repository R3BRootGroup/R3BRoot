...
void R3BStrawtubesMapped2Cal::Exec(Option_t *option)
{
  ...
  for (Int_t mapped_i = 0; mapped_i < mapped_num; mapped_i++) {
    R3BStrawtubesMappedData *mapped = (R3BStrawtubesMappedData
        *)fMappedItems->At(mapped_i);
    if (!mapped) {
    ...
    R3BTCalModulePar *tcal = fTcalPar->GetModuleParAt(mapped->GetPlane(),
        mapped->GetStraw(), mapped->GetSide());
    if (!tcal) {
      LOG(ERROR) << "R3BStrawtubesMapped2Cal::Exec: Tcal par not found, "
          "mapped=(plane=" << (int)mapped->GetPlane() << ",side=" <<
          (int)mapped->GetSide() << ",straw=" << (int)mapped->GetStraw() <<
          ")" << FairLogger::endl;
      continue;
    }
    ...
    if (time_ns < 0. || time_ns > VFTX2_COARSE_NS) {
      LOG(ERROR) << "R3BStrawtubesMapped2Cal::Exec: Bad fine time, "
          "mapped=(plane=" << mapped->GetPlane() << ",side=" <<
          mapped->GetSide() << ",straw=" << mapped->GetStraw() <<
          ", raw=" << mapped->GetTimeFine() << ", ns=" << time_ns << "." <<
          FairLogger::endl;
      continue;
    }

    time_ns = VFTX2_COARSE_NS - time_ns + mapped->GetTimeCoarse() *
        VFTX2_COARSE_NS;

    R3BStrawtubesCalData *cal = NULL;
    for (int cal_i = 0; cal_i < fCalItems->GetEntriesFast(); ++cal_i) {
      R3BStrawtubesCalData *cal_test = (R3BStrawtubesCalData
          *)fCalItems->At(cal_i);
      // We want the same straw.
      if (cal_test->GetPlane() != mapped->GetPlane() ||
          cal_test->GetStraw() != mapped->GetStraw()) {
        continue;
      }
      // Pile-up galore?
    ...
    if (!cal) {
      cal = new ((*fCalItems)[fNofCalItems])
          R3BStrawtubesCalData(mapped->GetPlane(), mapped->GetStraw());
      ++fNofCalItems;
    }
...
InitStatus R3BStrawtubesMapped2Cal::Init()
{
  if (0 == fTcalPar->GetNumModulePar()) {
    LOG(ERROR) << "There are no TCal parameters in container StrawtubesTCalPar" << FairLogger::endl;
    return kFATAL;
  }
...
void R3BStrawtubesMapped2Cal::SetParContainers()
{
  ...
  if (!fTcalPar) {
    LOG(ERROR) << "No StrawtubesTCalPar container." << FairLogger::endl;
    return;
  }
}
