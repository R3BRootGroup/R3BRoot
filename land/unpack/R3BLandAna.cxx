   ...
   if (fLandDigi) {
      ...
      for (Int_t i = 0; i < nLandDigi; i++) {
	    ...
	    fh_land_tof->Fill(time2);
            if (beta > 29.5 && beta < 30 && qdc > 300) {
	       Double_t normt = time2 - (l - 1300)/C_LIGHT;
	       if(barId < 51){
		if(barId == 25){
...
void R3BLandAna::CreateHistos() {
   ...
   fh_land_timebarid = new TH2F("h_land_timebarid", "Time vs Bar ID", fNofBars, 0.5, (Double_t) fNofBars + 0.5, 4000, 0., 2000.);
   fh_land_tofbarid = new TH2F("h_land_tofbarid", "ToF vs Bar ID", fNofBars, 0.5, (Double_t) fNofBars + 0.5, 1000, 0., 100.);
   fh_land_tof = new TH1F("h_land_tof", "ToF", 1000, 0., 100.);
   fh_land_qdctof = new TH2F("h_land_qdctof", "QDC vs ToF", 2000, -50., 150., 2000, 0., 2000.);
   fh_land_betabarid = new TH2F("h_land_betabarid", "Velocity vs Bar ID", fNofBars, 0.5, (Double_t) fNofBars + 0.5, 500, 0., 50.);
   fh_land_lbarid = new TH2F("h_land_lbarid", "Length vs Bar ID", fNofBars, 0.5, (Double_t) fNofBars + 0.5, 1000, 700., 900.);
   ...
   fh_land_beta = new TH1F("h_land_beta", "Velocity", 1000, -10., 35.);
   ...
   fh_land_norm_tof_total = new TH1F("h_land_norm_tof_total", "norm. ToF (total)", 400, 40., 50.);
}
