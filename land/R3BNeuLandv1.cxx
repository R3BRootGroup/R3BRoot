// -------------------------------------------------------------------------
// -----                        R3BNeuLandv1 source file                 -----
// -----                  Created 26/03/09  by D.Bertini               -----
// -------------------------------------------------------------------------
#include "R3BNeuLandv1.h"
...
// -----   Default constructor   -------------------------------------------
R3BNeuLandv1::R3BNeuLandv1() : R3BDetector("R3BNeuLandv1", kTRUE, kLAND) {
  ResetParameters();
  fLandv1Collection = new TClonesArray("R3BNeuLandPoint");
  fPosIndex = 0;
  kGeoSaved = kFALSE;
  flGeoPar = new TList();
...
R3BNeuLandv1::R3BNeuLandv1(const char* name, Bool_t active) 
...
R3BNeuLandv1::~R3BNeuLandv1() {
  
  if ( flGeoPar ) delete flGeoPar;
  if (fLandv1Collection) {
    fLandv1Collection->Delete();
    delete fLandv1Collection;
  }
}

void R3BNeuLandv1::Initialize()
{
...
Bool_t R3BNeuLandv1::ProcessHits(FairVolume* vol) {
...
//    Double_t trackstep_gas = gMC->TrackStep();
    ...
    // Set additional parameters at exit of active volume. Create R3BNeuLandv1Point.
	...
	if ( fPosIn.Z() < 30. && newpos[2] > 30.02 ) {
	  cerr << "-I- R3BNeuLandv1 : 2nd direction: " << olddirection[0] << "," << olddirection[1] << "," << olddirection[2] 
	       << " with safety = " << safety << endl;
	  cerr << "-I- R3BNeuLandv1 oldpos = " << oldpos[0] << "," << oldpos[1] << "," << oldpos[2] << endl;
	  cerr << "-I- R3BNeuLandv1 newpos = " << newpos[0] << "," << newpos[1] << "," << newpos[2] << endl;
	}
...
void R3BNeuLandv1::EndOfEvent() {
  
  if (fVerboseLevel) Print();
  fLandv1Collection->Clear();
...
void R3BNeuLandv1::Register() {
  FairRootManager::Instance()->Register("NeuLandv1Point", GetName(), fLandv1Collection, kTRUE);
}
// ----------------------------------------------------------------------------
...
TClonesArray* R3BNeuLandv1::GetCollection(Int_t iColl) const {
  if (iColl == 0) return fLandv1Collection;
  else return NULL;
}
// ----------------------------------------------------------------------------
...
void R3BNeuLandv1::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset) {
  Int_t nEntries = cl1->GetEntriesFast();
  cout << "-I- R3BNeuLandv1: " << nEntries << " entries to add." << endl;
  ...
  cout << " -I- R3BNeuLandv1: " << cl2->GetEntriesFast() << " merged entries."
       << endl;
}
...
R3BNeuLandPoint* R3BNeuLandv1::AddHit(Int_t eventID, Int_t trackID,
  ...
  TClonesArray& clref = *fLandv1Collection;
  Int_t size = clref.GetEntriesFast();
  if (fVerboseLevel>1) 
    cout << "-I- R3BNeuLandv1: Adding Point at (" << posIn.X() << ", " << posIn.Y() 
