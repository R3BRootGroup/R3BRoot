...
R3BPspxHitPar::R3BPspxHitPar(const char* name,
    const char* title,
    const char* context)
...
void R3BPspxHitPar::printparams()
{
  ...
  LOG(INFO) << "Detectorno.: No. of Strips, Orientation of Strips, Orientation Sign of X-Strips, Orientation Sign of Y-Strips, Length of Detector" << FairLogger::endl;
  for ( Int_t i=0; i< size; i++) {
    LOG(INFO) << i <<" :" << pspxhitparstrip.GetAt(i) << ", " << pspxhitparorientation.GetAt(i) << ", " << pspxhitparorientationxsign.GetAt(i) << ", " << pspxhitparorientationysign.GetAt(i) << ", "  << pspxhitparlength.GetAt(i) << FairLogger::endl;
  }
...
Bool_t R3BPspxHitPar::getParams(FairParamList* l)
{
  ...
  if (!l) { return kFALSE; }
  if ( ! l->fill("R3BPspxHitDetectors", &pspxhitpardetector) ) { return kFALSE; }
  pspxhitparstrip.Set(pspxhitpardetector);
  pspxhitparorientation.Set(pspxhitpardetector);

  if ( ! l->fill("R3BPspxHitStripsPerDetector", &pspxhitparstrip) ) { return kFALSE; }
  if ( ! l->fill("R3BPspxHitOrientationOfDetector", &pspxhitparorientation) ) { return kFALSE; }
  if ( ! l->fill("R3BPspxHitOrientationXSignOfDetector", &pspxhitparorientationxsign) ) { return kFALSE; }
  if ( ! l->fill("R3BPspxHitOrientationYSignOfDetector", &pspxhitparorientationysign) ) { return kFALSE; }
  if ( ! l->fill("R3BPspxHitLengthOfDetector", &pspxhitparlength) ) { return kFALSE; }
