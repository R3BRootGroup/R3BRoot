...
#include "R3BOnlineSpectraToFD_S522.h"
#include "R3BLogger.h"
...
R3BOnlineSpectraToFD_S522::R3BOnlineSpectraToFD_S522()
    : R3BOnlineSpectraToFD_S522("OnlineSpectraToFD_S522", 1)
{
}

R3BOnlineSpectraToFD_S522::R3BOnlineSpectraToFD_S522(const char* name, Int_t iVerbose)
    : FairTask(name, iVerbose)
    , fTrigger(-1)
    , fTpat1(-1)
    , fTpat2(-1)
    , fCalTriggerItems(NULL)
    , fNofPlanes(N_PLANE_MAX_TOFD_S522)
    , fPaddlesPerPlane(N_PADDLE_MAX_TOFD_S522)
...
InitStatus R3BOnlineSpectraToFD_S522::Init()
{
...
void R3BOnlineSpectraToFD_S522::Reset_TOFD_Histo()
{
    R3BLOG(INFO, "Reset_Histo");
    for (int i = 0; i < N_PLANE_MAX_TOFD_S522; i++)
    {
    ...
    if (fHitItems.at(DET_TOFD))
    {
        ...
        for (int i = 0; i < N_PLANE_MAX_TOFD_S522 - 1; i++)
        {
...
void R3BOnlineSpectraToFD_S522::Exec(Option_t* option)
{
    ...
    if (fMappedItems.at(DET_TOFD))
    {
        ...
        Int_t nsum_top[N_PLANE_MAX_TOFD_S522] = { 0 };
        Int_t nsum_bot[N_PLANE_MAX_TOFD_S522] = { 0 };
        for (Int_t imapped = 0; imapped < nMapped; imapped++)
        {
            ...
            if (iSide == 1 && iEdge == 1 && iPlane < N_PLANE_MAX_TOFD_S522)
                nsum_bot[iPlane - 1] += 1;
            if (iSide == 2 && iEdge == 1 && iPlane < N_PLANE_MAX_TOFD_S522)
                nsum_top[iPlane - 1] += 1;
        ...
        for (Int_t i = 0; i < N_PLANE_MAX_TOFD_S522; i++)
        {
    ...
    if (fCalItems.at(DET_TOFD))
    {
        ...
        UInt_t vmultihits[N_PLANE_MAX_TOFD_S522 + 1][N_PADDLE_MAX_TOFD_S522];
        UInt_t vmultihits_top[N_PLANE_MAX_TOFD_S522 + 1][N_PADDLE_MAX_TOFD_S522];
        UInt_t vmultihits_bot[N_PLANE_MAX_TOFD_S522 + 1][N_PADDLE_MAX_TOFD_S522];
        Double_t time_bar[N_PLANE_MAX_TOFD_S522 + 1][N_PADDLE_MAX_TOFD_S522][100];
        for (Int_t i = 0; i < fNofPlanes + 1; i++)
        {
            for (Int_t j = 0; j < N_PADDLE_MAX_TOFD_S522; j++)
            {
                vmultihits[i][j] = 0;
        ...
        for (Int_t ipl = 0; ipl < N_PLANE_MAX_TOFD_S522; ipl++)
        {
            for (Int_t ibr = 1; ibr < N_PADDLE_MAX_TOFD_S522 + 1; ibr++)
            {
    ...
    if (fHitItems.at(DET_TOFD))
    {
        ...
        Double_t x[N_PLANE_MAX_TOFD_S522][10], y[N_PLANE_MAX_TOFD_S522][10], t[N_PLANE_MAX_TOFD_S522][10],
            q[N_PLANE_MAX_TOFD_S522][10], bar[N_PLANE_MAX_TOFD_S522][10];
        for (Int_t i = 0; i < N_PLANE_MAX_TOFD_S522; i++)
        {
        ...
        Int_t nMulti[N_PLANE_MAX_TOFD_S522] = { 0 }, iCounts[N_PLANE_MAX_TOFD_S522] = { 0 };
        for (Int_t ihit = 0; ihit < nHits; ihit++)
        {
...
void R3BOnlineSpectraToFD_S522::FinishEvent()
{
...
void R3BOnlineSpectraToFD_S522::FinishTask()
{
    if (fCalItems.at(DET_TOFD))
    {
        for (Int_t i = 0; i < N_PLANE_MAX_TOFD_S522; i++)
        ...
        for (Int_t i = 0; i < N_PLANE_MAX_TOFD_S522 - 1; i++)
        {
            fh_tofd_dt[i]->Write();
        }
    }
...
ClassImp(R3BOnlineSpectraToFD_S522);
