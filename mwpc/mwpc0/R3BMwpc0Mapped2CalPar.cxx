...
// ----------------------------------------------------------------
// -----         R3BMwpc0Mapped2CalPar source file            -----
// -----     Created 09/10/19 by J.L. Rodriguez-Sanchez       -----
// ----------------------------------------------------------------

#include "R3BMwpc0Mapped2CalPar.h"
...
// R3BMwpc0Mapped2CalPar: Default Constructor --------------------------
R3BMwpc0Mapped2CalPar::R3BMwpc0Mapped2CalPar()
    : R3BMwpc0Mapped2CalPar("R3BMwpc0Mapped2CalPar", 1)
{
}

// R3BMwpc0Mapped2CalPar: Standard Constructor --------------------------
R3BMwpc0Mapped2CalPar::R3BMwpc0Mapped2CalPar(const char* name, Int_t iVerbose)
...
// R3BMwpc0Mapped2CalPar: Destructor ----------------------------------------
R3BMwpc0Mapped2CalPar::~R3BMwpc0Mapped2CalPar() { LOG(info) << "R3BMwpc0Mapped2CalPar: Delete instance"; }

// -----   Public method Init   --------------------------------------------
InitStatus R3BMwpc0Mapped2CalPar::Init()
{

    LOG(info) << "R3BMwpc0Mapped2CalPar: Init";
    ...
    fPad_Par = dynamic_cast<R3BMwpc0CalPar*>(rtdb->getContainer("mwpc0CalPar"));
    if (!fPad_Par)
    {
        LOG(error) << "R3BMwpc0Mapped2CalPar::Init() Couldn't get handle on mwpc0CalPar container";
        return kFATAL;
    }
    return kSUCCESS;
}
...
InitStatus R3BMwpc0Mapped2CalPar::ReInit() { return kSUCCESS; }

// -----   Public method Exec   --------------------------------------------
void R3BMwpc0Mapped2CalPar::Exec(Option_t* opt)
{
...
void R3BMwpc0Mapped2CalPar::FinishTask()
{
...
//---- Search Pedestals   -------------------------------------------------------
void R3BMwpc0Mapped2CalPar::SearchPedestals()
{

    LOG(info) << "R3BMwpc0Mapped2CalPar: Search pedestals";
    ...
    for (Int_t i = 0; i < fNumPadX + fNumPadY; i++)
    {
        ...
        else
        {
            ...
            if (i < fNumPadX) // plane X
                LOG(warn) << "Histogram NO Fitted in mwpc0, plane 1 and pad " << i + 1;
            else // plane y
                LOG(warn) << "Histogram NO Fitted in mwpc0, plane 3 and pad " << i + 1 - fNumPadX;
        }
    }
    fPad_Par->setChanged();
    return;
...
ClassImp(R3BMwpc0Mapped2CalPar);
