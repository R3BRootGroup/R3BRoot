...
//  TString ParFile = "r3bpar.root";
  ...
  if (fDetList.FindObject("TARGET") ) {
      ...
      run->AddModule(target);   //if I don't want have target in simulation - uncomment this
  }
  ...
  if (fDetList.FindObject("DCH") ) {
      ...
      ((R3BDch*) dch )->SetHeliumBag(kFALSE);  //without helium bag
     // ((R3BDch*) dch )->SetHeliumBag(kTRUE);  /with helium bag
      // Global position of the Module
      phi   =  0.0; // (deg)
      theta =  0.0; // (deg)
      psi   =  0.0; // (deg)
  ...
  if (fFieldMap == 0) {
    ...
    Double_t fMeasCurrent = 1797.;// I_current [A]  number from s318_172.txt file (Pb target)
//    Double_t fMeasCurrent = 1820.;// I_current [A]  number from s318_172.txt file (Pb target) not adjusted
...
//  TString iFile = dir + "/input/" + evtFile;
  ...
  Double_t pdgId=2212; // proton   2112 neutron
  Double_t theta1= 0.0;  // polar angle distribution
  Double_t theta2= 0.5;
  Double_t momentum=1.082474; // 10 GeV/c
