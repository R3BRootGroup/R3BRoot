...
Int_t sim_p2p_s438b(const char *path, int i){
  ...
  // Input files
  TString EventFile = Form("quasi_p%03d.out", i);

  // Output files
  TString OutFile = Form("%s/sim_v17_p2p_realistic2_p%03d.root", path, i);
  TString ParFile = Form("%s/testpar_v17_p2p_realistic2_p%03d.root", path, i);
  ...
  detGeo.Add(new TObjString("TARGET"),
  	     new TObjString("target_CH2Target_s438.geo.root"));
  ...
  detGeo.Add(new TObjString("CALIFA"),        
	     new TObjString("califa_17_v8.11_cc0.2.geo.root"));
//            new TObjString("califa_demo.geo.root"));
...
//           new TObjString("startra_v15a.geo.root"));
  //detGeo.Add(new TObjString("STaRTrack"),
  ...
  // 	     new TObjString("vacvessel_v14a.geo.root"));
//detGeo.Add(new TObjString("MFI"),           
  ...
  //     new TObjString("psp_v13a.geo.root"));
   ...
   //-------------------------------------------------
    ...
    Int_t nEvents = 40960; // ~ 100 / crystal
//   Int_t nEvents = 100; // ~ 100 / crystal
