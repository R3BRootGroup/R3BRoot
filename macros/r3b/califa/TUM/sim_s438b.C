...
Int_t sim_s438b(const char *path, int ID){
  ...
  // Output files
  TString OutFile = Form("%s/sim_s438b_demo_iphos_%03d.root", path, ID);
  TString ParFile = Form("%s/testpar_s438b_demo_iphos_%03d.root", path, ID);
...
//detGeo.Add(new TObjString("MFI"),           
  ...
  //     new TObjString("psp_v13a.geo.root"));
   ...
   Int_t nEvents = 2*10240; // ~ 100 / crystal
