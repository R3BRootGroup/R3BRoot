...
void create_califa_geo(const char* geoTag)
{
  ...
  TGeoShape *pCBWorldIn1 = new TGeoTube("Califa_Centerpart1",  // hole to accommodate the tracker
                                     0.,  // Rmin
                                     26.4, // Rmax
                                     170./2.0);  // half length shortened by 5cm
  ...
  Color_t col_alveolus = kBlack;
  Color_t col_wrapping = kGray;
  Color_t col_crystal = kGreen;

  TVector3 origin = TVector3(0,0,5);
  
  //Inner radius of endcap
  Double_t inner_radius = 43.7;

  //Full length of endcap crystals
  Double_t l=15.;
  
  //number of endcap rings in polar angle
  const Int_t num_theta = 15;

  //number of endcap crystals in azimuthal angle
  Int_t num_phi1 = 30;
  Int_t num_phi2 = 60;

  //maximum number of endcap crystals in azimuthal angle
  //for array size
  const Int_t num_phi = 60;

  //theta angles for endcap rings
  // Double_t theta[num_theta+1] = {7, 12.33, 15.65, 18.46, 20.99, 23.36, 25.62, 27.81, 29.96, 32.07,
  // 				 34.17, 36.27, 38.37, 40.48, 42.61, 44.66};
  Double_t theta[num_theta+1] = {7, 12, 15.5, 18, 20.5, 22.5, 24, 25.5, 27.5, 29.5,
   				 31.5, 33.5, 35.5, 37.5, 40., 43.};
  
  //Thickness of alveoli
  Double_t thick_alv = 0.0;

  //Thickness of wrapping;
  Double_t thick_wrap = 0.005;
  ...
  for(unsigned int i = 0; i < 15; i++) {
    ...
    A.SetMagThetaPhi(inner_radius, theta[i]*TMath::Pi()/180., 0);
    B.SetMagThetaPhi(inner_radius, theta[i]*TMath::Pi()/180., dphi*TMath::Pi()/180.);
    C.SetMagThetaPhi(inner_radius, theta[i+1]*TMath::Pi()/180., 0);
    D.SetMagThetaPhi(inner_radius, theta[i+1]*TMath::Pi()/180., dphi*TMath::Pi()/180.);

    A2.SetMagThetaPhi(inner_radius + l, theta[i]*TMath::Pi()/180., 0);
    B2.SetMagThetaPhi(inner_radius + l, theta[i]*TMath::Pi()/180., dphi*TMath::Pi()/180.);
    C2.SetMagThetaPhi(inner_radius + l, theta[i+1]*TMath::Pi()/180., 0);
    D2.SetMagThetaPhi(inner_radius + l, theta[i+1]*TMath::Pi()/180., dphi*TMath::Pi()/180.);


    TVector3 AB, AC, CD, AB2, AC2, CD2;
    AB = A - B;
    AC = A - C;
    CD = C - D;
    AB2 = A2 - B2;
    ...
    Alveolus_EC[i] = gGeoManager->MakeTrap(Form("Alveolus_EC_%u", i + 1), pCarbonFibreMedium, l / 2, 0, 0,
					   AC.Mag() / 2, CD.Mag() / 2, AB.Mag() / 2, 0,
					   AC2.Mag() / 2, CD2.Mag() / 2, AB2.Mag() / 2, 0);
    Alveolus_EC[i]->SetLineColor(col_alveolus);
    CrystalWithWrapping[i] = gGeoManager->MakeTrap(Form("CrystalWithWrapping_%u", i + 1), pCarbonFibreMedium, l / 2 - thick_alv, 0, 0,
						   AC.Mag() / 2 - thick_alv, CD.Mag() / 2 - thick_alv, AB.Mag() / 2 - thick_alv, 0,
						   AC2.Mag() / 2 - thick_alv, CD2.Mag() / 2 - thick_alv, AB2.Mag() / 2 - thick_alv, 0); 
    CrystalWithWrapping[i]->SetLineColor(col_wrapping); 
    Crystal[i] = gGeoManager->MakeTrap(Form("Crystal_%u", i + 1), pCsIMedium, l / 2 - thick_alv - thick_wrap, 0, 0,
				       AC.Mag() / 2 - thick_alv - thick_wrap, CD.Mag() / 2 - thick_alv - thick_wrap, AB.Mag() / 2 - thick_alv - thick_wrap, 0,
				       AC2.Mag() / 2 - thick_alv - thick_wrap, CD2.Mag() / 2 - thick_alv - thick_wrap, AB2.Mag() / 2 - thick_alv - thick_wrap, 0);
    Crystal[i]->SetLineColor(col_crystal); 
    Alveolus_EC[i]->AddNode(CrystalWithWrapping[i],1,new TGeoCombiTrans(0,0,0, rotUni));
    CrystalWithWrapping[i]->AddNode(Crystal[i],1,new TGeoCombiTrans(0,0,0, rotUni));
    
    TGeoTranslation trans[num_theta][num_phi];
    TGeoTranslation ref;
    TGeoRotation rot[num_theta][num_phi];
    TGeoHMatrix ta[num_theta][num_phi];
    TGeoHMatrix *t[num_theta][num_phi];

    for(unsigned int j = 0; j < count_phi; j++) {
      trans[i][j] = TGeoTranslation(Form("trans_%u_%u", i, j), 0, 0,inner_radius + l / 2);
      rot[i][j]   = TGeoRotation(Form("rot_%u_%u", i, j),(j - 0.5) * dphi,(theta[i] + theta[i+1]) / 2,0);
