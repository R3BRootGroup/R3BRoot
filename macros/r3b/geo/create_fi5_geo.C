...
void create_fi5_geo(const char* geoTag)
{
  ...
  TString geoFileName = geoPath + "/geometry/fi5_";
  ...
  Float_t detector_height = 25.000000;
  Float_t fiber_thickness = 0.020000; 
  
  TGeoShape *pGFITube = new TGeoBBox("GFITube", fiber_thickness/2,detector_height/2,fiber_thickness/2);
  
  TGeoVolume* pGFILog = new TGeoVolume("FI5Log",pGFITube, pMed35);

  TGeoShape *pGFITubeActive = new TGeoBBox("GFITubeActive", (fiber_thickness * .98)/2, detector_height/2-0.0001, (fiber_thickness * .98)/2);

  TGeoVolume* pGFILogActive = new TGeoVolume("FI51Log",pGFITubeActive,pMed35);

  pGFILog -> AddNode(pGFILogActive, 0, new TGeoCombiTrans());
  
  pGFILog->SetLineColor((Color_t) 3);
