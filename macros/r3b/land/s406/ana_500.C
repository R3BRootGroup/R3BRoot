...
TString fileName = "/Users/kresan/neuland/r3bcalibr.s406.test.500AMeV.root";
...
//  h_etot_I->Scale(1./(Double_t)h_etot_I->Integral());
  ...
  c1->Divide(2, 1);
  TPostScript *ps1 = new TPostScript("plots/s406.498AMeV.etot.eps", -113);
  ps1->Range(32, 16);
  c1->cd(1);
  ...
  TLegend *leg1 = new TLegend(0.321, 0.739, 0.926, 0.882);
  Style(leg1);
  char str[100];
  ...
  gPad->SetTickx();
  gPad->SetTicky();
  h_etot_I->GetXaxis()->SetRangeUser(0., 100.);
  h_etot_I->DrawClone();
  h_etot->DrawClone("same");
  c1->cd(0);
  ps1->Close();
...
//  h_ep_I->SetMarkerStyle(20);
  ...
  TCanvas *c1 = new TCanvas("c1", "", 10, 10, 900, 450);
  c1->Divide(2, 1);
  TPostScript *ps1 = new TPostScript("plots/s406.498AMeV.paddle.eps", -113);
  ps1->Range(32, 16);
  c1->cd(1);
...
//   h_old->Draw("same");
  ...
  cout << h_ep->GetMean() << " +- " << TMath::Sqrt(h_ep->GetRMS()/(506.-2.)) << endl
  << h_ep_I->GetMean() << " +- " << TMath::Sqrt(h_ep_I->GetRMS()/(506.-2.)) << endl;
...
//  h_m_I->Scale(1./(Double_t)h_m_I->Integral());
  ...
  TCanvas *c1 = new TCanvas("c1", "", 10, 10, 900, 450);
  c1->Divide(2, 1);
  TPostScript *ps1 = new TPostScript("plots/s406.498AMeV.mult.eps", -113);
  ps1->Range(32, 16);
  c1->cd(1);
  gPad->SetLeftMargin(0.15);
  gPad->SetRightMargin(0.05);
