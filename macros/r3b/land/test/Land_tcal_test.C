void Land_tcal_test(){
	...
	if(mpar->GetTimeTacquila(i) != -10000){
	  failed = true;
	  cout << "Expected tac-time of channel " << i << " to be -10000 but is "  << mpar->GetTimeTacquila(i) << endl;
	}
	
      Double_t tol = 0.002;
      const Double_t step = 25./width;
      for(i = 0; i <= width; i++){
	if(mpar->GetTimeTacquila(start[pmt]+i-1) > ((i * step) + tol) || mpar->GetTimeTacquila(start[pmt]+i-1) < ((i * step) - tol)){
	  failed = true;
	  cout << "Expected tac-time of channel " << i + start[pmt]-1 << " to be " << i * step <<  " ± " << tol << " but is "  << mpar->GetTimeTacquila(i+start[pmt]-1) << endl;
	}
      }
	...
	if(mpar->GetTimeVFTX(i) != -10000){
	  failed = true;
	  cout << "Expected VFTX-time of channel " << i << " to be -10000 but is "  << mpar->GetTimeVFTX(i) << endl;
	}
	
      Double_t tol = 0.002;
      const Double_t step = 5./width;
      for(i = 0; i <= width; i++){
	if(mpar->GetTimeVFTX(start[pmt]+i-1) > ((i * step) + tol) || mpar->GetTimeVFTX(start[pmt]+i-1) < ((i * step) - tol)){
	  failed = true;
	  cout << "Expected VFTX-time of channel " << i + start[pmt]-1 << " to be " << i * step <<  " ± " << tol << " but is "  << mpar->GetTimeVFTX(i+start[pmt]-1) << endl;
	}
      }
