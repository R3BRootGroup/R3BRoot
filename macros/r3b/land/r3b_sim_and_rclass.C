...
//------------------- Read Data from LMD-ROOT file -------------------------------------
   R3BLandData* LandData = new R3BLandData("s318_172.root", "h309"); // file input and tree name
   int entries = LandData->GetEntries();                             // entries number
   TLeaf* leaf1;                                                     // define one leaf...
   TLeaf* leaf2;                                                     // define another leaf
   cout<<"Entries number in TTree = "<<entries<<endl;

   for (int i=0; i < entries; i++)
   {  
      leaf1 = LandData->GetLeaf("Nhe",i);                                 // Accessing TLeaf informations
      leaf2 = LandData->GetLeaf("Nht",i);                              // Accessing TLeaf informations
   //   LandData->FillHisto1D(leaf1);                                  // Plotting 1D histogram ... automatical bin width
   //   LandData->FillHisto1D(leaf1, bin, binmin, binmax);             // Plotting 1D histogram ... specified bin width
      LandData->FillHisto2D(leaf1, leaf2);                           // Plotting 2D histogram ... automatical bin width
   //   LandData->FillHisto2D(leaf1, leaf2, bin, binmin , binmax, bin, binmin, binmax); // Plotting 2D histogram ... specified bin width
   //####### for diferent analysis ###############
/*
...
*/
    ...
    //   LandData->Draw1D();                                            // Draw 1D histograms
       LandData->Draw2D();                                            // Draw 2D histograms
}
