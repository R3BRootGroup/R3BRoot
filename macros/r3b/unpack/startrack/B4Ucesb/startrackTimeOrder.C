...
void startrackTimeOrder(char* input, char* output)
{
  //TStopwatch timer;
  //timer.Start();
...
*/
  ...
  const int MAXsort= 5000; //do we need maximum number when we do not want to sort any more!?

  long long n_entries = input_Tree->GetEntries();
  cout << "n_entries(=number of blocks) = " << n_entries << endl;

// Loop over the blocks: 
    //for(long long i=0;i<1;i++){  // n_entries is the number of blocks in the run
    for(long long i=0;i<n_entries;i++){  // n_entries is the number of blocks in the run
    //for(long long i=0;i<3;i++){  // n_entries is the number of blocks in the run
...
// Loop over the hits in the block
	...
	if(my_type==2)Total_typ2++;
	  ...
	  for(long long j=0; j<n_loop; j++){	    //transport sorted data to output_tree
	    ...
	    my_new_det_id=branchModuleId->GetValue(index_hit_temp.at(j),0,true);     // when calling InsertionSort2
	    my_new_side=branchSideId->GetValue(index_hit_temp.at(j),0,true);  // when calling InsertionSort2
