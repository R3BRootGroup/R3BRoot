...
void RecResultsS438(char* output) {
  ...
  gStyle->SetOptFit(0);
	...
	*/
	...
	TH1F *Theta = new TH1F("Theta","Reconstructed from hit strips",1440,0.,180.);
	...
	for(Int_t i=0;i<nevents;i++){
	    ...
	    */
	    ...
	    for(Int_t j=0; j<TrackMult; j++){
	      ...
	      //Xa[j]=Ya[j]=Za[j]=Phia[j]=Epa[j]=0;  // inner
	      Xb[j]=Yb[j]=Zb[j]=Phib[j]=Epb[j]=0;   // middle
	      Xc[j]=Yc[j]=Zc[j]=Phic[j]=Epc[j]=0;   // Outer
		...
		*////////////////////////////////////////
		...
		// Now We are only interested in event with Multiplicity>=4 and the MIDDLE and OUTER layer are hit:		
		...
		//if(x>=2 && y>=2)  // At least inner and intermediate layer are hit (= better case than inner and outer layers because of straggling in intermediate layer 
		if(z>=2 && y>=2)  // At least outer and intermediate layer are hit (= better case than inner and outer layers because of straggling in intermediate layer 
		  {
		    ...
		    // 
		    // Basic tracking for 2 tracked particles in miidle and outer layer:
		    //
		       
		    if( fabs((Phib[0]-Phic[0]))<0.2 && fabs((Phib[1]-Phic[1]))<0.2 ) //&& fabs(Phia[0]-Phia[1])>(pi-0.5) )  // 0.1 radian Window of the azimuthal angle to accept that it is likely to be the same particle
		      {


			phi1= GetPhiTrk(Xc[0]-Xb[0], Yc[0]-Yb[0]);                   // one calculates the azimuthal angle form the position in inner and intermediate layer
			theta1= GetThetaTrk(Xc[0]-Xb[0], Yc[0]-Yb[0], Zc[0]-Zb[0]);  // one calculates the polar angle form the position in inner and intermediate layer
			//Ep1=Epa[0]+Epb[0]+Epc[0];
			Ep1=Epb[0]+Epc[0];

			phi2= GetPhiTrk(Xc[1]-Xb[1], Yc[1]-Yb[1]);
			theta2= GetThetaTrk(Xc[1]-Xb[1], Yc[1]-Yb[1], Zc[1]-Zb[1]);
