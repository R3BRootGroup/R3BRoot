    ...
    */
    ...
    for (auto it = bar_map.begin(); bar_map.end() != it; ++it)
    {
        ...
        for (; top_i < top_vec.size() && bot_i < bot_vec.size();)
        {
            ...
            if (std::abs(dt_mod) < c_bar_coincidence_ns)
            {
                ...
                // register multi hits
                if (iPlane == 1)
                    ++multPlane1.at(iBar - 1); // multihit of a given bar
                if (iPlane == 2)
                    ++multPlane2.at(iBar - 1); // multihit of a given bar
                if (iPlane == 3)
                    ++multPlane3.at(iBar - 1); // multihit of a given bar
                if (iPlane == 4)
                    ++multPlane4.at(iBar - 1); // multihit of a given bar
                ...
                new ((*fHitItems)[fNofHitItems++])
                    R3BTofdHitData((top_ns + bot_ns) / 2, top->GetBarId(), pos, 1, 1, top_tot, 1);
                ++top_i;
                ++bot_i;
            }
    ...
    for (Int_t i = 1; i <= fNofPlanes; i++)
    {
        ...
        for (Int_t j = 1; j < fPaddlesPerPlane * 2; j++)
        {
            ...
            {
                if (Q[i][j] > 0.)
                    fhChargeLosTofD->Fill(Q[i][j], LosQ);
            }
    ...
    */
    ...
    for (Int_t j = 1; j < fPaddlesPerPlane * 2; j++)
    {
        ...
        if (Q[1][j] > 0. && Q[2][j] > 0.)
        {
            ...
            Z_ave=(Q[1][j]+Q[2][j])/2.;
            */
            new ((*fHitItems)[fNofHitItems]) R3BTofdHitData((tof[1][j] + tof[2][j]) / 2.,
                                                            (x[1][j] + x[2][j]) / 2.,
                                                            (y[1][j] + y[2][j]) / 2.,
                                                            (Q[1][j] + Q[2][j]) / 2.,
                                                            (tof[1][j] + tof[2][j]) / 2.,
        ...
        else if (Q[1][j] > 0. && Q[2][j] < 0.001)
        {
            // cout << " PLANE 1" << endl;
            new ((*fHitItems)[fNofHitItems])
                R3BTofdHitData(tof[1][j], x[1][j], y[1][j], Q[1][j], tof[1][j], Q[1][j], 1);
            fNofHitItems += 1;
        }
        ...
        else if (Q[2][j] > 0. && Q[1][j] < 0.001)
        {
            // cout << " PLANE 2" << endl;
            new ((*fHitItems)[fNofHitItems])
                R3BTofdHitData(tof[2][j], x[2][j], y[2][j], Q[2][j], tof[2][j], Q[2][j], 1);
            fNofHitItems += 1;
        }
    }
