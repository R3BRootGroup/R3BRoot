...
InitStatus R3BPtofMapped2Cal::Init()
{
	if (fTcalPar == NULL)
	{
		// Dima: which type of log? Compare to below
		LOG(ERROR) << "Have no TCal parameter container" << FairLogger::endl;
		return kFATAL; // ToDo: check if this really stops execution
	}
...
{
	if (fCalItems) // Dima: need to check? exec should have already errored()
	{
		fCalItems->Clear();
		fNofCalItems = 0;
	}
