    ...
    if(1 == 1){
		...
		for (auto it = bar_map.begin(); bar_map.end() != it; ++it)
		{
			...
			for (; top_i < top_vec.size() && bot_i < bot_vec.size();)
			{
				...
				Double_t top_trig_ns = 0, bot_trig_ns = 0;
				if (top_trig_i < trig_map_tofi.size() && trig_map_tofi.at(top_trig_i) && bot_trig_i < trig_map_tofi.size() &&
					trig_map_tofi.at(bot_trig_i))
				{
				...
				if (std::abs(dt_mod) < c_bar_coincidence_ns)
				{
					...
					// calculate y-position
					auto pos = ((bot_ns + parTofi->GetOffset1()) - (top_ns + parTofi->GetOffset2())) * parTofi->GetVeff();

					// calculate y-position from ToT
					auto posToT =
						parTofi->GetLambda() * log((top_tot * parTofi->GetToTOffset2()) / (bot_tot * parTofi->GetToTOffset1()));
					...
					if (fTofdHisto)
					{
						...
						// a fhQvsPos[iPlane+fNofPlanes - 1][iBar - 1]->Fill(pos, parz[0] * TMath::Power(qb, parz[2]) + parz[1]);
						// fhQvsTHit[iPlane+fNofPlanes - 1][iBar - 1]->Fill(qb, THit);
		...
		if (fTofdHisto)
		{
			...
			{ // loop over all hits
				...
				{
					fhQ[event[ihit].plane+fNofPlanes - 1]->Fill(event[ihit].bar, event[ihit].charge);   // charge per plane
					fhQvsEvent[event[ihit].plane+fNofPlanes - 1]->Fill(fnEvents, event[ihit].charge);   // charge vs event #
					fhTsync[event[ihit].plane+fNofPlanes - 1]->Fill(event[ihit].bar, event[ihit].time);
					fhQvsPos[event[ihit].plane+fNofPlanes - 1][event[ihit].bar - 1]->Fill(event[ihit].ypos, event[ihit].charge);
