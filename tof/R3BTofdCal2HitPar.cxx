...
void R3BTofdCal2HitPar::Exec(Option_t* option)
{
    ...
    Double_t t2t=0.;
    Double_t timeLos=0;
    Double_t t[N_PLANE_TOFD_MAX+1][N_PADDLE_TOFD_MAX+1], Q[N_PLANE_TOFD_MAX+1][N_PADDLE_TOFD_MAX+1];
     
    for(Int_t i=1;i<=N_PLANE_TOFD_MAX;i++){
        for(Int_t j=1;j<=N_PADDLE_TOFD_MAX;j++){
            t[i][j]=0.;
            Q[i][j]=0.;	  
        }    
    }
