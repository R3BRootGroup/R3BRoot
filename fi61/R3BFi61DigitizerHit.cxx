...
R3BFi61DigitizerHit::R3BFi61DigitizerHit()
    : FairTask("R3B Fi61 Digitization scheme ")
    , fFi61Points(NULL)
{
...
R3BFi61DigitizerHit::R3BFi61DigitizerHit(Double_t e, Double_t t, Double_t x, Double_t y)
    : FairTask("R3B Fi61 Digitization scheme ")
    , fFi61Points(NULL)
{
...
void R3BFi61DigitizerHit::SetEnergyResolution(Double_t e) { esigma = e; }
void R3BFi61DigitizerHit::SetTimeResolution(Double_t t) { tsigma = t; }
void R3BFi61DigitizerHit::SetXPositionResolution(Double_t x) { xsigma = x; }
void R3BFi61DigitizerHit::SetYPositionResolution(Double_t y) { ysigma = y; }

InitStatus R3BFi61DigitizerHit::Init()
{
    ...
    fFi61Points = (TClonesArray*)ioman->GetObject("Fi61Point");

    fMCTrack = (TClonesArray*)ioman->GetObject("MCTrack");

    // Register output array DchDigi
    fFi61Hits = new TClonesArray("R3BFiberMAPMTHitData", 1000);
    ioman->Register("Fi61Hit", "Digital response in Fi61", fFi61Hits, kTRUE);
...
void R3BFi61DigitizerHit::Exec(Option_t* opt)
{
    Reset();
    ...
    auto Digitize = [this](TClonesArray* Points, TClonesArray* Hits, Int_t NumOfFibers) {
        ...
        for (Int_t i = 0; i < entryNum; ++i)
        {
            ...
            LOG(debug) << "Point Data fi61 - x: " << (data_element->GetXIn() + data_element->GetXOut()) / 2.
        ...
        for (TempHit& Hit : TempHits)
        {
            ...
            LOG(debug) << "Hit Fi61 in: fiber: " << Hit.fiberID << " x: " << Hit.X << " y: " << Hit.Y << " z: " << Hit.Z
        ...
        for (Int_t i = 0; i < NumOfFibers; ++i)
        {
            for (Double_t& energyl : energy[i])
            {
                if (energyl > 1.e-12)
                {
                    ...
                    // cout<<setprecision(10) << "Hit Fi61 local-x from fiberNum: " << xx<<" from transf: " << x_local
                    ...
                    // cout<<setprecision(10)<< "Hit Fi61 global-dx,dy,dz: "<<posGlobalcheck.X()-posGlobal.X()<<",
                    ...
                    if (granularity)
                    {
                        ...
                        x_local = (int)((x_local + fiber_gran / 2.) / fiber_gran) * fiber_gran;
    ...
    if (fFi61Points)
    {
        Digitize(fFi61Points, fFi61Hits, fiber_nbr);
    }
}
// -------------------------------------------------------------------------

void R3BFi61DigitizerHit::Reset()
{
    if (fFi61Hits)
        fFi61Hits->Clear();
}

void R3BFi61DigitizerHit::Finish() {}

ClassImp(R3BFi61DigitizerHit)
