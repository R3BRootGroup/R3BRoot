...
R3BFi61DigitizerCal::R3BFi61DigitizerCal()
    : FairTask("R3B Fi61 Digitization scheme ", 1)
    , fFi61Points(NULL)
{
}

R3BFi61DigitizerCal::R3BFi61DigitizerCal(const char* name, Int_t iVerbose)
...
InitStatus R3BFi61DigitizerCal::Init()
{
    ...
    fFi61Cals = new TClonesArray("R3BFiberMAPMTCalData", 1000);
    ioman->Register("Fi61Cal", "Digital response in Fi61", fFi61Cals, kTRUE);

    fFi61TriggerCals = new TClonesArray("R3BFiberMAPMTCalData", 1000);
    ioman->Register("Fi61TriggerCal", "Digital response in Fi61", fFi61TriggerCals, kTRUE);
...
void R3BFi61DigitizerCal::Exec(Option_t* opt)
{
    Reset();
        ...
        for (Int_t i = 0; i < NumOfFibers; ++i)
        {
            for (Double_t& energyl : energy[i])
            {
                if (energyl > 0.0001)
                {
                     ...
                     *  MAPMT = 256
                     *  SPMT = 1
                     * This means we'll receive 512 MAPMT channels as 1..512, and 2 SPMT
                    ...
                    /*
                      ...
                      if (i > 255)
                          ichanSA = 2;
                      */
                    // cout << "Fi61, Fiber: " << i << " MAPMT channel: " << ichanMA << " SA channel: " << ichanSA <<
                    // endl;
    ...
    if (fFi61Points)
    {
        Digitize(fFi61Points, fFi61Cals, fFi61TriggerCals, fiber_nbr);
    }
}
// -------------------------------------------------------------------------

void R3BFi61DigitizerCal::Reset()
{
    if (fFi61Cals)
        fFi61Cals->Clear();
    if (fFi61TriggerCals)
        fFi61TriggerCals->Clear();
}

void R3BFi61DigitizerCal::Finish() {}

ClassImp(R3BFi61DigitizerCal)
